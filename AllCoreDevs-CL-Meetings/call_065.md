...
* We’ve done today a pre-release for the first time, so we’ve done a bunch of outgoing comms to users to tell them not to run it on sila-mainnet and explain what we’re doing with it. If there’s any power users on this call running Lighthouse, I'd suggest giving it a run on testnets – theres some good memory improvements and we’d like to see how they run on lots of different hardware.
...
* Assuming that goes well, and we get some fuzzing action going, and we continue to enhance the tests, that would be a positive signal towards moving towards picking upgrade dates on our two existing testnets. Probably doing another testnet in the meantime, and then looking towards sila-mainnet.
...
* 3) We need something that doesn’t interfere with existing validators on sila-mainnet today that use the 0x0 or 0x1 prefixes. Based on some discussion we’ve had in the Sil R&D discord, it turns out there is 3200 validators that use the 0x1 prefix on sila-mainnet right now. They didn’t sign up for any of the changes that need to be made to support a solution to this problem, and we don’t want to enforce those on them. So we need something that doesn’t interfere with all of the validators today.
