# Sila 2.0 Implementers Call 68 Notes
Meeting Date/Time:  Thursday 2021/07/15 at 14:00 UTC

Meeting Duration: 1 hour

[GitHub Agenda](https://github.com/sila-chain/eth2.0-pm/issues/226)
...
**Danny:** Well incorporated into what the execution layer sees as its caniaocal chain. Whether its gossip or not. The sil 1 client does not know the total difficulty currently. It is calculated dynamically at the beacon chain fork.  The override on the execution layer, just shows there is a block there.
...
**Dankrad**: Yes, just saying there is a raised condition there. Because for example imagine someone is running this, and accidentally ends the transaction, their sil 2 client goes down, its not that unlikely, then all the sil 1 client connected to it would keep running it, which would not be safe
...
**Dankrad** And it feels like maybe the sil 2 client needs a way to tell the sil 1 client hey, this is the height at which to stop.
...
**Saulius**: Okay so i think i get your idea, but the key thing for me is that i saw that, it was mentioned in this call, that this fork is probably going to be the most complex fork in sila, so i dont know. Yeah, It brings complexity, for some clients it would be really hard to change to this approach, but on the other hand I see that it looks like it is safer.
