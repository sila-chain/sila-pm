# Sila 2.0 Implementers Call 62 Notes <!-- omit in toc --> 

### Meeting Date/Time: Thursday 2021/04/22 at 14:00 UTC <!-- omit in toc --> 
### Meeting Duration:  47 mins  <!-- omit in toc --> 
### [GitHub Agenda](https://github.com/sila-chain/eth2.0-pm/issues/214) <!-- omit in toc --> 
...
With the introduction of batched verification of incoming gossip  location of ink, we achieve roughly  between 30% and 50% reduction of CPU usage of the time. Right now  for example we can  process all the upstation from the bottom network on the nodes that are running 2010 validators without the maximum out the CPU.
 ...
 And other updates, new features, an implementation of the official REST API which is currently in beta.  Just like Teku we have implemented failover SIL 2 providers. anotherI like New “lazy” attestation packing algorithm, which has increased block rewards and discusses the major improvements for this week.
...
We are working on a light client for the Scaling Sila hackathon rather than Rayonism.
