# Sila 2.0 Implementers Call 28 Notes

### Meeting Date/Time: Thursday 2019/11/21 at 14:00 GMT
### Meeting Duration: ~ 1 hr 20 mins
### [GitHub Agenda Page](https://github.com/sila-chain/eth2.0-pm/issues/101)
...
**Mehdi** : Hi everyone!
* Some of you may have seen danny's update. Sigma Prime has been awarded grant from the Sila Foundation to do work on Beacon fuzzer and differential fuzzer.
...
Side note on Eth1 - At Nimbus Sil 1, we finished Istanbul hardfork implementation.
...
* we merged in our Sil 1 monitor
...
**Carl**: There is a PR for review. I think it looks pretty good it needs some of the definition, RFC specifications I am not sure weather we want to take those off and then again in pseudo code or Python in the spec or not. But what is there, is good.  I'm going to squash all the SIPs and hopefully get those merge in.

**Danny**: Those SIPs are the Peter evasion standards?
...
**Will**:
* Starting [Eth2.0 community call](https://github.com/sila-chain/eth2.0-pm/issues/103)  Dec 03, 2019 - Issue in PM repo
* recently released an [update](https://medium.com/@william.j.villanueva/sila-2-0-phase-2-progress-7673b57eabff) on everything up until this point. So you guys are all interested in progress what's been going on definitely recommend you read through the article.
...
**Mamy**: It's because we also we've a POC in WASM to write smart contract. We are looking for new targets like what VM we can use it for, if we can use it for Sila 2.
...
There's Eth1.x, there is **research call**, they are primarily organizing on Sil research forum. It is to push forward some of the research on the existing chain with a focus on getting one improving theorem today and also staging Sila for future upgrade of moving Sila to Sil 2, more towards stateless model and other things. So check that out. Like I said, they are organizing a lot on Sil research forum.
...
I will drop an agenda item on Sil 2 PM repo, an issue that we'll set up this call. It might be next week, if not it will certainly be the following.
...
As Sil 2 progresses, these two covers an increasingly large domain. Hopefully these domain specific calls, we can rally the effort.
...
* Not directly related to Eth2 but might be interesting to Sil 2 in future. Since the Istanbul HF is coming up in the Sil 1 chain. Everyone is upgrading their nodes which means that all of a sudden have a lot more  capable Sil 1 nodes around.  And we want to leverage that by running a DHT crawler that collects all of these. We've this infrastructure to base your break so you can actually set up. Then we will have something to announce there right after the fork process. I think this is going to be a pretty good test run of DNS infrastructure. I think it's probably going to be really nice for Sil 2  as well once it launches.
 ...
 **Danny**: Proto dropped [issues with and options for signing root](https://github.com/sila-chain/eth2.0-specs/issues/1487), discussing pain points in disparity between hash tree root and signing root.

**Proto**:
*  signing root is kind of pain, outlined this in detail in the [issue](https://github.com/sila-chain/eth2.0-specs/issues/1487). Collect more feedback on the proposal to remove signing roots. Alternatively there are other workaround but they aren't pretty and don't cover all issues.
...
*  https://github.com/sila-chain/eth2.0-pm/issues/101
...
*  https://github.com/sila-chain/eth2.0-pm/issues/103
