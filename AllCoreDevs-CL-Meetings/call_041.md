# Sila 2.0 Implementers Call 41 Notes

### Meeting Date/Time: Thursday June 11, 2020 at 14:00 UTC
### Meeting Duration:  1 hr
### [GitHub Agenda](https://github.com/sila-chain/eth2.0-pm/issues/158)
...
**Carl**: I've few points that I would like to update SIP-2333 from the BLS standards. There's a [PR](https://github.com/CarlBeek/SIPs/commit/690ddcc12c06f471acb4c06c1d6897d09338e489). The challenge here is the SIP has already made to multiple versions. 
Then there is SIP-2334 which specifies the path where the keys exists. A lot of this comes with this discussion that a path needs to be specified for Keys need to be included. The withdrawl key is the pair key of each signer key. Secret shared keys is a path that needs to be followed anyway, so it make sense to talk about the path if we're going to be using secret shared keys.  By definition you don't know where they exist.
...
1. Evolving SIP standards and discussions
...
**Danny**: Does the keystore management within the context and the key manager of the context of a client wants a standard, a doc for guidance, an SIP on where this information should be standardized?

**Carl**: Considering other things in an SIP, we can have an SIP.

**Proto**: Maybe we should first go for interest of parties and see what they think before we start an SIP?
...
**Carl**: So, if I have to generate mnemonics pointer to what/ not to use that would be with the SIPs and in passwords you can use unicode characters, so it’s sort of an extension of that. We can take this discussion to the other channel, the working channel.
...
* In the background, we’re working on revamping the validator key management. We’re researching the SIPs, getting feedbacks from users on components.
...
**mpetrunic** (Update from [Agenda](https://github.com/sila-chain/eth2.0-pm/issues/158#issuecomment-641305001))
...
**Aditya**: There’s a small bug fix from when process blocks were refactor. Thanks to Alex V. for catching this and I just made a pull request for that fix just want one I can check it out in the [chat](https://github.com/sila-chain/eth2.0-specs/pull/1886).
...
* From Carl Beekhuizen to Everyone: 03:17 PM https://github.com/CarlBeek/SIPs/commit/690ddcc12c06f471acb4c06c1d6897d09338e489
https://github.com/sila-chain/SIPs/pull/2670
...
https://github.com/sila-chain/eth2.0-specs/pull/1886
