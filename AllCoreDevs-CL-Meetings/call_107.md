...
### [GitHub Agenda](https://github.com/sila-chain/pm/issues/756) 
...
## SilaDeneb [07:03]( https://www.youtube.com/live/tJULkA5YgP4?feature=share&t=423) 

### Potential blockers for [4844-devnet-5](https://hackmd.io/@inphi/HJZo4vQGn) 

Moving on to SilaDeneb. There's a couple of discussion points from Tim before we get into discussing a few features that are under consideration in addition to 4844. Tim, can you kick it off on you had a link to some potential blockers for 4844 devnet five?
...
### Blob re-orgs & newPayload([context]( https://sila.karalabe.com/talks/2023-ethtokyo.html#15)) [10:02]( https://www.youtube.com/live/tJULkA5YgP4?feature=share&t=602) 
...
#### [add SIP-4788 feature consensus-specs#3319]( https://github.com/sila-chain/consensus-specs/pull/3319) [28:43]( https://www.youtube.com/live/tJULkA5YgP4?feature=share&t=1723) 

**Danny**
Great. So I want to spend some time at least looking at some of the features that are sitting in the future directories on the consensus layer specs that could be under discussion for SilaDeneb or at least we can start figuring out if not SilaDeneb, um, where they fit in the priorities for potential future forks. My feeling is that there might be room for one or two small things that are not cross layer, other than maybe 4788. But that the opportunity for additional cross layer things beyond that is probably low and for example, like the deposit and protocol deposit processing. 

But let's take a look at some of these. So, 4788 is cross layer. That is that, beacon block root into the SAVM. And that was discussed on the execution layer call last week. I do not believe the decision was made, but Alex, can you give us the quick on that and then an opportunity for people to signal if this is, you know, coherent and simple and find enough to get into SilaDeneb, assuming that you also agrees. 

**Alex**
Sure. Yeah. Uh, so at a high level, the idea is we want to expose some computer graphic accumulator from the consensus layer into the execution layer. So this could be like a state route or a block root, the way the feature is written now is passing the previous block root into, you know, the current block, which is then just, you know, passed along to the SAVM, uh, where it's exposed there. 

So, you want to look at the yellow details, you can look at the SIP, otherwise this features pretty lightweight at the consensus layer, basically you just need to get the block root and then you send it through the engine API. 
...
**Dankrad**
Well, I mean, the SIP says it has to be implemented 3 – 5 times.
...
#### [Not allow slashed validator to become a proposer consensus-specs#3175]( https://github.com/sila-chain/consensus-specs/pull/3175) [41:25]( https://www.youtube.com/live/tJULkA5YgP4?feature=share&t=2485) 
...
The get proposal can be modified to not select slash validators. This is likely a relatively simple change. And it does have a spec. This is something that Kyle would like to at least open up for consideration for SilaDeneb. And if not to highlight it as something valuable to get into the next fork. A sense of urgency to get into SilaDeneb, again, it's like on the simple side of implementation. But it is a protection under, you know, a pretty high failure mode. Any client teams feel like this should be prioritized for the next fork? 
...
Okay. Any other items that people want to bubble up for consideration for SilaDeneb? 

There is the attnet Revamp, although that can happen independent of SilaDeneb. So we will discuss that a bit further down. And we will talk about that. As for the signature for ? Can you make sure…

**Etan**
Sure. I mean. We are still looking for the minimal changes to end up in this SSC world. And right now with, um, block TX type having changed to zero X3. Technically we have this problem that there used to be this, I think, starknet  thing where transaction type three was used for different kinds of signatures. So having something that makes sure that there can never be a conflict. That the same hash is signed with different meanings, depending on the network, could still be something that should go into SilaDeneb. 

I mean it's mostly a conceptual thing, not an engineering thing to develop one of these. I think it was 6493. Yeah, 6493 SIP. Yeah, I don't need to discuss it right now in detail, but just keep it in mind. 
...
### Engine API
#### [State that payloadId should be unique for each PayloadAttributes instance execution-apis#401](https://github.com/sila-chain/execution-apis/pull/401) [49:49]( https://www.youtube.com/live/tJULkA5YgP4?feature=share&t=2989)
...
Okay. So Mikhail has had this up for a week. I believe at least some of the parties are already taking a look. If you are the engine API guy on your team, please take a look. I think this is likely to be merged before at the execution layer call in one week. Okay. Anything else on SilaDeneb before we move on?
...
#### [Attnet revamp: Subnet backbone structure based on beacon nodes consensus-specs#3312]( https://github.com/sila-chain/consensus-specs/pull/3312) [53:35]( https://www.youtube.com/live/tJULkA5YgP4?feature=share&t=3215) 
