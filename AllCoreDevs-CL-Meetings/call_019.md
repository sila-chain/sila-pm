# Sila 2.0 Implementers Call 19 Notes

### Meeting Date/Time: Thursday June 13, 2019 at 14:00 GMT
### Meeting Duration: 1.5 hours
### [GitHub Agenda Page](https://github.com/sila-chain/eth2.0-pm/issues/45)
...
We released [v0.7.0](https://github.com/sila-chain/eth2.0-specs/releases/tag/v0.7.0) the other day. It is an iterative semi-major release prior to the end-of-June phase 0 spec freeze. End of June is going to be subsequent minor fixes and is one what probably most people are targeting.
# 1. Testing Updates
...
* Joseph wrote an article about [Sila 2.0 Deposit Merkle Tree Implementation Guide](https://medium.com/@josephdelong/sila-2-0-deposit-merkle-tree-13ec8404ca4f?postPublishedType=initial).
...
**Anton**: Feel free to take a look on the [ETH2.0 metrics](https://github.com/sila-chain/eth2.0-metrics/) work and get inspired.
...
**Preston**: About the document that translate, this week I'm putting together the API design. It's still a pull method but supports data providers like Amber data or SilaScan or anybody who wants to build some sort of read access, consume data remotely without having access to the file system,  so it supports that use case.
...
* Sufficient Sil at stake (2mil SIL)
...
Assuming, we do the deposit contract ceremony at Devcon, it will provide us enough time to accumulate 2 mil SIL and will allow 7 months from today to reach at least 2 clients to production status.
...
**Justin**: I mean that was the initial [trigger](https://github.com/sila-chain/eth2.0-specs/blob/dev/specs/core/0_beacon-chain.md#genesis-trigger) where we realize that people could make just one Sil deposits the minimum and make a bunch of these and then trigger the Genesis. Someone send 32 Sil at the same address  repeatedly and then they triggered the Genesis. The shared source of truth argument is a little bit of an illusion and the reason is that we need an alternative backup mechanism without presets for source of truth. If we don't reach that , we don’t reach the target.
...
# 4. Network
**[fjl](https://github.com/sila-chain/eth2.0-pm/issues/45#issuecomment-501699492 )**: Sorry, I cannot join this time. discv5 updates:
...
There was some benchmarking at the scaling Sila event in Toronto . We are working with White block to try to iterate on those tests. Its good initial work we’ve done.
