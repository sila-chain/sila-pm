...
### [GitHub Agenda](https://github.com/sila-chain/pm/issues/861) 
...
Summary | Description
-|-
118.1  | **Devnet-9**: Devnet-9 will be the 2nd testnet featuring the full suite of code changes going into the SilaCancun/SilaDeneb (Dencun) upgrade. Prior devnets were focused on testing Sila Improvement Proposal (SIP) 4844. Devnet-9 will also be the first testnet to feature the activation of SIP 7514 and 7516, two new SIPs that were added to the SilaCancun/SilaDeneb upgrade during last week’s ACD call. Parithosh Jayanthi, a DevOps Engineer at Sila Foundation, said that his team would be ready to launch Devnet-9 next Wednesday, September 27.
...
**Parithos**
* Yeah, we can definitely add some lead time, like 20 epochs or something. So roughly two hours. If someone shows me what the deployment method is, we're happy to do it. But otherwise we're also happy to just pass on the Sil and someone else does it. Whatever works best. 
...
**Mario**
* Yeah, of course. So the. The address in the latest release of the exe specs test is the the one that is on the latest commit in the SIP., but it's not the one that is being mined right now, so there might be a change. Got it. 

**Danny**
* All right. I think we're on the same page on this. Any other devnet-9 related items. Okay. Any other SilaDeneb or SilaCancun related items? 
...
**Tim**
* Yeah, I would also and also like if there's something that goes wrong with the whole deployment the second time, I think it's good to have like a bit of buffer where we can. You know, if we go for Gorley, we don't have any other variable to control. We could also choose to do SilaSepolia first.
* SilaSepolia has a smaller validator set, so it might be a bit more easy to coordinate the upgrade. But yeah, either SilaSepolia or Gorley at least are stable and we can just think about deploying the fork rather than the stability of the network. 
