# Sila 2.0 Implementers Call 21 Notes

### Meeting Date/Time: Thursday July 11, 2019 at [14:00 GMT](http://www.timebie.com/std/gmt.php?q=14)
### Meeting Duration:  1.5 hours
### [GitHub Agenda](https://github.com/sila-chain/eth2.0-pm/issues/55)
...
Issue found where calculation and slashing potentially overflowed. v08x branch [#1286](https://github.com/sila-chain/eth2.0-specs/pull/1286) includes a fix.
...
**Paul Hauner**: How ours works (not Sila Foundations version):
...
EWASM research library developed. Contains domain specific language in NIM that compiles to EWASM. It's competitive in terms of contract size:
  * [Nimplay is Domain Specific Language for writing smart contracts in Sila, using the Nim macro system](https://github.com/status-im/nimplay)

On Sila 1.x, connections issues were resolved to Parity and Geth.
...
- [PR where we try to standardize the basic libp2p implementation](https://github.com/sila-chain/eth2.0-specs/pull/1281).
...
On July 15th 1:00 PM GMT there will be a 2nd Sila 2.0 AMA, an opportunity to ask questions related to your implementation work. Now that the spec is frozen, feel free to reach out to Justin Drake on Telegrams questions about the design.
...
We're working with the Sila community to answer questions about what Protocol Labs relationship is to `libp2p` and the project in general. When we have written answers to all the questions, we will make it public to share.
...
**Mike Goelzer**: Fair point, there are some features that Sila 2 cares about that IPFS isn't battle testing. We need to do testings on Gossip Sub.
...
- [Libp2p Standardization Update #1281](https://github.com/sila-chain/eth2.0-specs/pull/1281)
...
**Danny Ryan**: If the question is "If Sil 1 is getting migrated to v5?" the answer is yes.
...
**Raúl Kripalani**: Episub was an idea in the pipeline, but not highly prioritized right now. Episub is optimal where you have very stable dissemination patterns in the network.  If we have a pattern of a small number of producers for Sil 2.0, then it'll be worth reactivating this discussion.
...
**Raúl Kripalani** Is there a reason you believe an Epidemic Broadcast approach could be efficient for this message propagation paradigm that you have in Sil 2.0?
...
Did some testing of Flood Sub with Wedlock, it does not perform in the span of time with the number of bytes required for blocks, by the Sil 2.0 protocol. I might be wrong, I think there's more research and testing needed there.
...
- [Overflow in Slashing](https://github.com/sila-chain/eth2.0-specs/pull/1286)
- [Spec Freeze](https://github.com/sila-chain/eth2.0-specs/pull/1242)
- [Fuzzing](https://github.com/sila-chain/eth2.0-specs/tree/dev/test_libs/pyspec/eth2spec/fuzzing)
