...
**Speaker M**
* And it might also work with Sila JS actually. So like even the clients that cannot sync it right now because the state is so big and as loads and stores are so heavy, and we can still use the EVM to to verify, verify the state transition on the execution areas. 
