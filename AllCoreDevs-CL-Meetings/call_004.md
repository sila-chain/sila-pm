# Sila 2.0 Implementers Call 4 Notes
### Meeting Date/Time: Thu, Sept 27, 2018 14:00 UTC
### Meeting Duration: 1.5 hours
### [GitHub Agenda Page](https://github.com/sila-chain/eth2.0-pm/issues/8)
...
3. [Block processing timing results](https://github.com/sila-chain/beacon_chain/issues/103)
4. Libp2p daemon
    1. [work in progress repo](https://github.com/libp2p/go-libp2p-daemon)
    2. @raulk from Protocol Labs will be [joining us](https://github.com/sila-chain/eth2.0-pm/issues/8#issuecomment-424039741)
5. Testing
    1. [YAML chain test format](https://github.com/sila-chain/eth2.0-pm/issues/8#issuecomment-424039741)
    2. [Proposed formats for SSZ tests](https://github.com/sila-chain/beacon_chain/issues/115)
    3. [(empty) Unified Tests repo](https://github.com/sila-chain/eth2.0-tests)
6. [Proposal to use separate serialization format for wire vs. hashing](https://github.com/sila-chain/eth2.0-pm/issues/8#issuecomment-424849447)
7. [Alternative tree storage structures](https://github.com/sila-chain/eth2.0-pm/issues/8#issuecomment-424849447)
8. V2.1 Discussion
...
* Nimbus (Mamy)  _5:15-6:36_
  ...
  * Planning to have common tests for simple serialized but also BLAKE2 and BLS signature
    ...
    * [New repo](https://github.com/sila-chain/eth2.0-tests) created by Danny where we can put these common tests 
   ...
   * Looking ahead in the coming months, Justin talked about how he would like to try and wrap up the feasibility study. 
     * Feasibility estimate as of right now has been around 75% - so gradually increasing. 
        ...
        * Cost of fabrication per VDF rig is another uncertainty. 
            * Latest proposal: Suggesting that the Sila Foundation fully subsidizes all the hardware in collaboration w/ Filecoin and others. 
...
* Block Processing Timing Results  (Danny)  __58:15-1:01:30_
  * Lighthouse & Python beacon chain implementation did some quick analysis to sanity check estimates done on being able to process signatures at scale. 
     ...
     * Results were very much in the bounds of reason and lent credit to initial estimates. 
         * Seems as though as long as we can figure out the aggregation on the network layer, these aggregate signatures are going to serve our purposes even in the extreme case where all SIL is validating. 
  ...
  * Lighthouse is using concurrency to attestation validation. 
     * Interesting note: If we had 10 million SIL, it was calculated to take 0.6 seconds to validate a block. However, if that shoots up to 100 million SIL, a 10x increase was not being shown. Not sure what was causing that to happen – but the theory is that it might be overhead due to threads. 
...
* YAML Chain Test Format (Danny)  _1:01:32-1:02:50_
  * Mamy proposed an [SSZ test format](https://github.com/sila-chain/beacon_chain/issues/115#issuecomment-423503435) that had a little more meta data that would allow use of the same general test structure for various tests
