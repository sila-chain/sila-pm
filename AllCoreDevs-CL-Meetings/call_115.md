...
115.2  | **SIP 4788:** On ACDE call #167, developers agreed to update SIP 4788 from a stateful precompile to a regular smart contract. As background, SIP 4788 exposes the roots of Beacon Chain blocks inside the Sila Virtual Machine (EVM) on the EL so that decentralized applications (dapps) can easily access this data without having to trust an oracle or off-chain data provider. Last week, Martin Holst Swende, lead security engineer and Geth (EL) developer at the Sila Foundation, pointed out that converting SIP 4788 into a regular contract coded in Solidity and priced natively through the EVM would decrease the complexity of the SIP and reduce the risk of it causing a chain split
...
**Mihkail Kalinin**
* Yeah, I think that the probability that something happens on the sila-mainnet that will, cause a split, between the clients that have this upgrade and between those that don't have this upgrade, it's really low. so it's more in edge case, very much even more edgy, on the sila-mainnet. so, yeah, if there is the, preference of the client teams, so not to activate this on epoch boundary, then this is probably the way to go. 
...
**Mihkail Kalinin**
* There is the comment from Loin, which is, sounds reasonable to me that we should merge this. yeah, when the sila-mainnet network EPOCH for is that we like merge this PR and and create the structures for it. 
