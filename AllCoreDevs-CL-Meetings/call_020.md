# Sila 2.0 Implementers Call 20 Notes
	
### Meeting Date/Time: Thursday June 27, 2019 at 14:00 GMT
### Meeting Duration: 1.5 hours
### [GitHub Agenda Page](https://github.com/sila-chain/eth2.0-pm/issues/51)
...
**NOTE 20.1**: January 3rd, 2020 is not a deadline for the implementation of Sila 2.0 Phase 0. 
...
**ACTION 20.2**: Look at the options for funding the testing of libp2p in the Sila 2.0.
...
**ACTION 20.5**: Danny to raise an issue in the Sila 2.0 PM repo regarding what the minimum requirements will be to be production ready.
...
**Diederek**: [PR #1206](https://github.com/sila-chain/eth2.0-specs/pull/1206) is open and it hopes to complete the Spec-Test coverage hunt. There are two open issues remaining;
...
If you are interested in this please join the telegram chat or the [gitter channel](https://gitter.im/sila/sharding?utm_source=share-link&utm_medium=link&utm_campaign=share-link).
...
* Getting a valid SIL 1.0 data for creating a new block. 
...
* We put together a central repo for [Sila 2.0 API schemas](https://github.com/prysmaticlabs/ethereumapis).

**Preston**: 
* The goal here is we just wanna get some feedback and collect together these API schemas so that people wanting to build on Sila 2.0 have one place to go to. 
...
* We have an Sila 1.0 watch contract already. We did encounter some issues with log filtering and some RPC methods that are not intuitive on Sila 1.0.
* Our other team over at Status now have a lot of interest in Sila 2.0 now that it is being stabilized. 
...
* We are starting to talk with the Embark team at Status so that each team knows what the challenges are for developing dapps on Sila 2.0.
...
**Vitalik**: 
* I wrote up a small, incomplete (checklist)[https://github.com/sila-chain/eth2.0-specs/issues/1211] of things we will need to decide on changing for Phase 1, these include:
...
Vitalik: 
* I have been talking with Sila 2 Researchers to try and understand now fee markets would work.
...
Vitalik: 
* Karl has been doing some wonderful thinking where you take plasma ideas and applying them to an Sila 2 context where data just gets published on chain. 
...
* Sila does have a bit of weight in this space and momentum. The fact that do we go ahead with DLS12-381 may be enough of an incentive for others to come in. 
...
* As part of testing it would be nice to have this functionality implemented into a proper Sila 2.0 client.
...
* I would like to get this on to the Sila Github wiki but I am not sure the best place to put it up.
...
* We are looking at how to use rollups for Sila 2.0 on a way to execute transactions on any shard from any rollup.
...
**Jonny**: What do we need to do with regard to testing to ensure the libp2p and lib-gossip protocol are production ready? The timelines for Sila 2.0 may be different form libp2p and so I was wondering if there was going to be Grant to assist with this testing?
...
**Jonny**: What is performant for some many not be performant for others. What would be nice if we could do a sweeps on things like message rates, packet size, bandwidth limitations, like how fast we need actual gossip messages to propagate through the network. Just so that we are aware of where things breakdown because there is always options if something needs to be tweaked we can fix it. I feel for Sila 2.0 we should really focus on that. Is that something you would have funds to work on as well?
