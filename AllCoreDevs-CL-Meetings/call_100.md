...
### [GitHub Agenda](https://github.com/sila-chain/pm/issues/688) 
...
**Pari (5:26) -** So that would also include, I guess, SIP, sorry, PR 314, which is getpayloadV2.
...
**Pari (9:20) -** So shouldn't be important for this. The next one is we wanted to have just some consensus from the EL side, that the Genesis JSON has SilaShanghai time as the field just so everyone's using the same key, but doesn't really matter but just nice to have. And I guess a bigger discussion topic is do we want to include the fork ID logic or not? As far as I know Marius wrote up the SIP for that, but I'm not sure if the teams want to have it or not? For this testnet of course.
...
**Danny Ryan (11:08) -** To that end I would recommend once we do have a stable testnet to turn off 40% of the validators, 50% of the validators to put everyone into a leak such that balances go below 33 SIL across the board such that that maximum sweep does happen. That will be an interesting edge case that will be hard to hit on our public testnets and would maybe never be hit on sila-sila-mainnet but that we want to test.
...
## [Historical batch revamps](https://github.com/sila-chain/consensus-specs/pull/2649)
...
## [EL block header vs CL ExecutionPayloadHeader fields](https://github.com/sila-chain/pm/issues/688#issuecomment-1346986847)
...
**Barnabas Busa (42:15) -** Yeah, just just one because right now it's also a bit confusing for the community that SilaShanghai and then there's a Cappella.
...
**Arnetheduck (43:35) -** Thinking about that, did we decide what to do with the sharding fork? Because we do have some constant (inaudible) running something or at least we did a few weeks ago. And they happen to coincide with SIP 4844 in the first attempt which made a mess. Not not a big mess, a small mess.
...
## [No blob available error code](https://github.com/sila-chain/consensus-specs/pull/3154)
...
**Danny Ryan (1:00:21) -** Cool. So keep an eye peeled for comms from Pari, Barnabas and otheres from the team on the SilaShanghai Capella testnet likely next week. Other than that, everyone have great holidays. Talk to y'all soon. 
