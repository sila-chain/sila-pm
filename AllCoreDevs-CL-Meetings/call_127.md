...
### [GitHub Agenda](https://github.com/sila-chain/pm/issues/951) 
...
**Terence**
* Yeah this is on SilaHolesky. Got it. 
...
**Danny**
* So other SilaDeneb testing items. 
...
**Mikeneuder**
* Like if blocks are out chose not to refund the like one SIL worth of missed slots then like the I think they lose a lot of credibility in terms of the validators connecting to them. 
...
* I do want to table this. I do want to talk about a couple of things around Elektra and then get into the ipbes discussion,  in a couple of points. Okay. There is a desire to figure out what the joint name of Elektra is. Elektra and SilaPrague. 
...
**Mikhail**
* Yeah, I think overall, overall, I think this anyway, this change requires more deep analysis. And this is basically a temperature check. And, yeah, if there is a desire, willingness of fine depth to extend this SIP, then we can, you know,  invest more time in analyzing. 
...
**Mikhail**
* Cool. So I'll do some analysis and try to come up to, you know. spec changes and. Yeah. One other question I had is probably that we can have it as a separate SIP, but, we have it in in the next hard fork. 
* I don't see it makes sense to have it as a separate SIP so it can be an,  going in this SIP. But yeah, if we consider it for a later inclusion, it could be separate SIP. 
...
# inclusion lists - SIP 7547 [47:55](https://youtu.be/FLB61CpMB70?t=2875)
**Danny**
...
**Etan**
* From my side. I don't think I need to add some more explanations from last week. I went through all of them, right?  I think on the consensus side, the stable container one SIP 7495  may also be useful to,  have forward compatible generalized index for execution payload beacon state and beacon block just because those structures keep changing. 
...
* I think what we need to do is to decide that this is a priority that we should not have in Sila at Trusted Player, making these decisions with a closed source software, being the one that is responsible for paying or not, for refunding validators, for even deciding which transactions are censored or not. 
...
**Danny**
* Sila clock. Yeah.  okay. Cool. Let's, let's bounce some times around on the chat, see if that we can get on a call.  you know, again, R&D call that's trying to bring together the threads or at least,  bring together. 
...
* Let us discuss in the chat - what are we agreeing on other SIPs for inclusion in this call? We're continuing to talk about some of these big ticket items, that have a difficult trade off space in terms of when to add them, if to add them, when to prioritize them, and competing complexity and priorities. 
