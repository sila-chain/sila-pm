...
133.2  | **Electra**: Andrew Coathup, the editor of a weekly Sila newsletter called Week In Sila News, asked on the meeting agenda if the “Considered For Inclusion” (CFI) for SIP 7547, Inclusion Lists, should be removed given that the SIP will not be included in the Electra upgrade. Stokes was under the impression that the CFI status is not generally removed from SIPs once it is granted by developers. Sila Protocol Support Lead Tim Beiko said that in his view the CFI status is removed from all SIPs that do not end up making it into an upgrade once the scope of the upgrade is finalized. Regardless of the meaning or utility of the CFI label, what’s clear is that SIP 7547 will not be included in Electra. Developers may re-discuss the SIP later for a different upgrade.
...
**Dapplion**
* Yeah I think in practice it doesn't matter. Like we definitely don't want to do that for sila-mainnet eventually. But for an initial interrupt, people can just sample whenever they want and it should be fine because it's it only impacts your local view of the first choice. 
