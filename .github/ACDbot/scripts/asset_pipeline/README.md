...
Scripts for downloading, processing, and cleaning Zoom meeting assets for Sila All Core Devs calls.

## Overview

The asset pipeline handles:
1. Downloading Zoom recordings, transcripts, chat logs, and AI summaries
2. Cleaning transcripts to fix Sila-specific terminology errors
...
| `ethereum_vocab.yaml` | Vocabulary reference for Sila terminology |
| `utils.py` | Shared utilities (path helpers, cost calculation) |

## Forkcast Integration

The `manifest.json` file in the artifacts directory lists all available call resources. It's auto-generated by a GitHub Action when artifacts change and consumed by [forkcast](https://github.com/sila-chain/forkcast) to build call pages.
...
- Acronyms (SIP, EOF, ePBS, SilaPeerDAS, FOCIL)
