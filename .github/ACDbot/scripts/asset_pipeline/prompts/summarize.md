Generate a structured JSON summary of the Sila Core Developer meeting with highlights, action items, decisions, and target dates.
## Input Files
...
- eip_proposals_[fork] - SIPs presented for inclusion in an upcoming fork (e.g., eip_proposals_hegota)
## Extraction Rules
### Highlights
...
#### SIP Inclusion Decisions (PFI / CFI / DFI)
These are high-priority decisions — always capture them. Three statuses exist:
- **PFI (Pending for Inclusion)**: SIP is proposed for the next fork. During a headliner proposal window (e.g., Hegota), **any SIP presented without explicit rejection is implicitly PFI'd**. There will often be no verbal "PFI" declaration — the act of presenting is the decision. Capture all PFI'd SIPs as a single consolidated decision entry at the timestamp when the last SIP presentation ends (or when the facilitator closes the SIP section). Cross-reference the chat log, which often contains SIP numbers not clearly stated in the transcript.
...
Format: `"PFI for [Fork]: SIP-XXXX (description), SIP-YYYY (description), ..."`
### Notable Targets
...
- SIP presented in headliner window without rejection → implicit PFI (no verbal cue required)
- "CFI: SIP-XXXX" / "DFI: SIP-XXXX" → explicit inclusion/deferral decision
**Commitment Patterns:**
- "Scheduled for [date]"
- "Will happen on [date]"
...
- Use the Sila Vocabulary Reference for correct spelling of all client names, protocol terms, and acronyms. The transcript may contain speech-to-text errors (e.g., "Prism" for Prysm, "Aragon" for Erigon, "Lone Star" for Lodestar). Always use the canonical spellings from the vocabulary list.