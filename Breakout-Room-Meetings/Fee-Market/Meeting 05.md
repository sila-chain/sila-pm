# SIP-1559 Implemnters Call 5 Notes

### Meeting Date/Time: Thursday, Oct 8th, 14:00 UTC

### Meeting Duration: 57 minutes

### [GitHub Agenda](https://github.com/sila-chain/pm/issues/209)

### [Audio/Video of the meeting](https://youtu.be/SHVfypwL5W8)

### [All Meetings](https://github.com/sila-chain/pm/tree/master/Fee%20Market%20Meetings)
...
| **5.2.2**   | Micah to add the 1559 change to reporting miner or user gas fee to the security considerations section on the SIP                                       |
| **5.2.3**   | Tim to give someone the task of writing the specs for the new SIPs (transaction by hash, block by hash, and send transaction, block by number)          |
| **5.2.4**   | Abdel to write the SIPs                                                                                                                                 |
...
| ----------------- | ------------------------------------------------------------------------------------------------------------------ |
| **5.3.1**         | [blockchain pricing investigation](https://github.com/sila-chain/research/blob/master/papers/pricing/ethpricing.pdf) |

### Agenda

- Status updates from implementers and researchers
  - Decision on the merged transaction pool PR: sila/SIPs#2924, incl. the transition from legacy to 1559-style transaction and the transition period.
- Thoughts on SIP-2718?
- SilaMainnet readiness checklist
- Sila Cat Herders' Community Outreach
...
**Tim** - That's great, yeah, that's a lot. So I had a couple of questions, I guess the first one around like that, the transition period, do you think are you like that still makes sense if we've, like, removed it from the SIP with the with because recent PR,
...
# 2. Sila Cat Herder's Community Outreach
...
**Abdel** - Basically. Oh, yeah, maybe we can update the Front-End actually and implement the decoding logic, actually. Yeah, so that will be easier actually to display sil 1559 transactions in the explorer.
...
5.2.2 | Video | [~17:26](https://youtu.be/SHVfypwL5W8?t=1046)
    ...
    Micah to add the 1559 change to reporting miner or user gas fee to the security considerations section on the SIP

**Tim** - Is that worth adding to like the security considerations section of the SIP? I feel like I'm the backwards compatibility section.
...
**Micah** - Yeah, or forever means TBD. we don't have a currently built mechanism for getting rid of them, at least not in 1559. But some future SIP probably will maybe.
...
**Micah** - Ok, so I think I believe the other clients, at least open sila and geth and I'm pretty sure nethermind, if you were talking directly to RPC, will accept anything because it treats you as kind of a privileged user when it comes to what it accepts. And so we will accept it. And I think they all actually have a separate pending pool sort of where transactions are protected from being ejected from the vending pool on that node if the node received that node from RPC not on RPC.
...
**Micah** - So I think we still do need a minimum percentage, just like we have currently on the network, which is I think 12.5 percent for open sila, I think. But I think it matters less whether that's a fee bump or a base fee bump. Or both. That's right, right, bump or if he kept them or both of them. Like, if we're kicking out transactions that aren't likely to be mined soon and we have something that the user has to keep increasing. I guess it does have a miner bribe, doesn't it, because if it's base fee, then they can spam.
...
**Micah** - And would that also sil block with the true flag for full blocks. The one that returns all transactions, I think it's sil get block by hash? [yes], I believe it's only two that return transactions, right?.
...
5.2.3 | Video | [45:45](https://youtu.be/SHVfypwL5W8?t=2745)
    ...
    Tim to give someone the task of writing the specs for the new SIPs (transaction by hash, block by hash, and send transaction, block by number)

**Micah** - It would probably be good Tim to have someone write the specs for those three and it'll be three new SIPs.

**Tim** - Oh, so the change. Yeah, the changes to the jason RPC have to be separate SIPs.

**Micah** - Yeah, well they should be. you don't have to I guess of course. Ideally yes there would be there'd be an SIP for each of them that justifies the changes that are being made to the adjacent RPC and then from their clients can implement it and while its providers can implement etc.

**Abdel** - I thought they were out of scope, oh they're not core SIPs

**Micah** - They'd be interface SIPs

**Tim** - Ok, so and so we basically need one SIP per Jason RPC call.
...
**Abdel** - Yeah, because you want to add to the base SIP well. Yeah, get block by hash and get block by number as well, because you you need to add the base fee header for 1559 blocks.

**Tim** - how about sil and roth transactions? Does that have to change as well?

**Micah** - No, I don't have to because you're not just sending a byte array. It's already signed. OK, so ignore it.

**Tim** - So there's four of them and we need an SIP for each of those.
...
**Abdel** - I can some, that would be an occasion to start my first SIP.

**Tim** - I mean, sure. If you want to do it. So we'll have you write those SIPs.
...
**Tim** - Yeah, I think nethermind is syncing it may be like with some recent changes to the SIP, there's some small tweaks to do, but I suspect like in the next week. Yes. And that nethermind at least should be should be on the same network and up and running. I don't know if nethermind supports mining.
