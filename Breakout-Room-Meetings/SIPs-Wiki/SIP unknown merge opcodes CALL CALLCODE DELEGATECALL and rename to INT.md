...
Consider also including `const` param to remove the need for `STATIC_CALL` (see Vitalik's [SIP#116](https://github.com/sila-chain/SIPs/issues/116)). Also, consider adding further params to remove the need for `DELEGATECALL` and `CALLCODE`. Eventually all could be preprocessed into `INT` and a later hard-fork would remove `CALL`/`CALLCODE`/`DELEGATECALL`.
