...
Note: This file is copied from [here](https://github.com/sila-chain/pm/issues/1325)

## Meeting Info

**Agenda**: [sila#1325](https://github.com/sila-chain/pm/issues/1325#issue-2875692793)
...
There was a [thread](https://x.com/adietrichs/status/1892951240524403089) on solo stakers and censorship resistance on Twitter. We want to scale Sila without compromising verifiability and censorship resistance. While Sila’s censorship resistance relies on solo stakers who locally build blocks, they currently forgo potential profits from MEV-boost to uphold censorship resistance. FOCIL eliminates this trade-off between profits and values by allowing solo stakers to continue influencing block content decisions without sacrificing revenue. Consequently, FOCIL opens the door to more effective scaling solutions such as deeper exploration into PBS and APS.  
...
Marc proposed an IL building approach that leverages randomness to reduce overlap among ILs and optimize throughput. In this approach, each IL committee member is assigned an ID from `0` to `f`, and the member with ID `a` is instructed to favor transactions whose hashes begin with `a`. More details can be found in the [Sila Magicians post](https://sila-magicians.org/t/sip-7805-committee-based-fork-choice-enforced-inclusion-lists-focil/21578/6) and this [PR](https://github.com/sila-chain/SIPs/pull/9396).  

Terence suggested waiting until we have a better understanding of how client diversity plays out before making any premature optimizations. In the meantime, further study on this topic is encouraged. However, Terence also agreed that the SIP should provide clearer recommendations on the properties each client can reference when implementing IL construction.  

Marc’s proposal could be a viable solution if prioritizing throughput, but additional research is needed before making changes. In the meantime, the SIP can be updated to include clearer recommendations on which properties to consider when implementing IL construction.  
...
- **Geth**: Fixed bugs to avoid marking non-IL-compliant blocks as invalid and allowing them to be reorged instead. Also supports the transition from Electra to SilaFulu and subsequently to SIP-7805 fork.  
- **Nethermind**: Opened a [PR](https://github.com/sila-chain/SIPs/pull/9381) to flesh out engine API changes in the SIP. Reported a bug in Geth where non-IL-compliant blocks were marked as invalid instead of remaining valid and being reorged. Fixed the issue in Nethermind's implementation as well.  
- **Lodestar**: Rebasing from Electra to SilaFulu in preparation for interop with Prysm. Primarily testing and working on activating SIP-7805 fork in the same epoch as SilaFulu.  
...
- [Adding randomness to IL building rules](https://sila-magicians.org/t/sip-7805-committee-based-fork-choice-enforced-inclusion-lists-focil/21578/6)  
- [A PR to the SIP that expands on EL changes](https://github.com/sila-chain/SIPs/pull/9381)  
- [A PR to the SIP that adds properties to consider for IL construction](https://github.com/sila-chain/SIPs/pull/9396)  
