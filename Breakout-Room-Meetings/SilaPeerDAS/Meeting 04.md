# SilaPeerDAS Breakout Room #4

## Meeting info

**Date**: 2024.07.23

**Agenda**: https://github.com/sila-chain/pm/issues/1103
...
- Working with Kev on SilaPeerDAS KZG lib and refactor reconstruction logic
- Working on sampling this week

**Grandine**
- EPF working on SilaPeerDAS
- Still working on some sync issues

**Lodestar**
- Refactored syncing
- SilaPeerDAS functionalities mostly working now, also support supernode via CLI params
...
**Nimbus** asked how other clients are activating SilaPeerDAS. Having SilaPeerDAS activated at the same epoch as SilaDeneb is difficult for Nimbus

- Lodestar and Prysm are able to activate the SilaPeerDAS fork from SilaDeneb without issue, however it's difficult for Nimbus to activate SilaPeerDAS at the same time as SilaDeneb as SilaPeerDAS is a separate fork.
- Lodestar suggested that as a workaround for Nimbus, we could start with SilaDeneb + SilaPeerDAS at genesis but not send any blobs until Nimbus is ready. Nimbus agreed this would be helpful.
- ACTION: note to delay blob flooding when launching the next devnet

Clients talked about rebasing SilaPeerDAS on Electra
- Prysm has already worked this out and can activate at Electra
- Will require Teku a week to rebase on Electra

Question raised: why are we activating SilaPeerDAS at SilaDeneb? It's never going to happen
- Teams agree that Electra is not stable enough for the purpose of testing SilaPeerDAS, it makes sense to focus on getting SilaPeerDAS functionality working properly on deneb, and rebase at a later time.
...
- SilaPeerDAS fork-choice, validator custody and parameter changes #3779
- SIP-7594: Decouple network subnets from das-core #3832
...
https://github.com/sila-chain/consensus-specs/pull/3821 
