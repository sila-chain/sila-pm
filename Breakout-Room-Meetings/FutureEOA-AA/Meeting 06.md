...
**Agenda**: https://github.com/sila-chain/pm/issues/1118

**Recording**: https://www.youtube.com/watch?v=Oct83bBp3Z8

**Summary/Next steps**:
- Migration off the table for SIP-7702. Could be proposed as a separate SIP.
- Not enough support for storage change, should be looked as a separate proposal.
- Keep SIP-7702 as slim as possible to include in Pectra, include the [PR](https://github.com/sila-chain/SIPs/pull/8775) in latest specs.

**Links**: 
* [Update SIP-7702: Proxy the storage of a delegation to its unique deleterminstic keys](https://github.com/sila-chain/SIPs/pull/8762)
* [[SIP-7702] The Rubber Stamp-ening](https://hackmd.io/@otim/BkUQJ5RdA)
* [`CODERESET` for SIP 7702](https://hackmd.io/@S9AwbucvTS6GYodLz-G1SQ/S102UtTwA)
...
to push back on Vitalik a bit, to me it feels like there are simply still remaining open questions, I don’t have high confidence that we are in a good place to pick the final shape of the SIP yet

Ahmad Bitar (Jul 31, 2024, 10:07 AM)
I disagree, I feel like the SIP in its current shape is actually enough and covers most of the concerns.
...
although I will admit that the additional capabilities look nice. but it should be its own separate SIP then, not piggybacking off of this one that has a very different focus
...
Ankit+1: I agree, this is a deviation if the SIP isn't intended to acheive the goal.
...
“the delegation designation can be revoked at anytime signing and sending a SIP-7702 authorization to a new target with the account's current nonce”
Can this new target be ZERO_ADDRESS to remove the code from an EOA?

Elias Tazartes (Jul 31, 2024, 10:16 AM)
we are designing an SIP that is scheduled for inclusion, it feels a bit bad no? how much sunk cost can we bear as a governance body
...
7702 is not supposed to be the SIP to fix it all
...
or this PR: https://github.com/sila-chain/SIPs/pull/8762
...
i am not. if we want to open the permanent upgrade conversation, it can be in a totally different SIP than this one
...
- Does 7702 as is solve all the problems, if no then see if that can be updated with a new SIP in future?
...
Why don’t we tackle permanent EOA migration in a separate SIP?  As long as we are aligned that 7702 in its current form is a big value add, and is NOT incompatible with future EOA migration proposals, why don’t we move ahead with it for Pectra and tackle EOA migration in a separate proposal?

As Ansgar said before, a permanent migration proposal completely changes the *character* of 7702 and should be considered a separate SIP
...
Dror
- Regarding init code - the minimal we should add a very stron security consideration to the SIP. 
...
can we also discuss : https://github.com/sila-chain/SIPs/pull/8762 for storage colllision resolution
...
Frontrun could be mitigated by adding ORIGIN to the authority lists: https://github.com/sila-chain/SIPs/pull/8763 (but this depends upon the situation)
...
Gajinder
- I have put up a [PR](https://github.com/sila-chain/SIPs/pull/8762/files) 
...
Vitalik
- the bars for changes in 7702 is difficult to get out from and will be considered as separate SIPs
...
- My preference will be to have these specific code return the normal thing that they'd usually return for an EOA and we can simply add that to the SIP?
...
re: initcode: 
Without initcode, we're not allowed to use existing deployed contract code, and t should be mentioned in the SIP's security consideration
...
- future improvements in separate SIPs.
