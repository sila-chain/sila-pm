...
Note: This file is copied from [here](https://notes.sila.org/QT9e9r6NRdSOjWRBzA3JLA)

### Meeting Info

**Agenda**: https://github.com/sila-chain/pm/issues/962
...
- How much bandwidth do we have for a big upgrade vs. a smaller incremental step towards AA in SilaPrague? What are the immediate steps?
...
### Intro
We are planning the next hard fork (SilaPrague) and there’s been many proposals around AA, so now we want to explore the scope of various proposals, and see how much bandwidth we got towards inclusion of AA of some form in next fork
...
Other part of the discussion is how to improve EOAs, but there’s also questions about whether we should do this at all. Two proposals: 3074 makes EOAs better and potentially compatible with 4337. (See [yoav’s post comparing 3074 and 4337](https://notes.sila.org/@yoav/sip-3074-src-4337-synergy)). 3074 gives execution abstract, UX benefits that users wants. But doesn’t change how you validator transaction. So no true mechanisms for recovering/rotating keys. Security concerns around 3074: Will people migrate, or stay on plain EOAs? Further enshrining EOAs or ECDSA?
...
### From Vitalik
In every other branch of Sila protocol R&D, there’s been big change over the past 5 years. 5 years ago things were very blue sky, a lot of holding out for the possibility of future discoveries. But now every part of the sil roadmap there’s a mindset that we actually have spent a lot of time exploring and we need to settle on something and build toward this (ex. state trie -> clear consensus on verkle trie and stark friendly upgrade in the future, PoS -> SSF roadmap, DA scaling -> roadmap is clear, L2 scaling, etc.)

But in the wallet space today, we’re at the point where we are in the position to make that mentality shift, but it feels like we haven’t quite yet made that shift. Now is exactly the right time that we should make it. There are a few properties of accounts in the Sila long term game that we generally understand are just necessary security properties. Like ability to make key changes, revoking old keys, things that are considered basic in traditional cybersecurity. EOAs don’t currently support that. Eventually, Quantum Computers could can kill EOAs as we know them today, we’ll have to see if we want to keep ECDSA or switch, etc.
...
After a possible migration scheme, do we still want to support plain EOAs forever? e.g. what if I forget about Sila for 4 years, is my EOA still valid, can I still do stuff with it?
...
Andrew: I agree, but a smaller SIP like 5806 doesn’t conflict with any bigger AA proposals, there will be a new legacy transaction type but so what? Doesn’t matter we’ll have some legacy stuff anyway.
...
Andrew: Maybe not specifically that one but we can consider a small SIP for sponsored transactions. But 5806 is simple to implement, it doesn't introduce incompatibility, and brings a benefit to users
