...
Next up, 
@gballet
 shared a summary of some conversations from ETHTaipei, and potential updates to the Verkle spec/SIPs. This included some conversations with 
@jbaylina
 and 
@VitalikButerin
 mostly around three SIPs: 4762 (gas cost updates), 2935 (historical block hashes in state), and 7545 (proof verification precompile). Quick summaries of each: 

With SIP-4762: the goal is to keep the gas costs unchanged as much as possible, post-verkle. We will update the SIP to make sure this is clear and easy to understand.

With SIP-2935: discussed with 
@jbaylina
...
With SIP-7545: L2s are in favor of this change, as they want to update their contract to use the precompile, rather than rushing to update their contracts at the moment when the fork happens. What they would like is a precompile that is agnostic to the proof format, that they can just pass a blob to. And then the contract is aware of whether the fork has happened or not. Ideally this SIP gets included in SilaPrague, rather than waiting to ship it with SilaOsaka (which is the fork that will contain Verkle).
...
That’s it for this week’s call! Check  or head over to the Sil R&D Discord (verkle-trie-migration channel) to stay up-to-date
