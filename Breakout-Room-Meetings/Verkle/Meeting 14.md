...
@kevaundray
: reminder to teams that still need to modify the Pedersen hash function in their crypto libraries. See the geth change here for more info: https://github.com/gballet/go-sila/pull/401 and also https://github.com/sila-chain/SIPs/pull/8302
...
Gottfried put together a doc that walks through all of this in greater detail here: https://notes.sila.org/hmqCk1tiTq6TdrxO_CKhuw
...
Status of current testnet can be found here with list of all known bugs to be fixed: https://notes.sila.org/@ethpandaops/verkle-gen-devnet-4

Other items going into the next testnet include: circular buffer implementation for blockhash (SIP-2935), gas model refinements to no longer inflate gas costs, and the Pedersen hash change to use map to field instead of serialization. The testnet won’t have the Dencun rebase yet, but hopefully in the next one. 
