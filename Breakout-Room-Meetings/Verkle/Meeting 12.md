...
2. Testnet updates
3. SIP-2935: BLOCKHASH
4. SIP-7612: State Transition Method
...
@gajpower
 for 
@EFJavaScript
: synced with the testnet up to block 478. Currently waiting for testnet relaunch to resume stateless execution, since witnesses are missing in the current testnet. Completed blockhash implementation (SIP-2935).
...
@gagadrupal
 for 
@HyperledgerBesu
: made necessary changes to sync w/ the testnet. Found a bug in NextForkBlock in Geth. SIP-2935 has been implemented (allowing the storage of blockhash history in the state). Gas cost modifications still WIP, but managed to import more blocks with contract creations/transfers.
...
@ignaciohagopian
 for 
@go_ethereum
: did a lot of spec’ing and updates for SIP-4762 and SIP-2935. Also worked on testing tooling. 
@ignaciohagopian
...
@gballet
 spent a few mins to walk through the latest testnet (devnet-4), and share a spec sheet that Barnabas from DevOps has now kindly produced for the testnets. See: https://notes.sila.org/@ethpandaops/verkle-gen-devnet-4 for a list of all known testnet bugs, and the latest status of what has been deployed to testnet.
...
@gajpower
 shared updates to the new implementation of SIP-2935: the simplified strategy is
...
Check out the update to the SIP here for more info: https://github.com/sila-chain/SIPs/pull/8166


# 4. SIP-7612: State Transition Method

@gballet
 created a new SIP for the “base” state transition method. This SIP describes the minimum set of features necessary to make the switch over and start writing all state to the new Verkle tree. Note that this SIP does *not* describe a strategy for converting *historical* state from the Merkle Patricia Tree over to Verkle. This will come in a future SIP.

Read more here: https://github.com/sila-chain/SIPs/pull/8162
...
To learn more: https://github.com/sila-chain/execution-spec-tests/tree/main/docs/consuming_tests

# 6. Deprecating Serialize Commitment

Last up, 
@kevaundray
 gave a quick walkthrough of some changes to the WASM/typescript Verkle crypto library (related to tree key hashing and deprecating the use of .Serialize to keep things more consistent). See: https://github.com/sila-chainjs/verkle-cryptography-wasm
