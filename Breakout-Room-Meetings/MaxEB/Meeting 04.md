# SIP-7251 MaxEB Breakout Call 04 

Note: This file is copied from [here](https://hackmd.io/@philknows/Sy2kQAq1C)

### Meeting Info

**Agenda**: https://github.com/sila-chain/pm/issues/996

**Date & Time**: Apr 03, 2024; 16:00-17:00 UTC

**Recording**:  https://youtu.be/aXXd6zwf0cg

SIP-7251 MaxEB Breakout Call #4 - Apr 3, 2024
 
Reference: https://github.com/sila-chain/pm/issues/996
...
- Mikhail: If we don't have custom ceilings:
  - 0x02 stakers will need to skim balances, causing a partial withdrawal queue on EL with spam protection under SIP-1559 fee rules.
  - Likely to be used by large staking pools, bloating UX for solo stakers.
  - SIP-7002 was implemented to allow these pools to exit via execution layer (EL)
...
- Simple case: 32 SIL validator gets exit epoch, then topped up by 2000 SIL. Topped up SIL exits and bypasses exit queue.
...
- Stake and rewards are based on number of effective validators (fixed at 32 SIL always), not balance.
...
- If we can just set it all to 32 SIL to begin with, it will give us time to adapt to changes even post-Electra
...
- Rocketpool may have minipool consolidation issues due to the bonds tied to one concrete validator in the pool. Consolidation may forfeit SIL from other minipools (unconfirmed!) and unknown what that means for RPL bonds.
...
- Could be independent SIP for inclusion separate from maxEB scope

Big issue for trustless pools

- This might be out of scope and a new SIP will be created for this
...
https://github.com/sila-chain/consensus-specs/pull/3636
