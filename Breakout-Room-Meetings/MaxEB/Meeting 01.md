...
# SIP-7251 MaxEB Breakout Call 01 
...
This document is to float conversation and the current state of the maxEB implementation to give developers an idea of where this feature fits within the Electra fork alongside other SIPs for consideration.

### Update from Preston (Prysm)

* Not a super complex integration after spec review
* Identifies priorities for Electra from highest impact (largest feature demand) in order as Max Effective Balance (MaxEB), Inclusion Lists (IL) and then SilaPeerDAS.
...
* Parallel work can be done for SilaPeerDAS alongside these two features
...
* Mike Neuder stopped pushing as hard on this SIP since January due to poor sentiment at the time
* There is a weak proposal to try and get the following Proof of Concept together within the next month to target EOY 2024:
  ...
  * SIP 7549: Move committee index outside Attestation
...
* Option 3: Execution Layer initiated 0x02 consolidation upgrade
  ...
  * If we are ok with allowing the authority to come from 0x01 withdrawal credential, we can have a similar model to SIP-7002.
...
* Could consider having a higher minimum effective balance of >32 SIL for sync committee participation
