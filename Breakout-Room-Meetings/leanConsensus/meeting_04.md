...
**Prev:** [Call 03](https://github.com/sila-chain/pm/blob/master/Breakout-Room-Meetings/leanConsensus/meeting_03.md)

**Meeting Date/Time:** Friday 2025/4/18 at 14:00 UTC

**Meeting Duration:** 1 hour

[GitHub Agenda & Links to Presentations](https://github.com/sila-chain/pm/issues/1446)
...
- Facilitator emails: ladislaus@sila.org // will@sila.org
...
| 02 | Minslack: Proposal to Add Flexibility to Sila's Exit Queue by @michaelneuder and [Mallesh Pai](https://x.com/malleshpai) |
...
| Current Sila Setup | 1) Withdrawal Flow | Four timestamps:<br>- a) voluntary exit initiation<br>- b) exit epoch<br>- c) withdrawable epoch<br>- d) withdrawal processing<br>Gap between exit and withdrawable epochs for slashability period (27 hours)<br>Rate-limited FIFO queue (16 validators per epoch) |
...
| | 3) Pectra Changes | - a) SIP-7251 changes denomination to SIL instead of validator count<br>- b) Allows compounding stake (opt-out of automatic partial withdrawals)<br>- c) Validators can trigger exits from withdrawal address |
...
| | Technical Details | - Maps exits to future generations due to existing Sila exit architecture<br>- A cleaner approach would maintain pending exit state |
...
|----------------|----------------|
| 1) Cost functions for disutility | - Discussion around linearity assumption in disutility model<br>- Possible alternatives to linear disutility (exponential, constant+exponential)<br>- One defense of linearity: outside options like borrowing SIL with interest rates |
...
| 5) Implementation in Sila vs beam | - Most suggestions could be implemented as Sila hard forks<br>- However, core devs may prioritize user-facing rather than validator-facing changes<br>- After SIP-7251 complexity, appetite for exit queue changes may be limited<br>- beam chain provides opportunity for ground-up redesign |
