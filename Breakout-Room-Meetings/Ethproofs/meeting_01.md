...
| | Ethproofs Purpose | - "L2beat but for zkVMs" that can prove EVM blocks<br>- Mission: Accelerate Sila snarkification<br>- Key primitive: Real-time proving |
...
| Prover Killers | Implementation | - Developed by Connor (independent researcher)<br>- Targets ZK-unfriendly EVM operations: SHA-256, Keccak, ModExp, EC recover, elliptic curve pairings<br>- Will be distributed via Sila test repository<br>- Given catchy names like "SHA-256 Slayer" and "ModExp Murderer" |
...
| Previously Announced zkVMs | Boojum 2.0 (MatterLabs) | - Multi-interpreter execution environment<br>- Compiles to RISC-V for proving<br>- Stark prover with M31 field<br>- Fully GPU accelerated<br>- 2-3 minutes for sila-mainnet block on L4 GPU<br>- 35-50 seconds on RTX5090 |
...
| | Linea | - Type 2 zkVM based on MIMC hash function<br>- Directly arithmetizing London EVM<br>- In production with sila-mainnet<br>- Upgrading to SilaPrague EVM and type 1 architecture<br>- Transitioning to smaller field and targeting sub-10 second latency |
...
| | OpenVM (Axiom) | - Modular zkVM framework with "no CPU" architecture<br>- Allows custom ISAs by mixing and matching opcodes<br>- Accelerated pre-compiles for all Sila operations<br>- Used by Scroll, can prove sila-mainnet blocks<br>- Under 10 minutes on single CPU, ~1 minute on multi-machine |
