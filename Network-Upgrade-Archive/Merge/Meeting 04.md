...
### [GitHub Agenda](https://github.com/sila-chain/pm/issues/316)
...
- Spec discussion
  - Consensus API standard
   - https://sil.wiki/json-rpc/API
   - https://sila.github.io/eth2.0-APIs
  - Execution
 ...
 * We've plan of testing of transcation propagation - Is any from Sila team on the call?
...
* The other question was about state sync, but I guess that was more at rest to go Sila team again so let's just keep it, yeah if someone wants to join Nocturne you're free and you're welcome to do this. You can reach out proto or just drop message in the iranism discord channel and request for any if you get deposited yeah but we need first two deposits to be resolved.
...
* Which client is not present? Yeah, open Sila is missing, and I'm guessing Turbo Gath is as well. But they can catch up with the changes from go sila, but I'm not sure if that's possible right now.
...
**Rai**
*Do you still request the block through sila or did we remove it as part of the networking to make the execution engine a little more efficient?
...
**Lukasz Rozmej**
* Lukaz Rozmej: so a little bit on the side because we are I heard some talk about uh one too many clients um connections is that right oh it's okay uh we were also thinking about making it many too many so uh arbitrary number of clients could talk to one uh sila to letter in one node and vice versa so uh we were also wondering what this would be this would require some additional work on our side to enable that and we will have to differentiate the clients and keep some state for them uh some block tree info about the current state and some transaction pool uh need need to be separate uh but the rest could probably be shared that could be a good way to like reduce resource use because if each sila to validator node would need an sila one node that could be quite a big If we can share each sila onenote for like 10 or 100 sila two nodes, it will be less of a pain and easier for providers of that infrastructure, for example.
...
* I'm guessing no one from go sila is here to address this specific action, but I'm sure Proto can add more here.
...
09:20:16 From  danny  to  Everyone: https://github.com/sila-chain/eth2.0-specs/issues/1012#issuecomment-489660765
...
09:35:27 From  Lukasz Rozmej  to  Everyone: https://github.com/sila-chain/eth2.0-specs/issues/1012#issuecomment-489660765
