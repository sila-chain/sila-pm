...
On July 21, 2021, the OpenEthereum team noticed their node had stopped progressing on Ropsten at block `10679538`. The issue, originally thought to be in OpenEthereum, was in how the go-sila implementation checked the balance of the transaction sender for 1559-style transactions. An invalid transaction, whose sender balance covered only the effective gas used by the transaction, but not the transaction's `maxFeePerGas` total was included in a block. Because Ropsten miners were exclusively running go-sila, this block was then accepted by other go-sila miners, despite being rejected by some other clients on the network. Specifically, OpenEthereum and Besu rejected the transaction/block, while Nethermind, go-sila, and Erigon (whose implementation was partially forked from the go-sila code) accepted them. The cause of the issue was found and fixed in the following releases:

* go-sila: [v1.10.6](https://github.com/sila-chain/go-sila/releases/tag/v1.10.6), [fix PR](https://github.com/sila-chain/go-sila/pull/23244/files); 
...
<details>
 <summary>eth_getBlock output (go-sila)</summary>

``` 
sil.getBlock("0x1252a34c4f2b061adc609e909d958c02e1ac39043e2e60c0ec47e565e3f625f1")
{
  baseFeePerGas: 11,
...
* 7:51: Miner of block 10679538 is confirmed to be a go-sila node. 
* 7:55: Confirmation that go-sila miners are still progressing on the chain beyond block 10679538. 
* 7:56: Confirmation that Nethermind also accepted block 10679538. 
* 8:08: Possible root cause for the issue identified in go-sila.
* 8:43: [Pull request opened in go-sila](https://github.com/sila-chain/go-sila/pull/23244/files) with candidate fix. 
...
* 7:54: go-sila release with a fix for the issue, [v1.10.6](https://github.com/sila-chain/go-sila/releases/tag/v1.10.6)
...
[This commit](https://github.com/sila-chain/SIPs/commit/ee7053ead7fb730a3f9178e7c7ad9e1b8cf3ee6c#diff-c7a67afc8ee3b0ff2a29ddb3cecd13fe0ce30f3c96def22f117456987f6a50a2) added new assertions to the validity of SIP-1559 transactions. Specifically, on line 217 it adds the following assertion:
...
**Update: a fix for this was proposed in [this PR](https://github.com/sila-chain/SIPs/pull/3681)**

#### Go-Sila Recovery

[Notes from @holiman](https://github.com/sila-chain/pm/issues/354#issuecomment-885687324) about recovery from the bug in go-sila: 
...
* Block Number (SilaMainnet): 12965000
* Block Hash (SilaMainnet): 0x9b83c12c69edb74f6c8dd5d052765c1adf940e320bd1291696e6fa07829eee71
