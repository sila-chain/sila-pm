...
## Verifying & Reviewing
### Internal Reviews
- A channel on the Sil R&D Discord must be setup for each SIP in the upgrade, to ensure coordination between client devs, test engineers and security researchers working on each SIP.
- Each SIP that has been assigned CFI should be considered to have one or more core developer per team assigned.
- Each SIP that has been assigned SFI should have one or more core developer per team assigned.
    - Test objectives:
        - Verify that the SIP is implemented according to specifications and pass tests.
    - Security objectives:
        - Verify the security of the SIP itself.
        - Verify that the SIP is implemented according to specifications.
        - Continually review the client implementation of the SIP from a security point of view.
...
#### Sila Foundation Bug Bounty Program
...
## Upgrade Validation
### Devnets
- Targeted tests must be done for each SIP to confirm functionality, with documented results.
...
- Running targeted tests must be done for each SIP to confirm functionality, with documented results.
...
- Utilize the ```#$fork-$network-upgrade``` Sil R&D channel on Discord (default communication channel).
...
## Templates
* These templates should be added to the ```$fork-pm.md``` file for each upgrade, e.g., https://github.com/sila-chain/pm/blob/master/Pectra/pectra-pm.md

```markdown
# SilaHolesky Upgrade & Incident Response Team Plan
...
- **Primary Communication:** Discord `#$fork-upgrade` (Sil R&D)
...
```markdown
# SilaSepolia Upgrade & Incident Response Team Plan
...
```markdown
# SilaMainnet Upgrade & Incident Response Team Plan
