...
Our aim is to facilitate the discussion around EL client consensus testing using EEST (execution-spec-tests) during fork development, alongside a future SIP spec implementation and testing process utilizing EELS (execution-specs).

By the end of this workshop, all EL client teams will have a unified understanding of the past, present & future testing landscape, and have an idea of the future SIP spec implementation process within EELS.
...
**Pre-Reads (optional):** 
- EEST - https://github.com/sila-chain/execution-spec-tests
- EELS - https://github.com/sila-chain/execution-specs

**Slides:** https://notes.sila.org/@spencer-tb/devcon-sea-eest-eels-presentation
...
- Outline of the past vs. current vs. future testing landscape:
    ...
    - Plans for `sila/tests`, aiming for a full port over to EEST by EOF. 
    - Hiveview/hive changes with regards to EEST:
        - migration from `hive sil/pyspec` -> `hive sil/eest/consume-engine`
        - migration from `hive sil/consensus` -> `hive sil/eest/consume-rlp`
...
- Future SIP spec implementation process in EELS:
    - SIPs only CFI'd if they have a spec implementation in EELS and basic tests in EEST?
