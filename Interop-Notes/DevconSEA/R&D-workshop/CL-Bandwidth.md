...
With SilaPeerDAS on the horizon and our goal to increase the global blob count, bandwidth requirements for Sila consensus nodes is becomming an important concern. This session will discuss the current state of consensus node banwidth usage, estimated usage with SilaPeerDAS and current and future research/plans to handle bandwidth in consensus nodes.
...
- Duplicates of messages seem to be received closer to end of slot?
    - Csaba: it makes sense to differentiate between topics, e.g. SilaPeerDAS
...
- current process a bit adhoc, is there a good way to do it? libp2p-spec vs beacon-metrics?
    - some libp2p metrics are not sila-specific
