...
**Summary:** Account Abstraction has been a long time Sila goal. We've built an off-protocol implementation (SRC-4337) which gained traction. This session is about the next step - an in-protocol implementation (SIP-7701) based on EOF.
...
* Account abstraction background
    ...
    * What is SRC-4337 and why is it not enough?
* SIP-7701
    * Goals and non-goals.
    * Current design
    * Open questions
...
* SIP-7701 (EOF code sections) vs. using an ABI (similar to RIP-7560)
    * Pros:
...
* "Interim 7701" to support secp256r1 accounts (and other sig schemes)
    ...
    * Upgrade mechanism similar to SIP-7702 but using the account's validation
...
## Why Isn’t SRC-4337 Sufficient?

- **SRC-4337** is widely adopted for AA outside the Sila protocol, with over 20 million accounts deployed and 90 million operations performed, demonstrating significant market validation.
- **SIP-7701** aims to bring AA fully into the protocol in a way that is minimally opinionated.

**Non-Goals for SIP-7701**:
...
2. **Entry Points in EOF (Sila Object Format)**: Adds an `entry_points` section to EOF contracts that maps `AA_TX_TYPE` call frames to specific code sections.
...
3. **EOF Forwarding Proxy**  
   ...
   - **Y’s Response**: To enable this, we’d need to enshrine the proxy type in a similar manner to SIP-7702. The proxy would pass validation and could act as a bridge to legacy accounts.
...
5. **Validation Section for Legacy Accounts**  
   - **A’s Insight**: A simpler approach would be to use the existing SRC-4337 ABI for validation without enshrining it in the protocol itself. By only enshrining the singleton in the mempool, we could maintain flexibility in protocol usage.

---

## Important Implementation Details for SIP-7701
...
- **Compatibility**: Implementing either SIP-7701 or RIP-7560 can simplify adoption of the other, making L2 adoption smoother for AA.

---

## Demand and Potential Adjustments

- **Demand for 256R1 Curve Support**: Rising demand for 256R1 prompts the AA team to consider a “lighter” version of SIP-7701 to meet these immediate needs.
...
- **Current Client Support**: As SRC-4337 is an SRC, it has no direct client implementation, but multiple bundler implementations are available across different programming languages.

---

## Additional Topics and Feedback

1. **Profit Levels for Bundlers**: Issues of over/underpaying bundlers are mitigated in the dedicated mempool, resolving a key challenge faced with SRC-4337 in the main mempool.
