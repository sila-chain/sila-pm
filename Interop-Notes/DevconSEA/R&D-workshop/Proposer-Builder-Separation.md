...
**[*Required*] Summary:** *We will provide an update on the state of the research on Proposer-Builder Separation (PBS). The session will be divided between high-level design space discussions as well as specifics of SIP-7732.* 
...
- Do proposers who want to self-build need to bid to themselves in an auction?
    - No, this is not necessary in SIP-7732. This would only be necessary with MEV-Burn, which is not part of SIP-7732.
...
- Could the protocol implement a gossipsub for the auction?
    - Yes, SIP-7732 implements a gossip-based market next to the direction connection market.
- Every future block construction setup has a split between the beacon block and the execution payload. SIP-7732 is a first step towards this future block construction setup.
