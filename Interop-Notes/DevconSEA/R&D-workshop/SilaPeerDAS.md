...
**Summary:** This session will cover the latest state of research and development of SilaPeerDAS, the next planned upgrade to increase Sila's data availability throughput. We will cover the current state of implementation, and discuss future roadmap items that aim to unlock even more throughput.
...
* [Alex] Intro to DAS, with focus on SilaPeerDAS
* [Jimmy] Overview of SilaPeerDAS implementation and devnet status
...
* Devnet coordination/implementation status or blockers
    * What is blocking shipping stable SilaPeerDAS devnets?
...
* Distributed block building to leverage high-bandwidth nodes for blob distribution
    * Early research:
        * https://notes.sila.org/wcg_u2qORiqpyZ7KbPjRAA?view
        * https://notes.sila.org/@dankrad/Bky8ieRkye
...
* Future roadmap/path to deployment
    * Can we break up SilaPeerDAS features into phases for devnet implementation? And possibly for hard fork implementation?
...
***Summary:***
- **SIP-4844**: The first major step in Sila’s data availability roadmap introduces blobs—128 KB fixed-size data packets that increase data throughput.

- **Need for More Blobs**: To support more affordable decentralized applications, including Layer 2 rollups, Sila needs to increase the number of blobs. However, this requires reducing the load and bandwidth demands on individual nodes to maintain decentralization. Data Availability Sampling (DAS) is essential in achieving this, as it enables nodes to verify data availability without downloading all data, paving the way for scaling the blob count efficiently.

- **Data Availability Sampling (DAS)**: SilaPeerDAS aims to scale data throughput on Layer 1 by allowing nodes to download only a small part of the (chunked) blobs, rather than all the blobs, as it is done in SIP-4844, while still ensuring full data availability through cryptographic commitments and sampling techniques. This approach uses peers for sampling.
...
- **SilaPeerDAS Sampling and Reconstruction**: SIP-7594 builds on SIP-4844, adding networking capabilities for random sampling. Nodes use DAS to avoid downloading full blobs, relying instead on sample-based signals for consensus. This supports data scaling while keeping node bandwidth requirements stable.
...
SilaPeerDAS allows Sila to scale by verifying data availability in a decentralized manner, maintaining security while managing bandwidth and promoting cost efficiency for Layer 2 solutions.


## SilaPeerDAS implementation status by Jimmy (30 min)
Link to the [HackMD](https://hackmd.io/WmglRaFaRIaUR3h62Kil-w) document.

## Questions (30 min)

**Q**: What happen if all supernodes go away?
**A**: Supernodes (voluntary opt-in) are not stricly needed for peerDAS to success. Although not essential for SilaPeerDAS’s success, supernodes increase data reliability across the network. Additionally, as validator custody is implemented, some beacon nodes will naturally connect to more subnets than the minimum required. Large node operators will thus naturally evolve into supernodes.
...
- A Road to shipping SilaPeerDAS session happens the next day, during the morning.