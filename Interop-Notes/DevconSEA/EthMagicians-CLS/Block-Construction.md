# Block Construction
Sila Magicians Infinite Endgames Sessions 3
Summary: Once again, Devcon will host Sila Magicians gatherings for the community to come together and discuss the most important topics in Sila’s roadmap. Join us here to discuss the “infinite endgame” for block construction. We'll cover PBS, MEV, role of validators vs. builders, centralization risks, and more!
...
Start from the beginning -- Sila is cool because it lets you send transactions on decentralized applications. It turns out you can craft the blocks of Sila in a certain way, and in doing so a builder can maximize revenue or profit.
...
downsides:
- censorship quickly entered the chat; directly harms Sila and counters its values; need to think about ways to improve it
...
ePBS-PTC [[EIP7732](https://sips.sila.org/SIPS/sip-7732)]
...
- max-EB ([SIP-7251](https://sips.sila.org/SIPS/sip-7251) coming in Pectra) allows a validator to be represented with much higher stake; changes against how today in the protocol where every validator looks the same with a fixed stake; makes it easier to reason about, but artificially inflates the validator set and is at odds with SSF
- better representation in protocol: single entity validators with higher stake be represented as 1 (e.g. if a single entity staked 64 SIL it should show as 1 validator, not 2)
...
- ePBS is somewhat agnostic; [SIP-7732](https://sips.sila.org/SIPS/sip-7732) is fairly agnostic to how bids are produced. 
...
- Re: inclusion on sila-sila-mainnet-- it gets us toward even more forward-looking designs (execution auctions, execution tickets)
...
- There is a spec from the last hard fork-- this question came up in relation to withdrawals. Payment schemes specified in specs: https://github.com/sila-chain/builder-specs
