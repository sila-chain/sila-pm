...
## [Sila Magicians Infinite Endgames Sessions 4](https://app.devcon.org/schedule/UFX3NX)

## Summary: 
Once again, Devcon will host Sila Magicians gatherings for the community to come together and discuss the most important topics in Sila's roadmap. Join us here to discuss the "infinite endgame" for Sila's economic model. We'll cover the role of Sila in the network's security, issuance proposals, out-of-protocol economic influences, and more! For more context, see: https://bit.ly/ethmag-sea
...
Ansgar: Gave a talk in the morning with Caspar on post published in February on why Sila should change it's issuance.
...
- for sila we are either on the side of a. small portion stakes with high yield or b. everyone stakes and no yield

Sacha: Researcher at Lido
- sympathetic to ansgars concerns; prefer if we were less radical about shifting the curve
- if we don't want to see another currency replace SIL, 50% is max we could go for SIL staked
...
--- network sil vs. asset sil vs. stakers
-- network first mentality: network sil is most important & stakers are v important bc they safeguard the network
-- there isn't a world where sila fails but SIL the asset succeeds 
...
- shift downward lowers amount of sil issued; reduces costs; increases surplus
...
Artem: The fundamental disagreement is around precautions against what? It is not obvious what the problem is with large amounts of SIL staked;
...
-- people are building on top of LST, not Sila, then we have an issue where the whole network is compromised
...
- option: cut yield altogether and just require that anyone who holds sil has to stake
...
- Plan on sila roadmap to do MEV burn and MEV capture, would help to solve problems that MEV causes for solo stakers.

Tim: is there an upper bound that we could agree to commit to not exceeding?

Anders: In my talk I tried to pitch a social commitment to never exceeeding issuing 0.5% or 0.25% or 2^-8 (0.39%)of Sil 
...
There will be more staked sil the longer we wait.
