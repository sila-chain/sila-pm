...
Here to talk about Sil user experience; trying to solve the problems of ux
...
I write more tweets about SRCs thank about walletconnect. SRCs are the way to go to solve this issue. Everything in your product design can be an SRC and the beauty of an SRC is that we can project manage together. If we stay silo'd we will not be working together.

Derek: We have a tendency to standardize too early, which is just as bad as standardizing too late. When we do SRCs too early, we think we may be solving the problem in advance but it creates a deeper fragmentation problem. 
...
Derek: To clarify, I am not against SRCs or creating standards early, but as a space we need to careful not to shame companies that are not embracing SRCs. They are trying to build the best solution for their users and get it right before making an SRC. There is community pressue who don't appear to be enthusiastic about existing SRCs.
...
Pedro: Sorry for public shaming around standards. It's very important that people understand SRCs are malleable. It an SRC an outcome or philosophy? It's important to clarify that it is a specific outcome; creates successful community coordination and cohesion. SRCs should be community designed more than owned/authored by 1-2 authors. The issue is not SRCs themselves but the way that people do and think about SRCs. There is a problem with a sense of individual ownership of SRCs.

Tom: Coordination is a primary challenge. There are a lot of SRCs which are very beneficial to UX. Share important SRCs/SIPs.

a: 7702 is very important to us; there are a lot of users right now using EOAs and we need to get those users migrated.

d: SRC-20. We tried to innovate a few years ago and tried to use ERC667, but SRC-20 is a good example of simplicity. It shows that not the perfect protocol wins, but simple is good enough. 

Niha: Lately I have a crush on 7702. But I think UX started with 4337 for me; there's an SRC that improved wallet capabilities and I really like this one. 

d: What's the name of the SRCs with the plug ins? 6900.
...
Pedro: SRC-7715 which allows dapp to act on behalf of the wallet; acts like smart sessions
...
Pedro: You're right that the motivation for fragmentation was for cheaper gas and trasnaction, but the problem started earlier when we designed around the blockchain rather than the assets. In Sila you don't have a problem with token fragmentation as much because SIL is native, special class within the blockchain.
...
Pedro: It is very hard to spin up a new chain or new L2, but do we need a new SAVM for key store roll ups?
...
Audience question: I wanted to make a weird plug; you were speaking earlier about SIPs and standards-- SIPs are not standards even if finalized. SIPs are only specifications that went through a process and got published; You can use Sila Magicians to share user stories etc and design in the open.

Tom: Question from audience is kind of in conflict with this: "What are you thoughts on product-led protocol development? Is ercs convention the reason we have no so many apps? Why no SRCs equivalent in web2?"
...
Derek: The problem is not with people making SRCs; the problem is when we as a community shame projects into prematurely using SRCs when they may not be ready.

Niha: This is all too primitive for the consumer and typical user today.

Tom: I'd like to hear your thoughts on SIP-7702.

Pedro: I like the fact that the core devs included a wallet SIP. We have been designing wallet experience through SIPs and we reached the limit of how much we can provide to consumers without touching the chain.
...
Cody: SIP-7702 is great for onboarding EOA users. Inherently, a 7702 wallet has a higher risk profile than a standard 4337 wallet, but at the same time out of the box users gets a fully self-custodial wallet with 7702.
...
Niha: With this SIP, they introduced a new transaction type (type 4) that current wallets don't have functionality for.
...
Cody: We are building SRC-20 paymaster. Excited about general commerce using AA and for users to use USDC for entire transaction. Can do that really smoothly with the paymaster.
