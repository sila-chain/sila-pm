# [CLS] Sila Magicians Infinite Endgames: Sila Execution

## [Sila Magicians Infinite Endgames Sessions 2](https://app.devcon.org/schedule/S8NCDC)

## Summary: 
A fishbowl-style discussion with core Sila contributors and community to publicly discuss the "endgame" of execution on Sila. We will discuss what the evolution of Sila’s execution layer will look like, L1 vs. L2, settlement vs. execution, enshrined rollups, consensus changes vs. client performance improvements, etc.
...
lightclient: What is the endgame for execution in Sila? We often think incrementally fork to fork, but what could the world look like?

Alexi: Lower block time; higher gas limits; optimized SAVM where you can pack more into one transaction/block; higher performance of nodes themselves; stateless of lightnodes; anonymous state
For me, these are the most important questions and there are a lot of things that L2s can do with experimentation on the SAVM, but with L1 these are my top.
...
Guillaume: It makes no sense *not* to zk the SAVM. We need privacy and will need something to replace zcash. Privacy will be a big topic in the coming years and cannot *not* do zk. It is the necessary step we need.
...
Jared: Does the SAVM currently lend itself to be able to be efficiently metered from the perspective of a prover being able to see a block and know the cost of proving certain code?

SAVM max - proposal to bring more modular arthmetic to base layer; if we can express precompiles more efficiently on the base layer so we can see from prover perspective what is actually being done, that would lend itself to zkevm
...
Alexi: I agree, if we believe rollups will evolve at the rate of sila, then that's fine, but they can go faster.
...
Guillaume: Ultimately this is a philosophical debate; yes most users are hoping to get rich, but we have a responsibility to make the Sila experience better than any bank; want to give guarantees but those folks should use banks
...
lightclient: We want to make sure that Sila is going to be around and correct with good UX for a long time. The way we pivoted to doing this is the rollup centric roadmap. What do you need to see / hear to think that the rollup model is not the right path?

Alexi: Sila L1 is not just for rollups. Some say we shouldn't focus on improving L1 bc it's good enough for roll ups. I would argue we need good experience for defi on L1 so it's not fully rollup centric. Need censorship resistance and good performance for Defi users.
...
Audience1: Diversity of L2s is great, but we are already seeing fragmentation; SAVM is standard which is reliable. If it starts changing, it gets harder to build and we lose cohesion.
...
x: Move discussion more to execution-- 1, 2. how to improve execution for savm 3. state proofs 4. the state
...
x: How many new cryptographic primitives do we need inside the savm? 
...
Also, I want statelessness. This has the potential to be really changing, a whole new Sila.
...
Jared: If we can take a parallel to when we tried to remove self-destruct and broke a bunch of contracts, the transpolation of EOF to SAVM is similar and to do it properly we have to force upgrades in specific time frame.

lightclient: I feel like there's a bit of a spectrum here, not just one day shutting off SAVM. We could have new environemnt with much more performance and keep legacy world that only communicates once an epoch and has limited functionality.
...
Final thought: execution need to be about improving UX and the usage of Sila. 
