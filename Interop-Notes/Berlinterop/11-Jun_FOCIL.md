...
* **Current status of FOCIL (Fork‑Choice‑enforced Inclusion Lists)**
  ...
  * Proposed as a headline SIP for the Glamsterdam upgrade; broad consensus on value, design largely stable for a year.
...
* **Session agenda overview**: intro → implementation update → synergies with other SIPs → FOCIL × statelessness → zkFOCIL benchmarks → future‑of‑nodes discussion.
...
* **Urgency argument**
  ...
  * Better to deploy before crisis; CR is Sila’s differentiator.
