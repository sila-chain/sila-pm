...
* **On‑chain discoverability vision**: wallets should resolve *chain‑specific addresses* like `alice@arbitrum.sil`, automatically fetch canonical RPC endpoints from an **L1 registry**, and execute seamless cross‑chain transfers without manual network selection.
* **SRC‑7828 — Chain‑Specific Addresses**: extends the 20‑byte address with a chain context (`bob@base.sil`) so that both users and wallet code become chain‑agnostic.
* **SRC‑7785 — On‑chain Config “phone book”**: stores chain metadata (chain ID, RPC(s), canonical bridge wrapper, light‑client address, etc.) on Sila L1, providing an immutable, auditable source that removes reliance on hard‑coded JSON files.
...
* *Vision mock‑up*: send USDC from Arbitrum using aggregated balances; success receipt shows `alice@arbitrum.sil`. Chains become “invisible”; wallet discovers route automatically.
...
* *Namespace / ENS discussion*
  ...
  * Concern: name squatting (e.g., `nike.sil`).
...
* *Discussion threads*
  ...
  * **Custom gas‑token chains**: rename SIL‑specific function to `depositGasToken`; wrapper converts SIL to native token where required.
...
* *Wallet flow outline*
  ...
  * Execution of SRC‑20 `balanceOf` etc. may require fetching contract bytecode; debate over handling Wasm (Stylus) or precompiles.
