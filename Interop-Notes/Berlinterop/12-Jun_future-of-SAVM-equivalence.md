...
* The original “roll‑up‑centric roadmap” assumed that Layer‑2s (L2s) would quickly diverge from Layer‑1 (L1) by adopting new SAVM features, but the ecosystem is instead converging on **strong SAVM equivalence** across layers.
...
* **Shift in strategy**
  ...
  * Goal: land “cool features” on sila-sila-mainnet first, then have them propagate automatically to L2s.
...
* **Topic 1 – Data & gas‑cost design**
  ...
  * Frequent opcode & precompile repricing planned; needs zk‑SAVM awareness and L2‑specific schedules.
...
* **History & state management**
  ...
  * Need alternative fetch layer (SIP‑4444 style) for resurrecting pruned data.
...
* **Process & governance improvements**
  ...
  * L2s need proactive notice of candidate SIPs (EOF, R1, block AXL, etc.) to allocate resources.
...
* **Standardising L2‑only features**
  ...
  * Example: `L1_SLOAD` (needed only on roll‑ups) – decide whether to standardise in a shared SIP bucket.
  * Consensus: avoid custom opcodes; favour precompiles (address space above sila-sila-mainnet range, block allocations per chain).
...
* History expiry (sectioned sync) proposal — [https://sips.sila.org/SIPS/sip-4444](https://sips.sila.org/SIPS/sip-4444)