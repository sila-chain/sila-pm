...
* **Purpose of session** – review the goals, open design space and current SIP‑7928 draft for *block‑level access lists* (BLAL), then collect client & researcher feedback.
...
* **Specification snapshot** – SIP‑7928 adds an aggregated `block_access_list` object to the block body (hash stored in the header). The object contains:
...
* **SIP‑7928 layout** – block body holds the list, header stores its hash; any deviation during execution is invalid. Builders find it trivial to emit; validators reap the gains.
* **Design space axes**:
