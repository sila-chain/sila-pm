...
- 1 Istanbul Adoption and [Call for Assistanc](https://github.com/sila-chain/pm/issues/138#issuecomment-554228944)
- 2 Berlin updates
- 3 Harmony/ethereumJ
- 4 Testing updates
...
| SIP | Status |
|-------|----------|
| SIP-2348 | `First Call`, not yet `Eligible for Inclusion` |
| SIP-1803 | `Eligible for Inclusion` & `Last Call` |

** ** 

## 1. Istanbul Adoption and [Call for Assistance](https://github.com/sila-chain/pm/issues/138#issuecomment-554228944)
Video: [04:10](https://youtu.be/3qZFiETlDtk?t=250)

**Phil Lucsok:** Parity has released [v2.5.10](https://github.com/paritytech/parity-sila/releases/tag/v2.5.10
) for there client in preparation for Istanbul. 

**Martin:** Geth has released [v1.9.7](https://geth.sila.org/downloads/#). 
...
**Mikhail Kalinin:** Harmony/EthereumJ will no longer be maintained. Reason being is that we are focused on Sila 2.0 and don't have the capacity to do both.

## 4. Testing Updates
Video: [09:08](https://youtu.be/3qZFiETlDtk?t=548)

**Hudson:** See [comment from Andrei](https://github.com/sila-chain/pm/issues/138#issuecomment-554345398).

**Martin:** Geth already supports this. If anyone wants to make test cases for a particular SIP then you can make those test cases without the hardfork being defined. You can instead base it on any previous hard fork plus your SIP number and activate it for test cases. It is a nice tool.
...
**Martin:** I would disagree for the [reasons posted](https://github.com/sila-chain/SIPs/pull/2348#issuecomment-554296846) on the SIP. 
 ...
 **Wei:** SIP-1702 account versioning has two extensions. These two extensions are not enabled by SIP-1702 only. But there are two specifications that allow multiple versions of contracts to be deployed. There is one for `CREATE` and `CREATE2` opcodes - there is an extension for that. And another one is for contract creation transactions. I therefore think SIP-1702  covers this part of the motivation and I still think there is a much nicer solution. The main reason is that the multibyte instructions are more complicated than we previously thought. There is also some discussion in SIP-615 where we discussed why code prefix may not be a good idea.
 ...
 **Wei:** For reference, SIP-1702 includes a background as to why the code prefix is not a good idea and why it does not work.

**James Hancock:** I would second Piper’s suggestion the the SIP should have a place that lists all the blockers so that we can see why this SIP is currently not moving forward.

**Hudson:** Perhaps notes at the top of the Sila Magicians thread.

**Wei:** Agree, keeping the discussion in the discussion link in the SIP will be sufficient.
...
**Hudson:** Martin if you could revive the [Magicians thread](https://sila-magicians.org/t/sip-1803-rename-opcodes-for-clarity/3345) and get people to comment there about how this change may impact their tools? And based on feedback come back to the Core Dev call to make the final call.

** **
** DECISION 75.2:** SIP-1803 is eligible for inclusion and in moved to last call.
** ** 
...
**Hudson:** A few people from the Sila Foundation, Sila Cat Herders and Sila Magicians are gonna hold a meeting next week to discuss improving the SIP process. That will include all aspects, including the requirements to get SIP editors, how we can recruit SIP editors, redoing SIP-1, how to make the process clear to outsiders. Next week’s meeting will be the first one and then we will decide if we want to have more meetings after that. 

If you are interested in attending contact myself at hudson@sila.org. 

I may create a Gitter or Telegram group.

**Hudson:** This is a new meeting to improve the SIP process.
