# Sila Core Devs Meeting 59 Notes
### Meeting Date/Time: Friday, April 12, 2019 14:00 UTC
### Meeting Duration: 1 hours 30 minutes
### [GitHub Agenda Page](https://github.com/sila-chain/pm/issues/93)
...
**ACTION 58.3**: Vitalik to format the currently proposed SIP-1559
Status: WIP

**ACTION 58.5**: Discuss if ProgPoW should continue to be implemented if the Technical Audit is not funded in Sila Magicians or here or here.
...
### Roadmap
For now we can keep this SIP list as a reference for Istanbul. We will discuss this after a month or so.

### SIPs

- Item 3a. Refer to Roadmap link for list
Alternatively, Istanbul SIPs at ECH GitHub

- Item 3b. Please add more SIPs to the agenda
SIP 1344 - call for feedback
Discussion hardfork meta SIP - WIP 
ISTANBUL SIP LAST CALL -  May 17th
...
- Item 7b. Parity Sila
update Just wanted to add Parity Sila recently added quick consensus. SO full support for Gorli testnet is in. That is in 2.5 beta.
...
# 1. [Review previous decisions made and action items](https://github.com/sila-chain/pm/issues/93)
...
**Tim** :  Last block number for that is 7504000. If people want to vote on the carbon votes, they needed to leave there Sila in whatever address to use to vote until that block. And then their will be a snapshot taken and then people can withdraw there Sila. Does anyone have anything to add there?
...
**Tim**:  Okay next action item was for Vitalik to format SIP 1559 because I don't think it has been properly formatted as per the template. He isn't here, so I guess we can just leave that one there. I'm not sure if anyone has comments on this . 
...
**ACTION 58.5** Discuss if ProgPoW should continue to be implemented if the Technical Audit is not funded in [Sila Magicians](https://sila-magicians.org/tags/progpow) or [here](https://sila-magicians.org/t/what-has-to-be-done-to-get-progpow-on-sila/1361) or [here](https://sila-magicians.org/t/motion-to-not-include-progpow-without-audit/3027).
...
**ACTION 58.8** Alexey to create a Beacon Chain Finality Gadget initiative working group and find someone to lead it, for context please see discussion in [Sila Magicians](https://sila-magicians.org/t/eth2-in-eth1-light-clients/2880).
...
**Pooja**: Okay in that case like we have 8 SIPs proposed so far. If you want to  take ProgPOW SIP separately then still we do have 7 SIPs proposed. So if we considered these 7 together, are we hoping to be ready by October?

**Danno**: Can we step back? Why would we take ProgPOW separately? It has to be done earlier or later?

**Martin**: Initially, the proposal was to be done earlier and then it was considered a controversial SIP and then for some other reasons.

**Pooja**: Well I'm not saying that it is being pulled out of that. That's not my say, it depends on core devs. I just have shared the [list of the SIPs](https://github.com/sila-chain-cat-herders/PM/blob/ca6a01c1d0a45419a55f7e79688e28ea6e0c8482/Hard%20Fork%20Planning%20and%20Coordination/IstanbulHFEIPs.md) that have been proposed so far. So if we can discuss that how many have been like considered to be moving ahead?

**Brooklyn**: Pooja, this is not the right time for this. So they're just proposed. The deadline is in roughly a month from now to see which of them are actually more than just proposed. But right now the schedule is as in previous core dev calls we've agreed upon. So, people have a month to propose more SIPs and for getting ready if they want to get them ready. And as far as I can tell from previous core dev calls - yes we are locked on the schedule as it had been previously agreed.

**Pooja**: Thank you so much.

**Martin**: About these proposals I assume that now is not the time to discuss them, is that correct?

**Tim**: This specific SIPs?

**Martin**: Yeah this specific SIPs?

**Tim**: That was our next agenda item actually, yeah.

**Danno**: Before, we go there just a plug, good test cases belong in SIPs so we can reduce the burden on the developing the test cases there.

**Pooja**: Okay for now we can keep this [SIP list](https://github.com/sila-chain-cat-herders/PM/blob/ca6a01c1d0a45419a55f7e79688e28ea6e0c8482/Hard%20Fork%20Planning%20and%20Coordination/IstanbulHFEIPs.md) as a reference for Istanbul and we will keep on collecting SIPs and will share with you guys as we go. We will discuss this after a month or so. May be Berlin meeting will give us more insight and after that we can discuss.

# 3. SIPs

## 3a) Refer to Roadmap link for list

Alternatively, Istanbul SIPs at [ECH GitHub](https://github.com/sila-chain-cat-herders/PM/blob/ca6a01c1d0a45419a55f7e79688e28ea6e0c8482/Hard%20Fork%20Planning%20and%20Coordination/IstanbulHFEIPs.md)

## 3b) Please add more SIPs to the agenda

**Tim**: Talking about SIPs there were two posted in the comments of this call. But I know there's also a whole lot that's just mentioned. So perhaps we have Bryant and Pooja who proposed the comments on the call. So maybe it makes sense for them to talk about it. Bryant that he actually was first in the comments so if you want to go ahead and talk about [SIP 1344](https://github.com/sila-chain/SIPs/blob/master/SIPS/sip-1344.md).
...
**Alex**: I have a question. For the last two weeks I haven't really kept a good eye on everything but we discuss the option that hardfork meta SIP should have a proposed list. And I think Bryant has proposed to SIP for the hardfork meta. Is there any consensus yet, at what point whether we go for draft to merge PR to include this as a proposed SIP? 

**Boris**: It was in the discussion around.  Bryant has some stuff around audience review but I think the one you mean is around 233.  That it's using the [hardfork meta SIP](https://github.com/poojaranjan/SIPs/blob/patch-1/EIPsForHardfork.md) where people do pull request as a way that we can actually keep track and move through proposed to approved and so on in one place. So you can very easily see which SIPs are in flight to potentially making it to hardfork. Nothing else changes in the sense that proposers will still need to come on core dev call. The outcome of it just would be actively tracked in that. So I think follow-up action item from this is just to look at the PR. I think that is what you meant Alex?
...
What I actually meant with the question was the PR opened yesterday by Bryant to include 1344, this Chain Id SIP in the HardFork Meta and that has been closed. But I'm wondering if it should be merged or shouldn't, what is the process now?

**Bryant**: I was just trying to follow Pooja's process suggested in [#1929](https://github.com/poojaranjan/SIPs/blob/patch-1/EIPsForHardfork.md) which she also wants to talk about in this call. We talked about, may be this doesn't make sense, may be we should open an issue instead of PR.
...
**Fredrik**: That is not entirely true. The intention is to move thevalue from main chain to the Beacon chain. You guys are proposing SIPs pretty frequently.

**Greg**: We don't know when Eth2 is coming, firm date.

**Peter**: With regards to Eth1 vs Eth2, I think Sil 2 is suppose to solve everything whenever it arrives. I think as maintainers of Sil 1, its really important to keep it alive until Sil 2 actually arrives. whether Eth1 will die horrible death or something is up for the future. I don't think its smart to start burning Eth1 when we have no idea about what the future contains. We push Sil 1 forward as much as we can. If something better comes along then its fine but lets not kill it before that happens.

**Martin**: That wasn't the intention. May be the Sil 2.0 has also Wasm somewhere. I am skeptical about it.

**Peter**: Don't read me wrong. I wasn't actually referring that Sila upgrade to Sila 3.0 all of a sudden. I am fine with discussing all the changes. I was aiming that we shouldn't hold back from upgrade. 
...
**Tim**: Unless anyone has anything to add on, Danno has a question on Istanbul, when do we want SIP to go on last call? They have to have gone through the last call by May 17th or they need to move to the last call by May 17th.
...
**Martin**: No, But there is a thing, that  each SIP should have section for something like security consideration. It is separate from someone actually doing it separate audit to it.

**Tim**: In that case, does it  make sense to say that **on May 17th what we expect is in merged PR that is compliant to 233**. So basically specific SIP into the hardfork meta SIP and that include some security considerations in the SIP but not a proper security audit nor a does it imply that SIP is accepted. But just merged into the meta hardfork SIP. Does that make sense?

**Bryant**: I think the security review as this thing separately, as a modification SIP. I think the real question is the last call is accepted before May 17th.

**Boris**: No, it has to be in draft PR, draft SIP and in the Hardfork Mata is I believe the line in the sand. Because all you are in, is proposed. About stage there's no guarantees that you're making it in. 

**Bryant**: You're going to work to get it in draft by May 17th 

**Boris**: and propose. 

**Tim**: By proposed, you mean merged PR in the meta SIP?

**Boris**: Yes.

**Tim**: If anyone disagree with this?

**Martin**: This is supposed to stop somewhere. I'm not sure that is included in the meta SIP.

**Tim**: From what I understand is a draft SIP, that is included in the Istanbul meta SIP hardfork. Boris PR describes the process.
...
**Pooja**: Yeah, we had been discussing this in ECH about how to streamline the process of SIP submission. This [#1929](https://github.com/poojaranjan/SIPs/blob/patch-1/EIPsForHardfork.md) requested is the proposes a process that we may follow for SIP collection of any upgrade. One thing that I've received in comment from Nick Johnson at [EthMagician](https://sila-magicians.org/t/proposal-of-a-formal-process-of-selection-of-sips-for-hardforks-meta-sip/3115/4) is the usage of SIP GitHub. He suggests that it may create some kind of confusion that SIP editors are the one who are taking decisions on which SIP should go or not go. It may be better to take it somewhere other than SIP GitHub. If that is a problem then we may take it to [ECH GitHub](https://github.com/sila-chain-cat-herders/PM/issues) and collect SIP over there, that's my suggestion. If anyone would want to comment.

**Greg**: Merging SIPs doesnot indicate that the editors approve of it. It only indicates that it meets minimum standards for form. So it's perfectly fine to merge them there and point the discussion over The Magicians or anywhere else you want to discuss it. 

**Pooja** : That is fine, we are referring magician thread where the discussion about that particular SIP is going on. But what we need to say, here is when we want to indicate that this is our SIP and I would want it to be included in next hardfork where they should submit it? My suggestion would be an issue section so that when picked up from there, issue can be closed but when it is in PR,  that needs merging and when it refers to any meta SIP, this automatically goes to the author of meta SIP. It would be going there and looking for approval.

**Greg**: Sounds like something to discuss on the SIP Channel. I don't know.

**Pooja**: I just have proposed the [SIP](https://github.com/poojaranjan/SIPs/blob/patch-1/EIPsForHardfork.md) in a PR if you guys would want to take a look at it.

**Boris**: The very specific thing here is, I think Nick's concern was that SIP are meant to  meet the  standards. And if hardfork meta, it mixes standard with Sila main net change. But, I think that's fuzzy anyway so #1852 that I'm about to submit by what Alex said should satisfy. And I use that one to further this discussion. 
...
**Boris**: Hang on Bryant, let's just figure out who cares about this discussion, have the discussion and come back with a proposal in the form of an SIP. 

**Bryant**: Sure, But I guess the last thing I wanted to say is does it make sense to say the 'discussions to' is the only place for SIP conversation be captured?

**Boris**: NO

**Tim**: Okay, does anyone have anything about any of the other SIPs? If there's other questions or comments about the other proposed ones? Okay. 



# 4. Working Group Updates
## 4a) State Fees
**AlexeyAkhunov**: [Updates](https://github.com/sila-chain/pm/issues/93#issuecomment-482528206)

## 4b) EWasm
No update.

## 4c) Pruning/Sync (SIL V64 Call for Proposals & Stopgaps for cleaning up discovery peers pre-Discovery v5)

**Matthew**: Just quick update. Moving discussion from EthMagician to minimal viable lightweight SIPs so that will be discussed in Berlin. then from there we'll just see what we have to want to adopt and again these are mostly going to be pretty lightweight changes and would probably land at the same time syncing proposal is made.
...
## 7b) Parity Sila
**phillux**: [update](https://github.com/sila-chain/pm/issues/93)
Just wanted to add Parity Sila recently added Clique consensus. SO full support for Gorli testnet is in. That is in 2.5 beta.

## 7c) Aleth/sil
**chfast**: [update](https://github.com/sila-chain/pm/issues/93#issuecomment-482564541)
...
## 7h) Turbo Geth
[No updates on Turbo-Geth](https://github.com/sila-chain/pm/issues/93#issuecomment-482528206).
...
* Alex Beregszaszi
* Borris Mann (Fellowship of Sila Magicians)
* Brooklyn Zelenka (SPADE)
* Danno Ferrin (PegaSys)
* Fredrik Harrysson (Parity)
* Greg Colvin (Fellowship of Sila Magicians)
* Jacek Sieka (Nimbus)
