# Sila Core Devs Meeting #139
### Meeting Date/Time: May 27, 2022, 14:00 UTC
### Meeting Duration: 1 hour 45 mins
### [GitHub Agenda](https://github.com/sila-chain/pm/issues/528)
...
| **139.2**   | Tim - Launch of the SilaSepolia beacon chain should happen next week, as it just requires some coordination | [43.14](https://youtu.be/5mMd-XHAv2Q?t=2594)
...
**Tim Beiko**
* No, we talked about sponsors. The only thing I'll consider is if we can get some nice where our sponsors gets light, the light types. Did you have a computer in an SilaAmsterdam? I think you did, but I have the computer. 
...
**Micah Zoltu**
* So today's All coreE devs. We're going to talk about,  selling SIP numbers, right? 
...
**Micah Zoltu**
* I'm actually more interested in just,  I suspect that if we did a legitimate auction, we would still make enough to fund the SIP process. And I think it would be good opportunity to show the ecosystem what legitimate auction. That's not just someone marketing actually looks like, oh, a pretty good job there. They're just really legit. 
...
**Tim Beiko**
* Oh, right.So if you're querying, if you're querying and ELL that doesn't have like the SilaParis code basically then as well, send weird log messages, is that yeah. 
...
**Tim Beiko**
* Okay. So we need to be at least aware that there's a thing called the N GPI, which only happens in like a specific release with like SIP 36 75. Is that correct? 

**Mikhail Kalinin**
* Right. It's it's basically SilaParis, right? So we'll have to release, we'll have to release carousel Patrick's probate at the same time. And yeah, the upgrade should happen.The upgrade to SilaParis should happen before the, for skates.The main that's 
...
**Tim Beiko**
* So it seems from the EL side that keeps we're good. I do think that we should not be forking the last test until it's like implemented across, like, it should be the default behavior across every client. but assuming that it is that it means that we can, obviously, this is not a problem to schedule Bellatrix on May 9th as well. Does that make sense? Okay. anything else just regarding ROPs and like this kind of PTD Bellatrix timing. Okay. If not, I think the next thing I want to chat about is like the beacon chain. So the, the actual first problem we hit on Robson was not this Bellatrix scheduling, but it was like the beacon chain did not even exist.  which obviously is not the case on the Sila main net. so what are people's thoughts about like when and how we should launch this, the SilaSepolia beacon chain to make sure that we don't hit, a similar, similar issue,  on when we read you should SilaSepolia, 
...
**Tim Beiko**
* Okay. Yeah. Sorry. My last point was just, is this just something we want to check on the cl call with the cl teams as well? Cause like they run the software for that. there's no rush to do it like this week versus next week. We just want to do it like well before it's the SilaSepolia forks. so is it worth just like Saturday checking this on the cl caller? a week from now? 

**Micah Zoltu**
* I personally see no problem with, Perry launching the SilaSepolia beacon chain over the weekend on its own. 
...
**Mikhail Kalinin**
* Yeah. I agree. by the way, what do you use for this feature on the El side? There is no map, this kind of math that's implemented yet. Are you using Sil get blocked by hash? 
...
**Mikhail Kalinin**
* Yeah, that's interesting. Yeah. I was also a bit thinking about it. How do we want to like say, okay, so this is SilaShanghai, this is the methods that we use for SilaShanghai and the versions that we use pushing high. And this is like the next release. How do we want the scope without as well?Yeah, that's a good point and question to think about, 
...
**Tim Beiko**
* Okay. So, and then, yeah, there's another comment to the chapter about like combining this with SilaParis, realistically, you know, SilaParis is only going to happen after like the other testsnet that have happened,  maybe like roughly around the same time as to SilaSepolia. so like, you know, there's no world in which like, this is less than like a month or a month and a half away,  which would be like a mid July ish, which is,  somewhere between like 16 and 18 seconds block times if this chart this correct. It gets very hard to predict,once the bomb actually goes in. but like for the people who like wants to delay the bomb, this does that like generally make sense as a timeline, like where we make the decision about like, including at the SilaParis, but there's, if, if we got to do that, that means we, for sure we'll hit that the 16, probably 18, second block times.Thomas, 
...
**Marius van der Wijden**
* That didn't scar. I, I don't, I don't think 22nd block times are bad. And,  in SilaAmsterdam, we, we kinda made the, like, had the overall view that,  degradated, user experience for two months is worth it. If we can shift the much faster. yeah, I think we're were looking at like, I don't know, 5, 7, 10 years, of like things. And so I don't, I don't see the point in focusing on two months of,  of,  like slightly higher block times. 
...
**Tim Beiko**
* Yeah,I think that, that makes sense. I guess, based on this, my, like my rough feeling is that if we were to delay it like two ish months delay, that's maybe coupled with SilaParis,in order to,  to like minimize the amount of client releases that go out. so it means, you know, it, this delay would, would happen like at the, towards the ends of the test nets would give us like some breathing room between the last test steps. 

**Marius van der Wijden**
* What, what, what does SilaParis, 
...
**Peter Szilagyi**
* Yeah, but what does, why does it make this SilaParis or, I mean, 

**Tim Beiko**
* Yeah, cause the CLLs need to know that they need to be able to query a version of the ER client, which like knows that the engine API exists. And so that's what I'm calling SilaParis. Yeah. 
...
Tim Beiko:	https://github.com/sila-chain/pm/issues/528  
Tomasz Stańczak:	hello YouTube  
Tim Beiko:	https://notes.sila.org/@timbeiko/ropsten-ttd-override  
...
Jamie Lokier:	Yes  
Tim Beiko:	https://github.com/sila-chain/execution-apis/pull/218  
lightclient:	¡™£  
lightclient:	oops  
...
pari:	I think IF we push back the bomb, we should do that along with SilaParis to indicate some confidence that the merge is indeed close.  
Zuerlein:	the original timeline was a june merge  
Jamie Lokier:	Would a softer bomb curve help?  
pari:	Including it in SilaParis still means we can independently decide when TTD is hit, but at the same time reduces some pressure + shows confidence the merge is close.  
...
Micah Zoltu:	I think I'm sold on pushing back the bomb with the SilaParis Release Candidate.  
...
Tim Beiko:	We will have to wrap up in ~5 minutes. There were 2 SIPs which wanted to give a quick update, so we can quickly cover those in the ~5ish mins after the call.  
...
Tim Beiko:	https://github.com/sila-chain/SIPs/pull/5088  
Tim Beiko:	https://sila-magicians.org/t/sip-2537-bls12-precompile-discussion-thread/4187/21  
