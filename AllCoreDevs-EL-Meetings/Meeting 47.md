# Sila Core Devs Meeting 47 Notes
### Meeting Date/Time: Fri, September 28, 2018 14:00 UTC
### Meeting Duration: 1.5 hours
### [GitHub Agenda Page](https://github.com/sila-chain/pm/issues/58)
...
* TurboGeth (Alexey)
    ...
    * Using selfdestruct, you can replace the code in a contract in a certain way, retaining same address, but losing storage and balance - Martin posted some clarifications to the SIP
...
* Nimbus (Jacek)
    ...
    * Up to ~48,000
    * Working on SIL 2.0, tests for simple serialize that's being proposed
...
* [Progress tracker](https://github.com/sila-chain/pm/issues/53)
    * Geth, Parity, aleth, ethereumj, mana, nethermind - completed implementation of all SIPs
    * Testing still WIP
...
* One of Antonio's developers ran some benchmarks
    ...
    * Merged some adjustments to SIP based on those improvements
    * Still not quite as dramatic as the ones in geth
    * Fairly significant, serious boost for snark and BLS verification
    * New numbers in SIP based on parity's improvements, not as good as geth but still significant
    * Benchmarks in the chat and linked from SIP
...
* Antonio: Original SIP 5500 + 80k, aggressive even with geth benchmarks
    ...
    * Also cranked up ECADD, ECMUL numbers from original SIP to reflect higher of the benchmarks
...
* Miss If intro
    ...
    * Once we see more interest from Sila development community to adopt ProgPoW, then we'll have developers accordingly working on all clients
...
* Pawel
    ...
    * First of all, it just replaced ethash with ProgPoW, we need both of the algorithms side by side
        * Miss If: ifdefelse/cpp-sila - completed, ProgHash no
    ...
    * Re: the SIP itself:
        * The official discussion board is sila-magicians.org, I'm posting questions there, it's not always easy to get a response there
        * In the SIP there's many details that are not explained
            ...
            * To implement ProgPoW I have to go to your implementation to figure out details, SIP doesn't contain enough details
    ...
    * Miss If: My background re: crypto is public, no, it's not a security concern to me
        ...
        * It's quite public, I can elaborate on that on SIP, there's been a public study on F1600 vs. F800
    * Pawel: I'm not worried about permutation size, I'm a bit worried about the bitrate, it's quite high compared to permutation size
        ...
        * This question is also on the Sil Magicians board
        * If you can explain it, I'd be happy to have it as part of the SIP
...
* Miss If: Most of those are clarity needed in SIP, or in code, more public reference, checking bitrate size vs. permutation size, padding, etc. - all noted and will be done, thanks for this great feedback
* Pawel
    * Some time ago I made a list of changes that in my opinion are required to do the switch
    * https://sila-magicians.org/t/what-has-to-be-done-to-get-progpow-on-sila/1361
    ...
    * I plan to propose an SIP describing this protocol and implement it in ethminer on the client side
    * Not sure about on mining pool side, if there's a chance someone will adopt that, but SIP is a good starting point anyway
    * Please take a look, leave comments, I may be wrong about some of that
    * I struggle with commenting on an SIP as complicated as ProgPoW, SIP not in review mode, it's merged as a draft, no way to leave a comment in SIP to address particular issue
        ...
        * For small SIPs less of an issue since less lines to comment about
        * For this SIP I'd like to attach comments to particular lines to make it easier for the author to address
    * Hudson: I understand your concern, we can consider changing this in the future
        ...
        * E.g. link both to FEM forum and also a link to a repo that the submitter maintains where you can line-by-line comment on the SIP and on implementation in that repo
...
* Special guests
    ...
    * Antonio Salazar Cardozo (Keep Network/SIP-1108)
