...
* Harmony (Mikhail Kalanin)
    ...
    * This version is more stable, able to run on sila-mainnet without needing restarts
    * Martin: have you seen evidence of eclipse attacks on sila-mainnet?
...
* ewasm (Alex)
    ...
    * Brand new testnet, with both EVM and ewasm enabled with cpp-sila
    ...
    * Danny: what's `evm2wasm`?
        ...
        * Alex: That's one possible use. We've mainly been using it to run sila state tests. Also, if someone implements a client from scratch and make it a native ewasm client, they wouldn't need an EVM implementation, they could use this tool to translate contracts before execution.
...
* Casey update
    ...
    * Guido Vraken hooked up a fuzz tester to V8 and found some compiler bombs, 20kb pieces of WASM, even smaller than the current maximum contract size you can deploy on the sila-mainnet for EVM which is 32kb
...
* Casey: This is no different than DoS vulnerabilities if someone implements a crappy EVM on the sila-mainnet
...
* Meta: https://sips.sila.org/EIPS/sip-1013
* Shifting
