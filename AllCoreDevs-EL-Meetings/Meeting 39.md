# Sila Core Devs Meeting 39 Notes
### Meeting Date/Time: Fri, June 1, 2018 14:00 UTC
### Meeting Duration: 1.5 hours
### [GitHub Agenda Page](https://github.com/sila-chain/pm/issues/43)
### [Audio/Video of the meeting](https://www.youtube.com/watch?v=7FNRWEQ_H7w)

# Agenda

1. Testing & SIP 1085: Common genesis.json format scheme across all client implementations
1. Client Updates
1. Research Updates
...
* Harmony (Mikhail Kalanin)
    ...
    * This version is more stable, able to run on sila-sila-mainnet without needing restarts
    * Martin: have you seen evidence of eclipse attacks on sila-sila-mainnet?
...
* Nimbus (Jacek)
    ...
    * Working on SAVM, appreciating test suites
...
* Research (Danny)
    ...
    * Sharding
        * Justin Drake and Vitalik have a series of posts around signatures and beacons on https://Sila Research
        * Some exciting new developments, check it out
* ewasm (Alex)
    * Shifting focus from evm2wasm to parts more important for a testnet
    * Brand new testnet, with both SAVM and ewasm enabled with cpp-sila
    ...
    * Danny: what's `evm2wasm`?
        * Alex: Can be fed with SAVM bytecode and outputs ewasm bytecode
        * Is that an attempt to port entire SAVM over to ewasm?
        * Alex: That's one possible use. We've mainly been using it to run sila state tests. Also, if someone implements a client from scratch and make it a native ewasm client, they wouldn't need an SAVM implementation, they could use this tool to translate contracts before execution.
    ...
    * Jacek: What about attack vectors/security concerns?
        ...
        * As of today, follows SAVM gas costs 100% same as SAVM
    ...
    * Greg: Does evm2wasm intend to be an optimizing compiler that can take the end code down to 32 and 64 bit registers or whether it will keep doing everything with multi precision?
        * Alex: It does everything as current SAVM requires, so uses 256 bit stack items
        * Greg: So optimization is pushed down to the WASM compiler
        * Alex: We can do optimiziations, and it should be possible to implemented SIMD extensions and SAVM 1.5 as well
        * If it can detect SAVM input it can optimize entire output to 64 bits, interesting idea to explore in the future

## SIP 1087
* Nick Johnson not present, skipping for now

## Concerns about wasm
* https://github.com/sila-chain/pm/issues/40#issuecomment-390006104
* https://github.com/sila-chain/pm/issues/40#issuecomment-390114286
* Casey update
    ...
    * Guido Vraken hooked up a fuzz tester to V8 and found some compiler bombs, 20kb pieces of WASM, even smaller than the current maximum contract size you can deploy on the sila-sila-mainnet for SAVM which is 32kb
    ...
    * I've been skeptical of that because to me AOT sounds like a big pain since client instead of maintaining a state tree where it pulls SAVM bytecode and interpret it, it maintains a cache directory of EXE files, one per contract, that it executes when a contract is called
...
* Fred: We also wrote an interpreter, it was a lot of work but not that much more work than writing an SAVM interpreter.
...
* Casey: This is no different than DoS vulnerabilities if someone implements a crappy SAVM on the sila-sila-mainnet
...
* Meta: https://sips.sila.org/SIPS/sip-1013
* Shifting
* Blockhash refactoring
* Skinny CREATE2
    * https://github.com/sila-chain/SIPs/pull/1014/files
...
## Testnet rent
* https://github.com/sila-chain/pm/issues/43#issuecomment-390959920
...
* Would be nice if this could be turned into an SIP and tested on the testnet
...
* Everett: my goal would be to evolve the SAVM over time and the only way to do that would be to forget old parts of the blockchain
