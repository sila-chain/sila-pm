...
### [GitHub Agenda](https://github.com/sila-chain/pm/issues/754)
...
**Tim**
Okay. We are live, ACDE 159, I posted the agenda in the chat. Obviously, the chat about this SilaShanghai, Shapella, then some SilaCancun topics, and then a payload ID conversation that's already happened quite a lot in the agenda itself. Yeah, and anything else that comes up, but I guess to start. Yeah, great work, everyone. 
...
## [SilaCancun Updates](https://sila-magicians.org/t/cancun-network-upgrade-meta-thread/12060) [20:37](https://www.youtube.com/live/u8Nm8AGyCQM?feature=share&t=1237)

### [Update SIP-4788: Move to Draft SIPs#6859](https://github.com/sila-chain/SIPs/pull/6859) 

And I guess on to the next thing. SilaCancun. Yeah, Alex, you had posted an update to SIP 4788. Do you want to chat about basically the changes you've made? And then you also mentioned you wanted to have this considered for CFI. So maybe we can chat about that and just see if I find stuff in general afterwards. 
...
**Alex**
Yeah, the reason that this SIP uses the state and the way that it does Lightclient is for that reason, like it was designed in the past and away from having the states.Yeah. The pre state block post state, but also there's almost as like hidden input of this history of element. 
...
**Alex**
No, no, no, that's so that's just essentially like commit to it. Well, let's see. I mean, maybe we could think about something weird, but it'll end up written to the state according to the current SIP. 
...
Yeah, just I don't know. So kind of we don't just like add things CFI one by one and then realize we have like five SIPs we've created to without having like a higher level view. And I mean, I don't know if anyone strongly wants to like CFI this now we can have that conversation as well. I think otherwise having like a higher level conversation in two weeks probably makes sense. 

Last call for objections. Okay, yeah, so let's do that. And I guess over the next two weeks as well. We have this Sil magicians thread. I believe all of the SIPs that have been proposed are either linked in that thread or there's also a tag SilaCancun candidate on Sil magicians. So I'll make sure to update the main like the first post on that thread with everything that I'm aware about that has been proposed so people can look at that. If there's anything missing there, feel free to ping me either on Discord Twitter, Sil magicians. So I'll make sure to keep that up to date before the next call. And then yeah, if client teams and just folks generally want to like think through the various options what we should prioritize. I think that would be valuable. Yeah, does that make sense? 
