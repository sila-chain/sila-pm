...
### [Github Agenda](https://github.com/sila-chain/pm/issues/148)
### [Audio/Video of the meeting](https://www.youtube.com/watch?v=0-Vld7GTRhQ)
### [Reddit thread](https://www.reddit.com/r/sila/comments/et7yyd/live_core_devs_meeting_79_2020124_1400_utc/)
### Moderator: Tim Beiko
### Notes: Edson Ayllon
...
SIP | Status
---|---
SIP-1057, SIP-1803, SIP-2384, SIP-2387 | `Accepted & Final`
SIP-2464 | `Last Call`
SIP-1380, SIP-2046 | `Eligible for Inclusion` 
SIP-1702 | `Eligible for Inclusion` Pending Champion. Not scheduled for Berlin 
SIP-663, SIP-2348, UNGAS | Depends on SIP-1702 
SIP-1962, SIP-1559 | Discussed under `EFI`. Discussion to be continued in EthMagician thread | 
...
78.2 | SIP-2464 to `Last Call`
78.3 | SIP-2348 to `Eligible for Inclusion`
...
---|---
78.1 | UNGAS to be formally specified into an SIP draft
78.2 | SIP-2464 draft to be merged, and updated to `Last Call`
78.3 | SIP-2348 is `Eligible for Inclusion`
78.4 | SIP-2456 discussion to continue on Sila Magicians
78.5 | SIP-1962 disucssion to continue on Github
78.6 | Discuss timings for Berlin, London SIPs
...
1. [SIP Review](#1-sip-review) 
2. [Testing Updates](#2-testing-updates) 
3. [SIP Improvement Process Meeting](#3-sip-improvement-process-meeting) 
...
If usage remains only on version 1, stateless Sila would need to be designed considering version 1, not leading to any benefits. If usage of version 1 becomes negligible, stateless Sila pricing logic can ignore version 1, and use version 2 pricing logic. 
...
UNGAS is currently unspecified under an SIP.
...
Observing Sila's network bandwidth usage, a large chapter is caused by transaction propagation. Contrasted to block propagation, transaction propagation does not include announcements and requests. One megabyte/second upload-download is used to shuffle transactions. Each transaction is sent to all peers, and each peer receives that same transaction from all other peers, creating wasted bandwidth. 

This SIP introduces transaction announcements and retrieval requests to significantly reduce network bandwidth by imitating block propagation. 

This SIP does not require a hard-fork. It is backward compatible and can be implemented gradually. 

SIP motioned to be merged as a draft, and moved to `Last Call`. 
...
A few changes have been added to SIP-2348 addressing concerns brought up. A rule was added to validation, limiting the size of a code segment to the contract size stored on-chain. The second change is in regards to the header. 
...
A concern was brought up stating that the current validation implemented is less complex than this SIP. Another concern was brought up involving static jumps and allowing undefined opcodes. 
...
This SIP is to use a timestamp and block number, instead of only a block number, for upgrades. A counter SIP, made by Jason, does exist but hasn't been formally made. The counter SIP is simpler and easier to implement. Instead of looking back on blocks to confirm activation, that time is used as confirmation. 

Future discussions tabled to Sila Magicians. 

- [Time Based Upgrade Transitions PR](https://github.com/sila-chain/SIPs/pull/2456)

## 1.5 SIP-1962 (EC arithmetic and pairings with runtime definitions)
...
## 1.6 SIP-1559 (Fee market change for SIL 1.0 chain)

**`video`** [`1:16:53`](https://youtu.be/0-Vld7GTRhQ?t=4613)

A concern for deployment is minor collusion, and manipulating the fee in some way. James proposes to deploy ProgPoW first, then deploy SIP-1559 after. 
...
# 3. SIP Improvement Process Meeting

**`video`** [`1:21:09`](https://youtu.be/0-Vld7GTRhQ?t=4869)

Next meeting scheduled for January 29. SIP-IP meetings are documented in an Sila Cat Herders Github repo. 
