...
### [Github Agenda](https://github.com/sila-chain/pm/issues/396)
...
| 124.3         | SIP-3607 moved to final                         | [1.01.30](https://youtu.be/BTtwbvZZpfs?t=3690) |
...
Great so that he will make sure we are ready by February. So will I. Cool. Let's do 10.7 I doubt we're gonna get a cleaner consensus than this and now the other kind of big decision to make is when do we actually have this upgrade? So the bomb is scheduled to start showing sometime in December we probably need a month between the client releases and the actual upgrade on SilaMainnet because that means kind of probably three weeks by the time we announced them and the fork happens. So maybe starting backwards like given that we have the number to push back the bomb for today how quickly can client teams have a release out that has the bomb number and obviously we also have tests for it?
...
And one quick question about the December upgrade. So obviously it's a small upgrade, just changing the difficulty-bomb block. I know there are still like 20% of open Sila nodes operating and open Sila has been deprecated. Do you think that open Sila would be upgraded for this purpose? Or is the expectation that anyone depending on open Sila would switch by then?

**Marius Van Der Wijden 1:03:38**

The expectation is that you should have already switched by now. But I think it's it's it's not a big change to update open Sila. And I think someone from the community will probably do it. But that shouldn't mean that you should run open Sila, you should have switched by now.

**Tim Beiko 1:03:59**

Yeah, I wouldn't be surprised if somebody just made the PR to open Sila. But the client is no longer officially maintained. I don't think there's anyone on the call who's still a maintainer of open Sila, who could like give more cutter. Yeah, and we can ask in all core devs after if there is someone who's like, willing to do that, but yeah, generally. Yeah, the client doesn't have a full time team working on it anymore. Any other questions, comments? I guess that was it then. Thanks, everybody. And I'll make a PR to the SIP for the difficulty-bomb and the Arrow Glacier spec today so that we have the updated numbers in them. Yeah, thanks a lot for joining.
...
https://github.com/sila-chain/pm/issues/396
...
https://blog.sila.org/2021/10/15/amphora-merge-milestone/

15:12:24 From Tim Beiko to Everyone:
https://github.com/sila-chain/pm/issues/397
