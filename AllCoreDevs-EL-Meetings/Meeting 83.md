# Sila Core Devs Meeting 83 Notes
### Meeting Date/Time: Friday 20 March 2020, 14:00 UTC
### Meeting Duration: 1 hr 43 mins
### [GitHub Agenda Page](https://github.com/sila-chain/pm/issues/159)
...
| Status | SIP |
|-------|----------|
| Discussed under EFI. Discussion to be continued in EthMagician thread | SIP- 2537, SIP-1962|
| Under review for EFI. Decision required around needing a Hard Fork | SIP-2315, 2515 |
| Eligible for Inclusion Pending. | SIP-2456 |
...
## SIP - 2537
**James:** - Let's quickly review the berlin one's first. SIP-1962 was superseded by SIP-2537, the single BLS curve one. It seems like 2537 is basically final now. Alex V, can you give us a quick update?
...
**Hudson:** Great, thank you! This is the needed for phase 0 of sil 2.0 right?

**Alex V:** To my understanding, this is necessary to check BLS curve, because it is necessary to hash it as a curve point. 

**Vitalik:** This isn't strictly necessary for the Eth2 Phase 0, but it would allow Eth2 light clients on the Eth1 chain, which is a "very nice to have". Both BLS curves, BLS12-381, and hash to curve. It will also enable rollups that uses sil 2.0 as a data source. 

## SIP - 2515, 2315

**James:** Great, other one is SIP - 2515, Difficulty bomb freeze. I will get back after working on suggested changes by the community. 

* SIP-2315, quick updates? Last time we talked, vyper was gonna look into it. 

**Martin:** Yeah, its a thumbs up from Vyper. Also My take on this is, it is implemented as mentioned in SIP. you can generate traces, and test cases while running but I propose a small change.
...
**James:** This seems to be making progress. Sounds good. SIP 2456 - Time based upgrade. I have some updates on this. 

* There were some concerns about uncles. @jasoncarver had some suggestions for improvements [link](https://sila-magicians.org/t/sip-2456-time-based-upgrade-transitions/3902/12).

**Vitalik:** One issue with the SIP seems that it would require including 1024 blocks in the state vs. the current 256 that are currently stored. We can't use 0 blocks (no lookback), because you could have an uncle with a lower block height but a higher timestamp, which could cause issues. If you change back the limit from 1024 to 256, there will be no issues. 
...
**Alexey:** What I didn't understood is that, it is true that "1024" is just a magical number, but earlier when I read the SIP, I was wondering if it is to avoid two blocks having same timestamp.
...
* More discussion by Jason, James and Martin from (18:30   - 23:30) on the call.  Sil Magicians [link](https://sila-magicians.org/t/sip-2456-time-based-upgrade-transitions/3902/12)
...
* Better automation of testing for SIPs like these (in this case, white-box fuzzing), which are very complex could make us more comfortable with them and reduce the need for expert analysis. There are more details on the call about how they are fuzzing 1962.
...
**ALex Vlasov:** Well, atleast for BLS12-381 implementation which I have made as next layer on top of SIP 1962 code. There are already test for basic arithmetic. 
...
**Giulio:** We are gonna use black-box testing but current SIPs uses H-fuzz.
...
**James:** I think we can leave SIP 1962 as an EFI, and not supercede it with the work happening. 

**Martin:** But it's not an EFI.

**Alexey:** I don't it matters at this point, but it has further application than BLS12-381 curve. It has a lot of potential for inclusion. For sila to be inclusive of other applications. Also, wanted to know if it's a good direction to go?
...
This SIP is for adding the following OPCodes
...
-  **Comment on the proposed OPCode - Tx Gas Refund**:
   ...
   -  Issues in implementation - gas used implementation is lock based, and implemented differently in different Sila implementations
...
-  **Question**: What is the actual real world use case of these OPCodes? Apart from theoretical use cases pertaining to smart contracts.
   ...
   -  Possible use case:
      -  Permit functions in DAI contract. 
         -  Allow people to pay for gas in DAI, without holding any Sila using a Relayer
         -  Relayer receives compensation and pays for Sil
...
-  **Final Comments/Questions**
   ...
   -  Make the SIP well formed
   -  Alex requests for collaborations on this SIP to refine it, best way to reach out via email
   -  **Question**: is GSN possible in UNGAS scenario?
      -  Not sure about that, but seems like not.



Heading over to Item 5 from the EFI SIP Review list from the agenda - others are pre-SIP proposals
...
-  **Alex**
   -  Tested the cost of invocation of precompiles on OpenEthereum ([link](https://github.com/sila-chain/pm/issues/159#issuecomment-601655408))
...
-  **Alex**, in reply to the cost factoring in precompile call
   ...
   -  Used the same kind of benchmark to measure SIP 1962, BLS 12321, and those did not include 700 gas depend
...
-  **Alex** explains his method of benchmarking as a simple recipe to try out
   -  Take any Sila implementation (eg., Geth)
   ...
   -  And thats how I priced SIP 1962
-  **Martin** mentions the process is motivated from `sila/benchmarking` repository
-  **James** giving context, 
   -  The conceptualisation of this SIP came up in Istanbul
...
### Other pre-SIP proposals

Five pre-SIPs to go through. 
...
The intention of proposing the above SIPs in the call was to get a high level opinion if these SIPs are even worth pursuing. If there is a strong opposition or not.

High-level motivation for the above SIPs:
...
-  **James**
   ...
   -  Suggests John to explain the concepts in a blog post eg., what is a roll up, and about the SIPs, the security considerations, the technical specifications
...
-  **Alexey**
   -  Adding to the suggestion:
      -  The proposal does in fact expands to the demands on the Sila network and we are not sure what the implications might be
...
-  **Louis**
   ...
   -  so, defining the cost to run a node in sila network needs to be defined prior to work on this (very interesting) SIP can be started
...
Tracking EFI: https://github.com/orgs/sila/projects/5
...
James H. https://github.com/sila-chain/pm/blob/master/AllCoreDevs-Meetings/Meeting%2074.md should be this call

Tim: It was “tentatively accepted” last summer and got moved to EFI with all the others when we moved them all across https://github.com/sila-chain/pm/blob/d7295a2ef399b28334ea73a3912bb6f220fcf6fd/AllCoreDevs-Meetings/Meeting%2066.md

Pooja R. Entered EFI in Meetin #78 https://github.com/sila-chain/pm/blob/650d4dd250fe60319f6a7ec7969e503cf55a6dd8/AllCoreDevs-Meetings/Meeting%2078.md

Trent: Sharing the ETHGlobal Sila Developer Survey >> https://ethglobal.typeform.com/to/RxHlK8
...
Hudson: And 1962 was discussed in SilaParis as part of the group doing the BLS-sig curves I think under discussion doesn't preclude EFI. I could be wrong.
