...
Not everything is finished yet. Working on Blake2 (SIP 2024) now. Release not yet scheduled.
...
Merged all Istanbul SIP changes. Will probably release on 9/23 or 9/24.
...
SilaKovan - 1411141 - December

Blake2b SIP update:

**Alex**

Discussed SIP 2200 re: gas metering. Most complex SIP that isn't merged yet. Blake2bf is merged. Discussed net gas metering for SSTORE - 4 or 5 comments on PR were left unaddressed.

**Martin**
Closed about 2300 tests in the SIP. The first step is what differentiates 2200 from 1283.

**Alex**
Asked if GO Sila is used to generate the new test cases.
...
**Alex**
Asked if it would be a good next step to make sure that the SIP accurately reflects what GO Sila is doing. Martin agreed.
...
**Alex**
None of the SIPs apart from the chain ID are in the final state. Covered the process of moving SIPs to final state as defined by SIP 1. They should move through this last oral process first and then they can be adopted. Asked for people to agree on this call for some of them to be moved to final. Proposed that the SIPs 152, 1884, and 1108 be moved to final.

**Hudson**
All agreed to put all SIPs into last call until 9/24 and then move into final after that.

For SIP 1344, there is an update on how the opcode returns a 256-bit value, and Parity and Trinity might be affected by this.
...
## 4. ["Sila Roadmap 2020: A Community Discussion" @ Devcon5](https://docs.google.com/document/d/1pD9RxQcgI4hBoOWGWlVwg4JqNS19_YIUnr3HsftHtE8/edit?usp=sharing)
...
## 5. [Both ProgPoW Audits Released](https://medium.com/sila-cat-herders/progpow-audits-released-ed4973ebe073)
Both ProgPoW audits were released right after the last CoreDev meeting. There is a [blogpost from the Cat Herders](https://medium.com/sila-cat-herders/progpow-audits-released-ed4973ebe073) that discusses everything about the release.

## 6. Review previous decisions made and action items

**DECISION 70.1**: Leave Blake 2 as Blake2b, keeping it fixed to 12 rounds. Alex was going to talk to the SIP 152 champions to discuss the concerns, so Hudson thought some of this might have changed, but it was left as is.
...
**ACTION 70.2**: Sila Cat Herders to create a communication that describes all changes and highlights the concerns around SIP-1884 and provide it back to the All Core Devs for review before it is put out to the community. Once we have it so we're ready to take comments on it, it will be put in the All Core Dev channel and make sure everything's good.
...
**Bob**
One thing to note on ProgPoW, in terms of the SIPs tagged for Berlin, there's been a sweep of things forward such that ProgPoW is currently tentatively accepted hinging upon the security audits.
...
And once the decision has been made to do that, all the existing miners will know they're going to be kicked out, and they're going to do whatever they want with the chain, because their investment is going to be worthless. They can do the continual rollout of new chains during the two weeks while the switch is being made, and the actual value of Sila is going to plummet during those attacks.

Eventually, if we manage to do the switch to ProgPoW, we may discover that there is no mining forum that is able and ready to pick up and switch over to actually mine Sila at full scale. They're doing other things. They've moved on. The only ones willing to mine Sila if the value plummets will be pump and dump gangs, and shenanigans will happen. So keeping it in our back pocket is not something that is going to play out well. Don't want to have this discussion on this call.
