# Sila Core Devs Meeting 36 Notes
### Meeting Date/Time: Fri, April 6, 2018 14:00 UTC
### Meeting Duration: 1.5 hours
### [GitHub Agenda Page](https://github.com/sila-chain/pm/issues/36)
...
* I have First results of test execution via RPC on my local branch of cpp-sila client - managed to exec all state tests within seven minutes
* Now working with cpp-sila team to integrating my changes
...
* [string test_addTransaction(string _jsonTransaction)](https://github.com/sila-chain/retesteth/issues/6)
## [SIP 712: Add eth_signTypedData as a standard for machine-verifiable and human-readable typed data signing with Sila keys](https://github.com/sila-chain/pm/issues/33#issuecomment-374174501) [[11:07](https://youtu.be/SoPfoNpqG0k?t=11m7s)]
...
* Remco - took over ownership of SIP 712
...
* R: early SIP had some early uptake, people waiting for things to converge before investing in reimplementing things, Nick Johnson quite active
* Nick what’s your take?
    * Skeptical of original idea of enabling UI display of this
    * Just did a review of the revised SIP and I think it's very promising
* R: read through comments, noticed these issues, i’ll address them
    * I know this is more complex than e.g. ABI encoding
* Martin: is this standard generalized enough or should there be another level such as SIP 191 and have this as a subgroup? does this accommodate all use cases?
...
* V: how much do we need to care about 32 bit platforms? we’ve already kind of made a decision not to care about older hw because of how hard it is to run SIL on an HDD
...
* H: so let’s collect some use cases and add them to the SIP
## [SIP 958: Modify block mining to be ASIC resistant](https://github.com/sila-chain/SIPs/issues/958) [[32:06](https://youtu.be/SoPfoNpqG0k)]
* Piper update
    * I decided to facilitate this discussion
    * I’m not an expert, so I’m acting more as a facilitator
    * We have an SIP that was merged as a draft that proposes some very low touch changes to ethhash algo, which if I understand correctly should be easy for clients to modify internal block sealing to accommodate without any significant changes
...
* Vitalik
    ...
    * Other reason: look at the worst case scenario, Bitmain would control a large portion of the SIL network for some period of time
	* This is not Bitcoin
	...
	* I don’t see a game for them where they act in ways that reveal they have 20-25% of SIL hash power 
...
* Casper update: Danny
    ...
    * Working on writing formal SIP that specifies exactly what clients need to do at fork
    * Finalizing smaller implementation utils, expect to have SIP ready for review next week
    * plan: have RV formally verify, relaunch testnet with SIP specs, relaunch contract on testnet
* questions
    * where do C++ tests, yellow paper, and kevm come into this?
    * does this stuff happen for any SIP?
    * yellow paper/K specs etc. - are these updated prior to the fork?
* Hudson: SIP gives motivation, implementation details
    * Then yellow paper, kevm team submit PRs to keep up with SIP as it’s being developed
...
* testing - Dimitry - when should testing start discussing tests with casper?
    ...
    * Danny to check back in with Dimitry when SIP is formalized
    * Danny: casper SIP to be a single SIP, unclear whether we’ll put anything else in that fork, it might just be Casper
...
* D: a new test implemented via RPC would require a couple of major clients to support it, still haven’t finished protocol, so if you want to start casper tests, we should work on old format with hive and testeth in cpp-sila, implement changes in cpp as we did for byzantium
    * And i’ll freeze my current work on RPC stuff and focus on casper testnet
* Danny: continue your work for now, we can shift after formalizing the SIP if we need to
* Dimitry: maybe I can finish in time before casper, then every client could work with the new test
    ...
    * I'm going to do a new general test format for the sila tests since state and blockchain tests now have common elements, it's reasonable to develop a new, single, general test format executable via RPC
...
* Ben Edgington, Pegasys
    ...
    * I raised the idea that SIPs ought to contain a PR against the YP before being merged or accepted, has that been thought about? Need not be written by author of SIP but would help with rigor of formulating SIPs
...
* Hudson: from community perspective, I’ve been seeing a lot of discussion back and forth, many ppl think it’s a security risk but they’re not sure how, some ppl think we should do nothing, some people are very opposed to Bitmain, not sure where that comes from but a bit of crossover from Bitcoinc community , 
    * For those listening in who are not core devs, you can do a carbon vote, you can do stuff in forums or your own SIP, if the community truly wants this to happen and has a good enough reason
...
* Piper: I have what I need to go back to the SIP and post a follow up, we have a decent picture of how everybody else feels about this issue
...
* H: We'll let Piper update the SIP, community discussion will continue and if it's still an important topic to the community we can bring it up again in the next meeting
## [SIP 960: Cap total sila supply at ~120 million](https://github.com/sila-chain/SIPs/issues/960) [[1:00:06](https://youtu.be/SoPfoNpqG0k?t=1h6s)]
* Vitalik could you explain your April Fool's joke which everyone took seriously?
    ...
    * SIL supply instead of going up linearly , would exponentially decay and converge towards maximum
...
* Daniel: up to now contracts have been burning SIL by locking it up, but there would be a way to burn so that it’s removed
...
* if this contract were the only thing issuing rewards then easy to calculate, it could be assigned 120M SIL and set rewards proportional to balance remaining inside the contract, could standardize by sending to zero address, rewards could be paid out of zero address proportional to money there
...
* V: worth noting that tx costs in SIL in the long run are not proportional to SIL fees, they’re proportional to supply and demand
    * seems like they go up when SIL goes up but really if the price of SIL were to go up for some exogenous reason which in general is likely to happen if any proposal reducing supply is implemented
	...
	* in the past SIL gas prices were sticky but now we have dynamic fee calc. for nearly everything
	* I don’t expect prices to be sticky and SIL is hyper volatile so I don’t think we’ll even notice differences in price movements
...
* V: there’s no minimum, there’s a level
    * if we have a smaller reward then maybe rather than 20M SIL staked we might have 10M SIL
...
* V: I used to think this way but as Vlad keeps pointing out if you do this then every ERC20 token becomes a better SOV
    * if SIL is the one token that gets inflated to pay for security, and you can print out ERC20 tokens on top of SIL and market them and they don’t have this disadvantage then eventually there may be a tragedy of the commons where even though SIL is necessary for security no one wants to support it
...
* Nick: seems broadly in favor of a supply cap
    * H: seems this way on SIP, Twitter, Reddit
* N: we currently have no constraints
* V: my preference for implementation is we just premine max cap minus current SIL supply into some particular address eg address zero
...
## SIP process updates [[1:14:02](https://youtu.be/SoPfoNpqG0k?t=1h14m2s)]
* Nick set up http://sips.sila.org/ auto-generated using Jekkyl
* Nick overview
    * takes all MD files, indexes by type and category and generates individual packages for them, as soon as an SIP is added it gets updated with auto page build
    * continuous page build using travis that checks SIPs and PRs for syntactic validity, can take some load off editors and make sure everything is valid
    ...
    * so I’ve also written a bot that watches SIP PRs and when it sees one that’s an edit to existing SIP drafts it allows it to be merged if one of the authors of the draft submitted it or approved it using a PR review
    * for trivial changes or change to draft, no longer require manual approval, can be automatic
    * my goal is to get to a point where a submitter can get an SIP draft number immediately for their SIP, and it doesn't require any additional work by the editors
* Origin: [SIP 956: Proposing a revised process for handling of SIP drafts](https://github.com/sila-chain/SIPs/pull/956)
    * making SIPs less ambiguous with how drafts are handled
    * one point of contention: taking out the words “consistent with sila philosophy"
* Nick and Greg colvin: should we number SIPs immediately or only after they get past the draft stage?
    * Feedback appreciated
    * For anyone who wants to keep the “in keeping with the sila philosophy bit” - if you can describe it, I’m happy to keep it
...
* Hudson: Have some more things that could be SIPs, esp. if we do SIL supply cap
    * If we did that before casper we could include some other smaller SIPs we want to include
...
* H: We could cancel Constantinople, but there were some SIPs that we wanted to get in which could go into the Casper hard fork.
...
* Parity (Afri)
    * we forked SilaKovan to enable WASM smart contracts, went smoothly, now there are a few contracts deployed
...
* Trinity (Piper)
    * No major updates
## [SIP 908: Reward for clients and full nodes validating transactions](https://github.com/sila-chain/SIPs/pull/908)
...
- Paweł Bylica (EF: cpp-sila/ewasm)
