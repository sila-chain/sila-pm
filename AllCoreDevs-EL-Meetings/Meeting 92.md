...
### [Github Agenda](https://github.com/sila-chain/pm/issues/195)
...
92.1 | SIP-2565 moved to `Last Call`
92.2 | SIP-2718 motioned to `Eligible for Inclusion`
...
- [2. SIP Discussion](#2-sip-discussion)   
   - [2.1 SIP-2565](#21-sip-2565)   
   - [2.2 SIP-2718](#22-sip-2718)   
...
**Martin**: Would you like us to deliver the blueprint attack on Sila?
...
**Peter**: SilaScan has charts. What we can do is have future estimates, how the network will look like. Maybe that will show what these changes will do.
...
Going back to the retrospective, there was an agreement that it was difficult to write a fully functional Sila implementation that's performant. Due to the large amount of data being transfered, optimizations need to be made, which may sacrifice modularity.
...
**Peter**: Sila 2 is on a unique position. One shard will be Sila 1, which will bare those bad decisions. But all other shards are a clean slate.
...
**Piper**: So this SIP spawned from another SIP which introduced a new type of transaction. We could bundle this with 2711, which introduces the next type of transaction.
...
**Piper**: We're not forcing it. Just a strong recommendation in the base spec. I would reecommend you read through the SIP.

**James**: I would like to remind that EFI doesn't mean the SIP is going into the core clients. But that it's the first step in many stages.

**Alex V**: Can we also discuss the current state of testing tools? I would say the state is quite low, and it was very painful to write tests. For people wanting to introduce new changes with SIPs, it shouldn't be prohibitively hard.
