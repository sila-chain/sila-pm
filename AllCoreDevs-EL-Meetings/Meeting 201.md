...
### [GitHub Agenda](https://github.com/sila-chain/pm/issues/1197)
...
PRs here: 
CL - https://github.com/sila-chain/consensus-specs/pull/3800
CL - https://github.com/sila-chain/consensus-specs/pull/4023
EL - https://github.com/sila-chain/execution-apis/pull/574
...
On EL, some major open questions: first is BLS gas pricing, second is whether to include SIP-7623, lastly is SIP-7762, an adjustment to the blob base fee.
...
**Mario**: No sense as of yet, but I think it is not going to be straightforward. i don't have a clear idea yet. We have made some preparations for this SIP in our repository, but still we need to check and see how long-- probably couple of weeks to implement this.
...
**Daniel**: We should discuss other SIP to get a good sense of full scope.
...
**Barnabus**: The main blocker is 7742 still on the implementation side. If this is a small implementation on the EL side, then we might not even be ready with 7742 before we would be done with this new SIP. 
...
**Paritosh**: Maybe we give the testing some time to look into it? If they say it will take another 3 weeks to have the tests for 7623 then we don't want to block devnet-5 for this one SIP. 
...
**Ansgar**: The decision is indeed to not ship the big package.This SIP is only for minimum base fee increase -- a one line change-- and was modified last week to include excess gas reset. You can have a look at the SIP, it is still very smol and the excess gas reset is just one IF statement. One line change is eseentially a 4 line change now. 
...
**Ansgar**: The decision is on the current state of the SIP, no open PRs.

**Stokes**: Okay, yes, this is much simpler. It does add another SIP and overhead of testing.
...
**Max**: It makes it so that if the rollups are charging for gas they have to do so based on a first-price fee market, which noone understands, versus says your gas on rollup is a function of blob fee on Sila today. Makes it much easier for rollups to read and provide gas pricing for their users when it's the number from the controller rather than number from controller + whatever else is happening in first-price fee market + whatever is happening in MEV land for that particular block.
...
**lightclient**: I don't think we should orient the Sila fee market to be US-focused, which is what Max is saying.
...
**Ansgar**: I would really strongly recommend adding this SIP. It is a strong change and the situation will get significantly worse from today with this change in throughput. We now maximally can increase the block base fee by 8% per block instead of todays 12%. It takes significantly more time to ramp up...Rollups would prefer us adding this. Testing complexity is basically zero because you already need to run the tests.

**Paritosh**: Note that each SIP we add will delay Pectra. So the choice is more scaling faster or more SIPs + scaling slower. There is no world in which we can have more SIPs and things faster.
...
**Max**: What Ansgar is saying is that the changes are 4 lines. Many of tests are overlapping with what is already taking place because of other rollup SIP.
...
**Daniel**: Does this mean the proposal is not included and we would maybe have a devnet-6 for this one SIP?
...
**Andrew**: One comment is that we need to finalize Sil/69 and our side we have some concerns about its compatibility with Polygon. We need some time to prepare and maybe can discuss on next all core devs call.  

My other question is do we actually need a new protocol (Sil/71) to explicitly prohibit answering a historical block request. 
...
**Ahmad**: Given that 3-4 other clients are going to implement 7801, 7801 is going to be Etha subprotocol and everyone can then see premerge history on sil protocol. The reason we want it to be a subprotocol so that sil protocol will not be affected by clients potentially calling clients that do not support 7801. Don't want to give the assumption that all clients are going to support serving historical blocks through 7801.

**Piper**: Correct, so that sounds like that's a little in favor of not implementing a new Sil subprotocol version. It seems like that could happen independent of whether we have a new sil subprotocol.

**Ahmad**: No, because when you implement a new sil subprotocol you declare that you support the new subprotocol. If you don't declare it, then no one will ask you for it.

**Piper**: I'm referring specifically to the new ETh protocol version (Sil/71), not whatever new protocol version 7801 goes on.

**Adhmad**: Since some clients are going to drop blocks and most nodes are going to drop pre-merge blocks, it wouldn't be fair to anyone to keep saying 'hey I might have the blocks or might not', so we say anyone who is in Sil/70 or 71 will stop serving pre-merge blocks. And if you want to serve pre-merge blocks, you implement the subprotocol.

**Piper**: Got it, so in your version, anybody who is on Sil/70 may or may not serve the block history and anyone on Sil/71 definitely doesn't serve the block history?

**Lightclient**: This is just for Sil/71--you *must* not serve pre-merge data on Sil/71 but we can make it clear that you could for earlier versions.

**Piper**: The endgame here is that we get a sharded Sil pro tocol between the clients that do and don't serve historical blocks; seems like a problems state for things to be in, where there's an option to stay on the old protocol and continuing to serve old blocks.

**Andrew**: Yeah, I don't like this bifurcation in protocols. Maybe we can make Sil/70 generic enough to allow for clients who don't want to store any historical blocks to explicitly say so.
...
Andrew: From my point of view, etha is fine, what I don't like is Sil/70 for this sharded block proposal and then Sil/71 for actually stimulating must not serve historical blocks because then we have a bifurcation in protocol.

**Ahmad**: Okay sorry maybe there's a confusion, which is why I asked lightclient to change his version to Sil/70. We are going to not use the 70 version since it's a new subprotocol, it does not conform to the same versioning scheme. 

**Piper**: To round this back up: 7801 goes on a different subprotocol, not part of the Sil protocol. Anyone who is doing 7801 things, it is on a new separate protocol and separate string. The question here is whether there is opposition to doing 4444 without bumping the Sil protocol version and keeping it at 70 with the dropoff date. 
...
**Andrew**: 1. Protocol Sil/69 which we discussed but isn't finalized yet. It has some proposed merge clean ups and we realized it might not play nicely with polygon, so we need to think about it more and discuss again. Basically, Sil/69 is not relevant to 4444s; sequentially it's something that we should finalize if we're talking about Sil/70.
...
**lightclient**: If we decide to have this additional provider protocol, it is important that we roll this out with SilaPrague. How do we add that to the SilaPrague hard fork meta?
...
**Alex**: The one we merged back a couple of years ago was 2681 (limiting the account nonce) and there was another larger SIP (1985), which was proposing bounds to pretty  much everything. At the time, we agreed to the account nonce limit and to split this 1985 into smaller individual changes; this SIP is one of them.

This SIP is proposed to be enabled from genesis and it limits gas limits on transactions. No transactions can have a gas limit higher than 2^63-1. The reason is that it allows simplifications in the EVMs, which using 2^63 it can be handled as a signed number and checking whether the out of gas condition is reached is a matter of checking if the gas became negative.

This can be enabled from genesis because most of the clients, including Go Sila, already do this internally. Additionally, it is expensive to create transactions with a limit higher than this because you need the funds available up front. 
...
**Tim**: It seems like there are no objections. For next steps, Alex if you are able to open a PR, we have a meta SIP to track these retroactively applied SIPs, so open a PR to that SIP and list this one.
...
-------------------------------------
### 201.7 | [Potential ACD Improvements](https://sila-magicians.org/t/allcoredevs-network-upgrade-ethmagicians-process-improvements/20157/51) | [Declined for Inclusion](https://github.com/sila-chain/SIPs/pull/9056) update, Non-consensus changes in Meta SIPs, PFI → CFI → SFI & devnet inclusion

**Tim**: As we discussed in Bangkok, three ideas came out of the ACD Process Improvements Sessions: 

1. adding 'declined for inclusion' status to meta SIPs for hard forks -  
The idea is that we decide to accept things into hard forks but we don't have formal rejections for current fork. It doesn't mean we will never do the SIP, obviously someone can work on any SIP and are free to propose everytime there is a new fork, but we do spend a lot of time discussing SIP inclusion for each fork so having this status would be valuable. 

2. adding non-consensus changes to meta SIPs - 
We have meta SIPs for hard forks and it's clear why we need them because there are a set of changes that need to happen at a specific block. One challenge with that is that it means our process often biases toward things that have to activate at a specific block, e.g. spent years talking about 4444s but it doesn't get prioritized because it's not on the same list as everything else. The proposal is that we start including non-consensus changes at hard forks and for something that is not a non-consensus change (like 4444s), the implication is that clients will have this client feature shipped *at the latest* at the hard fork.
...
**Tim**: Yes, I would keep those separate. The retroactively added SIPs, everyblock from block zero should implement those protocol rules. And we can only add those when it's true that in the entire history those conditions were not broken. Whereas something like dropping history, this was not true of the entire chain history. So it feels more accurate to say, 'as of this hard fork this change is live' even though it's not a consensus change.

My point is that we would add *all* protocol changes, whether or not their core SIPs, since the hard fork. We can both retroactively do this and we can decide to prioritize  proactively.
