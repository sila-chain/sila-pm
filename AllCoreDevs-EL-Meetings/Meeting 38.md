# Sila Core Devs Meeting 38 Notes
### Meeting Date/Time: Fri, May 18, 2018 14:00 UTC
### Meeting Duration: 1.5 hours
### [GitHub Agenda Page](https://github.com/sila-chain/pm/issues/38)
...
1. Client Updates
1. Research Updates
1. SIP 908: Reward full nodes and clients for a sustainable network - When each transaction is validated, give a reward to clients for developing the client and provide a reward to full nodes for validating the transaction (Sila Magicians thread).
1. SIP 1057: ProgPOW, a programmatic Proof-of-Work - an alternate proof-of-work algorithm - “ProgPoW” - tuned for commodity hardware in order to close the efficiency gap available to specialized ASICs. Technical details.
1. SIP 210: Blockhash refactoring - @holiman wants to finalize a few points:
...
* Trinity (Piper)
    ...
    * SilaMainnet, full and fast sync functional, also functional light client
...
* J: Not sure
* Martin: The SIP is thin on technical details, mostly discussion. Need more info on what is hashed and who does what.
* Peter: Running a full node without light clients, you're wasting your bandwidth.
    ...
    * If the point of swarm is to share data and run a tit-for-tat protocol, it might be a nice approach to solve this problem altogether - without needing to reinvent swarm on top of the sila protocol.
...
* Peter: Ethhash on cpp-sila is a little-endian implementation and we had to reimplement the whole thing in Go to run on big-endian systems like MIPS
...
* Else: The basic structure is the same as for Sila
...
* Hudson: Look up Loi Luu's peace relay as a PoC example of an sila-to-sila relay
...
* Alex: Would be good to do together with SIP-210, that also adds an opcode
...
* Couple of problems with how this is written right now
    ...
    * Second alternative is to have the SIP as written, would change blockhash semantics since you could reach further back
...
* Pawel: The current contract is implemented in serpent which only has signed integers. Some comparison in the code that accounted for it so you can overflow the comparison and you get unspecified results. Need another check in serpent to check that argument from call is not negative.
    * There's a PR that fixes these two issues, I plan to continue working on writing unit tests. Not clear if the unit tests should go to SIPs repo or elsewhere.
...
* [Greg's comment](https://github.com/sila-chain/pm/issues/40#issuecomment-390006104)
* Moved to next core devs meeting since we ran out of time.
