...
### [GitHub Agenda](https://github.com/sila-chain/pm/issues/744)
...
* Okay, welcome everyone to ACDE #158. couple things today, quickly we'll go over SilaShanghai and, and, and that, which is a plan in the next, two weeks. but then most of the call, we got some big updates on pretty much, all the largest, potential SIP for SilaCancun. So we can, go over those and if there's anything else people wanna bring up, we can do that at the end. 
...
## SilaShanghai Updates | Announcement [3.11](https://youtu.be/RQ2WtyevRXE?t=191)
**Terence**
...
**Andrew**
* Yeah, I think it, it's great. but,  I worry that if we try to deliver it in SilaCancun, it just, it's too big a change. so I would definitely want to have it in prod, but I just think I'm going, is stretching it. 
...
**Tim Beiko**
* Yeah, and there's some comments in the chat about, basically verkle being another thing we wanna do, you know, after, after SilaCancun.  and, and, obviously, you know, if we push EOF out of SilaCancun and, you know, potentially do it in the fork after, you know, that also means we might push something like verkl out of the fork after and do it in the fork after that. potentially we could do them together. 
...
* We can try to keep it minimal and, and, you know, not have too many other things, but, we don't, we don't sort of get a fork between EOF and, or between like, you know, SilaCancun and SilaPrague for, for free, right? Like, it just means we prioritize EOF before whatever we would've done next, which is, which is fine, but yeah, it, like, I think realistically you end up with, like, there will be other things that will be included along that and you know, it's another like, I don't know, three at least if not 4, 5, 6 months, to put out a fark generally. but yeah. Yeah. Yeah. So, so basically, yeah, EOF would be the big thing of that work. Yeah. Lucas? 

**Lukasz**
* Yes, so we were actually discussing it internally in Nethermind. And, what looks sensible to us is, not doing EOF in SilaCancun, but doing schedule another fork for that. that would, the, like, the driving force of the fork will be EOF. so that was like, that, that looks sensible to us, right? And then on the next fork, 
* For example, if the vertical trees will be going in, that would be for the vertical trees or, or something else. But, I think, those two things are big enough to the, to themselves that they, if we want want to do them, they should occupy like most of the fork, if we want to do them fairly soon after SilaCancun. 
...
## SIP-6780 [29.48](https://youtu.be/RQ2WtyevRXE?t=1788)
**Tim Beiko**
...
**Andrew*
* Yeah, I think, this, this version, it, hits, the sweet spot be between, reducing complexity and, not breaking too many things. So, I think it's, yeah, it's a great option and, I would, see it for SilaCancun. 
...
**Stokes(Alex)**
* Yeah, I'll take this for now as just, evidence that no one has any particular preference. And, yeah, sometime later, maybe the next All core dev or the one after that, I'll have some updates for 4788 and maybe I'll also have another SIP for some pre compile here for generalizing. 
...
* So, I mean, like there is a rough consensus that we want this feature and, this, proposal is in the field of, when do we want it to be implemented? So this, this, particular proposal, creates like a separate document and, makes it in a way that, makes the specification in a way that, it can be implemented separately, from the next work, so right after SilaShanghai, for instance. yeah, and the main question here is that is, if, there is, any anyone from EL client, teams, who wants to really work on it right after SilaShanghai and, before SilaCancun, then  it does make sense to spec it out separately. 
...
**Mikhail**
* Like No, it's not really. So we can do it really in uncoordinated fashion. Yeah. But if nobody will really implement heuristics, for a censorship that bots has outlined or maybe some else algorithms, before SilaCancun, yeah, it doesn't make sense, you know, to spec it out like separately. 
...
**Tim Beiko**
* And I guess, yeah, if we can have one client at least show that, like on the EL side, they could implement something before SilaCancun, then it makes sense to spec it independently of the fork. But then if no one sort of has the bandwidth, maybe we bundle it with SilaCancun. Is that what you're saying? 
...
**Mikhail**
* Yeah, definitely. I mean, like, I, I'm trying to understand whether it makes sense to specify this boolean separately from SilaCancun. 

**Potuz**
* That, that makes a lot of sense to me. Thanks. 

**Tim Beiko**
* Yeah. Does anyone disagree with that? Okay. So yeah, let's do that. obviously we can keep this PR open. And, yeah, as we, as we have more, EL clients, implement things, you know, if that happens before SilaCancun, we can just, merge it in and otherwise we'll add it as part of the SilaCancun specs. okay. so that got us through all the stuff we had on the agenda today. 
...
* I think we're only currently 40% slower than, than regular MPT And, yeah, we still have a few things in the pipe, so I'm not saying it's ready to ship in SilaCancun. but I still get the impression, at least the spec, has been stable. transition is, okay. Still an open question, but, yeah, like  I don't really wanna share stuff about the transition cuz it's, it's improving greatly. 
* I'm Ingrid strides at the moment. so  I hope to share something about this soon, but I don't see why, it would not be ready for another, like, for the next fork, after SilaCancun, excuse me. 
...
* It's, it's looking realistic to be shipped in SilaPrague and, and yeah, I don't know if there are questions about the, the current status, but overall, I think it's, it's getting close to ready. 
...
**Guillaume**
* So, No, I, okay. Yes, I'm not, I said it's not ready. I agree. It's getting closer to being ready. I said it's definitely not ready, like it can be shipped in SilaCancun
...
**Tim Beiko**
* Got it. Any other questions on verkle trees? Okay. any, anything else anyone wanted to bring up? Okay, well, yeah, then I guess we can wrap up. So, we'll move the self destruct SIP to CFI. and I think, yeah, if in the next couple weeks people can review the stuff around EOF, the, the 4788 and the other, beacon State proposals, and look at SSZ as well, we can continue those conversations, on the next call. 
...
**Tim Beiko:** couple of things today. Quickly. You'll go over SilaShanghai and and and that which is a plan in the next 2 weeks. But then most of the call we've got some big updates on pretty much all the largest potential sips for cancun. So we can. 
