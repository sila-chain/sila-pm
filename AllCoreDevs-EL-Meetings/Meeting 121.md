# Sila Core Devs Meeting #121 Notes
### Meeting Date/Time: 
### Meeting Duration: 90 minutes
### [GitHub Agenda](https://github.com/sila-chain/pm/issues/379)
...
* Merge Call before Sil-2.0 call next week for one hour
* Discussed post merge syncing and reorg processing 
* Discussed SIP-3756 A global gas cap
* Discussed moving two previous SIPs for Sil 64, and Sil 65 to final status because they are in use and needed for Sil 66
...
the SIP here is the question from Martin what if the parent block state is missing some error type will that be defined this document like has  
...
validate if you end from the SIP it's easy it's sent to the execution client by the consensus client when the when the beacon block gets  
...
choice updated there is the PR to the to the SIP I'll drop it into the chat that unifies the two previous events which was the chain  
headset and the block finalized into the one so this document is matched the follows the SIP currently or vice versa anyway here is the  
...
as well so there is one out of this unification there is one note here this is more for consensus client developers in the SIP this event  
...
identifies the block so we can we can go to the I guess we can go to the Eth2 section now so in the from the sil I describe this thing in two  
...
of the sync so that's kind of it from the Eth2 point of view and now we can go to the sil one are there any questions at this point 
...
issue is as follows so in the in the current sila one name main network what the there is an assumption in in the clients especially  
...
implemented by everyone in some way but we have to like this I think it's more a matter of you know like agreeing among the sil one clients how we're gonna do this 
and so it's important yeah for you guys to basically check it and and think if it makes sense for you or 

# SIP-3756: Gas Limit Cap 
...
and the last kind of big thing we had on the agenda which apologies we'll probably have to do a bit quicker and we can also discuss again that a future call is SIP-3756 
...
while it was originally a part of 1559 and then removed and then in march of this year there was sip-3382 the proposed hard code the gas limit and I think that 3382 failed for 
...
you know building on top of that SIP the next the next plausible solution would be to just have a upper bound of the gas limit and that's what three five three seven five 
...
and this allows them to to bypass the SIP process in all core dubs process and in making decisions about the protocol that could negatively affect the decentralization security of it 

**Tim** 

right and one bit of context I think I would add is when we had the discussion around I forget the number but the previous SIP they capped the gas limit 
...
sila becomes more and more valuable so yeah I I think the the kind of reasoning that we had around like well miners have always been 
...
yeah well my question is somewhere like for consistency of this SIP which was proposed in a very short form without any any estimates on 
...
I I'm just curious and more consistency maybe it's just not in the SIP yet but there is already some analysis would be great to see it 
...
oh I was just going to say briefly you know we can add more to the SIP I think there's a lot of benchmarks that have been done generally and we can we can add more things to it it was just something to propose the idea quickly 
...
would we consider this SIP to basically be included in the ice age fork because otherwise it seems to me to not like like this would be the only circumstances 
...
merge and second is that currently the fees are very high on sila so if the guest if the miners or the validators raise the 
...
I think Pooja you had put a couple SIPs on the agenda SIP 2364 and 2464 which are basically the sil 64 and sil 65 protocol SIPs as I understand that the issue is like both of those are shipped but the SIPs are still like in draft right 

**Pooja** 

right so the main issue here is SIP-2481 which is for Sil 66. that is in the last call actually the last call duration has also passed and we would want to move that to the final status 
but the problem is that proposal requires Sil 65 which is 2464 and Sil 65 requires Sil 64 which is two three six four and both these proposals are still in draft status so we would want that these two 
proposals should move to the final before we could make move SIP 2481 to the final status I'm happy to make the request to request a status change it's just that we wanted to make sure that it 
...
to do something like this there was like a huge amount of backlash for some reason because then people came on and you know like wanted to actually see some justification for these SIPs even 
though they are like four years old or something I don't really know like this is definitely something that I would like to avoid so I feel like these SIPs they have been you know we don't even 
use Sil 64 anymore it's already like past you know it's basically it's already happened like it we there is really no reason not to move it to the final because 
...
okay we have some SIP editors on the call if they have you I just have pasted like a link to the earlier pull request 
which we created for SIP 2481 where we received some comments from SIP editors mentioning that these two proposals should be moved so if we can directly make it to the final happy to do that 
...
cool and okay so last thing and that was it in terms of content but next friday support for 1559 yeah so if you are kind of an application or wallet or just generally interested in kind of broad adoption for sip-1559 you can join that we'll post the link 
in all core dev and there's an issue on the sila pm repo for it and yeah that's pretty much all we have thanks everybody I appreciate you staying real 
...
* [Agenda](https://github.com/sila-chain/pm/issues/384) 
