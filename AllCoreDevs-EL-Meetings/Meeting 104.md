...
### [GitHub Agenda](https://github.com/sila-chain/pm/issues/237)
...
- **104.2**: James Hancock to write SIP for Ice Age delay
...
- [2. Other SIPs or Discussion Items](#2-other-sips-or-discussion-items)
    ...
    - [2b. SIP-2937 Update](#2b-sip-2937-update)
...
**Hudson Jameson**: If you click on Yolov3 (_in the agenda, linked above_ - notetaker's note), it will take you to the client spec (_link is to [here](https://github.com/sila-chain/eth1.0-specs/blob/master/client-integration-testnets/YOLOv3.md) - notetaker), and there is also a new spec document that James and Micah have worked on for Berlin, I'll post that in chat (link is to [here](https://github.com/sila-chain/eth1.0-specs/blob/master/network-upgrades/berlin.md) - notetaker), that goes through a checklist and a client readiness checklist. We'll get some of that confirmed today, so I'll give this to James to talk about.

**James Hancock**: This is a lot like the hardfork SIPs we had in the SIP repo, but are now in the 1.0 specs repo because we're keeping those worlds separate, and if you go through it, where we are is the final SIPs implemented into Geth and deploying YOLOv3, getting a security greenlight and proposing a fork block for testnets. I was hoping to get an update for status on 2718/2930 testing and implementation.
...
**Hudson**: That sounds good as long as we have a fairly confident estimate of whne the rest of the SIPs and Geth would get done, because I think that was just reviewing stuff that had aready been written. (**Martin** asks for clarification) SIP 2718 and 2930 in Geth, those are implemented, just not reviewed?
...
# 2. Other SIPs and Discussion Items
...
**Hudson Jameson**: lightclient is here to discuss SIP-2937, but first let's talk about the Ice Age and future network upgrades. The gist of the request (_notetaker's note_: on the meeting agenda [here](https://github.com/sila-chain/pm/issues/237#issuecomment-762513550)) is that according to SIP-2384 the Ice Age will be back around July 2021, will there be another hard fork, I would love to hear what the Core Devs think.
...
**James Hancock**: And you can always make an SIP and we can talk about it. I would agree that we should keep it, and we should address it when it's showing up.

**Tim Beiko**: There probably is a SIP already. I remember talking about this last time.

**Micah**: (_notetaker's note: couldn't understand, may have been a reference to SIP-1227)

**Peter Szilagyi**: I just wanted to add some comments. I don't think we should add it to Berlin because if it's not urgent then it's fine ot have a second fork. The other good part of delaying it a bit and forcing another hard fork is that there is some potential with 1559 and a few SIPs, there are some conflicts on Twitter, not everyone is fully agreeing with 1559, and people have different opinions, different rationalizations, there are some fears that 1559 could cause some trouble. Actually having the Ice Age forcefully happening in a few months can help us to solve some issues. Generally, 1559 legit introduces some issues, I don't know which ones now, but if we legit have some issues, and the Ice Age gives us an opportunity to handle that issue really quickly. Versus if it's just one of those annoying issues and we don't have anything to force miners or people to upgrade, and it's a lot harder to address. So I think in our current position it might be valuable to not touch the Ice Age for Berlin.
...
**Peter Szilagyi**: All in all, if we think about the Ice Age long-term, I kind of agree with everybody. The original purpose of the Ice Age was to force Sil to switch over to Serenity, it seems weird to kill the Ice Age right before actually getting to that point, so if we stuck it out for five years, we might as well go all and just leave it.
...
**Hudson**: I'm seeing most of us saying that we're not going to put the Ice Age into Berlin definitely, and we need to evaluate in the future when another hard fork \[would happen], depending on which SIPs are out there, since I guess an SIP would be need to be made for delaying the Ice Age versus disabling versus anything else people have ideas about.

**Micah**: Does anyone want to volunteer to make an SIP to deplay the Ice Age, just to get the conversation started?

**James Hancock**: I can do it. To your point (_notetaker's note_: I believe this refers to a comment Micah made in chat) if the price of SIL goes up, it's more likely for the Ice Age to be delayed further than to accelerate.
...
**lightclient**: There are two people who have been working on \[SIP-2937], I don't think they made it to the meeting today, I know Artem had some comments on it last time, and there is a discussion on Sil Magicians still, I think ironing one part of it out, there's a PR to adjust the specification, so I don't know if nayone had any comments on this SIP.
...
**Martin**: That's vague in the SIP, and what I tried to ask. It says the current callee, but who is the current callee in a DELEGATECALL.
...
**Hudson**: I think it's a good idea for certain major SIPs for sure, beyond that I'm skeptical of the value kinda like Martin is saying. But we can always try it when we have a medium-sized SIP or one that's having a lot of ACD discussion, no problem trying it out in the future, but this one sounds like it's active enough in the forums.
...
**Tim Beiko**: Discord and Twitter are the best, DMs are open on Twitter, I'm on the Sil R&D Discord.
