# Sila Core Devs Meeting 34 Notes
### Meeting Date/Time: Fri, February 23, 2018 14:00 UTC
### Meeting Duration: 1.5 hours
### [GitHub Agenda Page](https://github.com/sila-chain/pm/issues/32)
### [Audio/Video of the meeting](https://www.youtube.com/watch?v=GhUtruRZOlo)
### [Reddit thread](https://www.reddit.com/r/sila/comments/7zpxe3/notes_from_ethereum_core_devs_meeting_34_22318/)

# Agenda
1. SIP process improvements
...
## SIP process improvements [[3:59](https://youtu.be/GhUtruRZOlo?t=3m59s)]
- Want a channel where it's easy for people to communicate about what changes they want to happen
- There is a group called Fellowship of Sila Magicians (Hudson to post link), self-organizing group to maximize technical opportunities
  - Like IETF for Sila, self-organized group of engineers, scientists, technologists who "keep things running" - make sure all of our equipment speaks the same protocol, etc.
...
- Nick: We need to distinguish between writing a standard and adopting it
  ...
  - Firm up the process of what is considered merged, and the technical requirements of SIP editors
  - Make it clear to everyone involved that SIP writing is a technical process, and that if something is merged it does not mean that it will ever be adopted
...
- Greg: we don't have a good way to keep technical issues separate from policy issues
  ...
  - There should not be huge debates about whether this SIP should be assigned a number, it should not be so contentious
...
- Avsa: not merging things until all objections addressed? This is very hard, same objection can be made in different language
  - We could add an obligatory section on every SIP where you list every main objection and address each one somehow
  ...
  - Greg response: things can become a draft, drafts don't have an SIP number. Once it's a draft it can be accepted, deferred, rejected, etc.
...
- Vitalik: In general I dislike allowing coin votes for anything
  - Random people who happen to have a lot of SIL are not necessarily the people who have strong technical opinions
  ...
  - Tim: as Sila becomes more popular you'll get more random feature requests
  - Vitalik: decision markets are hard because choosing a single objective function is hard--I don't believe it's SIL price
...
- Vitalik: In general, our governance mechanism is really not that bad, the main flaw is not so much in the mechanism itself but in how we communicate them
  - E.g. re: standardized SIL recovery, the impression that a lot of community members got from the outside is that if it's merged, it's a lot closer to being accepted than anyone involved in the decision-making process intended to signal that it is
...
- Nick: Some people suggest we should not formalize governance because then it will be vulnerability to exploit capture, etc.
  ...
  - Vitalik: Agree that having these calls is good, giving relative indications about status of various SIPs, etc., but then just because everyone on the call agrees an SIP is good it should not 100% necessarily be approved
- Avsa: Someone opened an SIP, was probably a joke, can you have one standard negating another? Both can be drafts!
...
- On last call, we decided that some SIPs could definitely go in
  ...
  - [EIP168](https://github.com/sila-chain/SIPs/issues/168), [169](https://github.com/sila-chain/SIPs/issues/169) - Killing dust accounts, replay protection for this [[42:20](https://youtu.be/GhUtruRZOlo?t=42m20s)]
    ...
    - [Alexey Akhunov data](https://medium.com/@akhounov/more-data-for-sip-168-kill-dust-accounts-83e8dd0938d3)
    ...
    - Avsa: people who have moved SIL to WSIL would not be affected by this as there is no dust cleaning for tokens
      - Martin: don't think people would mind if dust is cleaned, most of it is probably from people who tried to move all SIL out of an account but didn't use Javascript bigints and didn't get the math exactly right
    - Hudson: no clear agreement on this so let's move on
  - <a id="eip859"/>[EIP859: account abstraction](https://github.com/sila-chain/SIPs/issues/859) [[58:42](https://youtu.be/GhUtruRZOlo?t=58m42s)]
  ...
  - [EIP232: New tx formats](https://github.com/sila-chain/SIPs/issues/232) [[1:02:18](https://youtu.be/GhUtruRZOlo?t=1h2m18s)]
  ...
  - Timing [[1:05:44](https://youtu.be/GhUtruRZOlo?t=1h5m44s)]
    ...
    - Are there other things we want to add? Or should we just continue to focus on casper and sharding?
      - Vitalik: We should consider a substantial reduction in gas costs of curves, ZK-snarks opcodes, this would be a fairly trivial SIP to put into the hard fork
        - Actual numbers are in [benchmarking repo](https://github.com/sila-chain/benchmarking)
...
# Test RPC protocol discussion [[1:11:48](https://youtu.be/GhUtruRZOlo?t=1h11m48s)]
  - https://github.com/sila-chain/retesteth/issues/4
...
# EIP648: Easy parallelizability [[1:13:29](https://youtu.be/GhUtruRZOlo?t=1h13m29s)]
  - https://github.com/sila-chain/SIPs/issues/648
  - Tim: Wondering if you guys think it's important enough to start considering
  - Similar to introducing a new tx type for [EIP232: New transaction formats](https://github.com/sila-chain/SIPs/issues/232)? Not quite
  ...
  - Casey: until we know bottleneck about disk I/O and how to optimize it, this SIP doesn't make it possible to increase block gas limit without some of the other disk I/O optimizations that are being explored with e.g. TurboGeth
    - TurboGeth disk I/O optimizations are therefore the most important things to work on prior to this SIP
...
# BLOCKHASH opcode refactoring (EIP96/EIP210) [[1:20:26](https://youtu.be/GhUtruRZOlo?t=1h20m26s)]
  - https://github.com/sila-chain/SIPs/issues/96
  - https://github.com/sila-chain/SIPs/pull/210
...
# Client/research updates [[1:21:39](https://youtu.be/GhUtruRZOlo?t=1h21m39s)]
  ...
  - Geth (Péter)
    ...
    - Martin implemented and we merged in bitshifting SIP
  ...
  - CPP
    ...
    - Dimitry: testeth tool now separated from cpp client, I've been working on retesteth repository, current version of testeth supported by Yoichi in his fork of cpp-sila, will remove from cpp client and make it more stable
  - EthereumJS (Casey)
    - no updates
    - Most team members working on cpp-sil and ewasm
...
- Alex Van de Sande (Mist/Sila Wallet)
- Casey Detrio (Volunteer/EthereumJS/ewasm)
- Christian Reitweissner (cpp-sila/Solidity)
- Daniel Ellison (Consensys/LLL)
- Dimitry Khokhlov (cpp-sila)
- Dmitrii (EthereumJS)
...
- Lane Rettig (Sila Foundation)
- Martin Holst Swende (geth/security)
- Nick Johnson (geth)
- Paweł Bylica (cpp-sila/ewasm)
- Péter Szilágyi (geth)
- Tim Siwula
