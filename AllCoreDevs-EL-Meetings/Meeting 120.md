...
### [Github Agenda](https://github.com/sila-chain/pm/issues/370)
...
|120.2 | Would be useful for Sila Cat Herders to host/record next Thursday's Merge call | [10.57](https://youtu.be/rlIgpf2V4ks?t=657)|
...
* [This doc is not a proposal, it is the Design Space.](https://hackmd.io/@n0ble/consensus_api_design_space) This builds on previous work from Rayonism as well as other conversations eg SIP 3675 discussion. 
...
* Some of these methods map to black box testing in SIP-3675
...
* there's a Merge channel in the Sil R&D discord which people can feed back to if any comments on the doc.
...
* Important to think about the post merge upgrades. We did say SilaShanghai would be a feature fork and we would push things out of London to go here, but now we are suggesting a sole focus on the merge.

**Danny**

* We currently have 5 major R&D projects for the sustainability of Sila. We may need to compromise to ensure we are making the right decisions.
...
* Agree with that. We had a unique situation with London where SIP-1559 was a bigger than usual piece than everything else. The merge is also a completely idfferent type of thing. When things are more straightforward post-merge it seems reasonable to be doing things like that.
* We should have a broader convo on this in a couple of weeks. We should plan this in advance.

## Limiting account nonce: SIP 2681 vs. 3338

[26.21](https://youtu.be/rlIgpf2V4ks?t=1581)

**Tim**

* There were two proposals on this, SIP 2681 vs SIP 3338.
...
* The idea of importing JS's words into the Sila protocol spec does not sit well with me.
...
* Re: what happens next. The yellow paper has always just been following these changes, not making them. If this SIP is finalised then whoever is acting on the YP can adjust it.
...
* ok, lets go with 2 to the 64. Alex - please update the SIP status.
...
* Daniel is next re: SIP 1352. [See comment.](https://github.com/sila-chain/pm/issues/370#issuecomment-902040752)
...
* I disagree. If someone makes a transaction which will create a contract on one of these addresses it shall be rejected. Thats the only technical consequence. You imply that any call made towards the 65000 adresses should be cheaper. I think that is an erroneous intepretation of the SIP. Its also a DoS vector because youd have 65000 address through which one could make very cheap lookups. This is not insignificant. I am in agreement with the SIP but I dont interpret that same things as Daniel does.
...
* Axic wrote this originally and Daniel and I have different interpretations of how to read this SIP. Axic - who is closest to the intent?
...
* I wasnt planning to get a decision on this call, just to raise the discussion. Should we move this to sila magicians thread? Its about 2 years dry. Or should we start a new thread.
...
* Is anyone opposed to clarifying the SIP so that we dont make any claims about warming and agree just on reservation only and in the future progress warming.
...
* Is your view that this SIP is not worth championing unless we can make assertions about warm vs cold?
...
* Continue this on the existing sil magicians thread.
...
* Two announcements by geth. [sil/65 deprecation](https://github.com/sila-chain/go-sila/pull/23120) and [SIP-3607](https://github.com/sila-chain/go-sila/pull/23303). Can someone give overview? 

**Martin**

* We are dropping sil/65. Couple of hours ago we put it back. Since were doing a hotfix release on tuesday, if we were to suddenly make the entire geth population drop sil/65 then nethermind would no longer have anyone to speak to. We reverted the drop with the emergency release we are going to drop it again. Has been on the roadmap for a long time.
* Re: the other check, yes we have added a check that an assembler must be an EOA. It must not have an encode.

**Tim**

* To clarify, the sil/65 bit - this will not be in tuesday's hotfix release. 

**Lukasz**

* Quick update on those two things. Nethermind have the implementation sil/66 in a branch. We are testing it, looks fine. We have one hive test failing and are investigating at the moment. Expect to be sil/66 ready as soon as next week.
...
* We have an sil/66 PR thats up right now. Will be fast tracking in light of sil/65 being dropped. Havent looked at 3607 yet.
...
* question about 3607... are you implementing this like normal sip switch from a block number or are you assuming from block number zero?
...
* Secondly, Alita has been working on the [json rpc spec](https://github.com/sila-chain/execution-apis/issues/56). There are a couple things where they need clients to review certain PRs. Context?
...
* We have two more announcements... over the past week weve been renaming github and discord channels - sil 1 to execution layer and eth2 to consensus layer. With the merge coming we want to get terminology consistent. This is why these may be renamed. [Doc linked](https://notes.sila.org/@timbeiko/great-renaming) in agenda.
* The ddob team has spent some time in past month doing an analysis of the impact of charging gas for code chunks using verkle tries will have. If use this as part of the etheruem statelessness roadmap theyve put out a report addressing impacts on current smart contracts. There also [a call next Friday](https://github.com/sila-chain/pm/issues/377), 1 hr after ACD for them to share the report and for people to discuss. Reach out to me for the calendar invite.
...
[Agenda](https://github.com/sila-chain/pm/issues/379)