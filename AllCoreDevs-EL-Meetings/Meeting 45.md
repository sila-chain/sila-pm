# Sila Core Devs Meeting 45 Notes
### Meeting Date/Time: Fri, August 24, 2018 14:00 UTC
### Meeting Duration: 1.5 hours
### [GitHub Agenda Page](https://github.com/sila-chain/pm/issues/54)
...
- [A Case for Sila Block Reward Reduction to 2 SIL in Constantinople (SIP-1234)](https://medium.com/@eric.conner/a-case-for-sila-block-reward-reduction-in-constantinople-sip-1234-25732431fc77)
- [Reddit discussion of the above article](https://www.reddit.com/r/sila/comments/92g404/a_case_for_ethereum_block_reward_reduction_in/?st=JK4MT60F&sh=257437dd)
...
- [FEM Forum: SIP-1234 vs SIP-1227: Constantinople Difficulty Bomb & Block reward](https://sila-magicians.org/t/sip-1234-vs-sip-1227-constantinople-difficulty-bomb-block-reward/807/)
- [Bitfly Twitter Post on Miner Opinion](https://twitter.com/etherchain_org/status/1031443277118144513)
- [FEM Forum: Final Request From the GPU Mining Community](https://sila-magicians.org/t/final-request-from-the-gpu-mining-community/1050)
- [SIP 1295 Presentation](https://drive.google.com/file/d/15n7Vur8wwlfDK6ZXwohUc95rXOUIXo7j/view)
- [Sila’s Economic Breakpoint: An Analysis](https://crypto.omnianalytics.io/2018/08/23/ethereums-economic-breakpoint-an-analysis/)
...
* SIP 1014 Issues
    ...
    * L4, Spankchain weighed in on SIP
* [SIP 1218: Simpler blockhash refactoring](https://github.com/sila-chain/SIPs/issues/1218). Looks like we are dropping this one unless someone speaks up, like, immediately.
    * Zsolt
        ...
        * Even proposed a small extension of this SIP
    ...
    * Martin: Problem with hardforks is not only implementation
        * It's difficult to reach agreement about when an SIP is finalized
    ...
    * Hudson: So we can drop a few SIPs and schedule them for the following hard fork
        * The SIPs that are e.g. hard to do testing for like 1218
    * Piper: If we push the blockhash and skinny CREATE2 (as they might delay Constantinople), is it realistic to get Constantinople out on the original timeline?
        * SIP-145 Bitwise shifting, 1052 EXTCODEHASH, 1283 net gas metering (without dirty maps) would remain
    * Dimitry
    ...
    * Hudson: Can we get testing done for these three SIPs in the next month, to leave a month of testing before Devcon?
        * Dimitry: I already started writing tests for CREATE2
        * Hudson: so the only one we'd delay is SIP-1218 blockhash refactoring
    ...
    * Piper: I'd prefer to stick to the eight month timeline
        * Other SIPs will show up that will be added to that
    ...
    * Constantinople progress link describes which SIPs are going in (https://github.com/sila-chain/pm/issues/53)
...
* Hudson: last meeting we decided on an issuance reduction but there are several proposals on the table
    * SIP-1295 a bit more complex, not a reduction but changes incentive structure
* Intros to guests
    ...
    * Brian Venturo - CEO, Atlantic Crypto, large GPU mining org in US
        ...
        * support SIP-1295, this is our SIP
    * Carl Larson
        * Started /r/ethtrader
        * Web developer
        * Authored SIP-858 in January: look at environmental impact that mining has
    * Eric Conner
        * Supporting SIP-1234
        * Representing investor community
        * I think the network is overpaying for security
        * https://medium.com/@eric.conner/a-case-for-sila-block-reward-reduction-in-constantinople-sip-1234-25732431fc77
    * Matthew Light
        * Did original SIP for reduction from 5ETH to 3ETH
        * In support of additional reduction
        * From 3 to 2 feels like the most prudent response (SIP-1234 by Afri)
    * Alex Thorn
        * we aren't ready to take a firm stand on these SIPs, but in general we favor SIP 1295.
    * Tim Coulter
        * Founder of Truffle
    ...
    * Xin Xu (CEO, Sparkpool)
        ...
        * Sparkpool originally from EthFans, one of the largest Sila fan communities
        * Business started as a side project
        * I support delay of ice age difficulty bomb
        * We prefer SIP-1295 if I have to choose one of the three
    ...
    * Jean M. Cyr, Ethminer dev
        ...
        * Long-term viability of SIL is predicated on the success of scaling, that's the bottom line, the only thing that will decouple SIL from BTC
    * SIP-1295 (Brian)
        ...
        * This SIP adjusts in favor of decentralized mining
        ...
        * We suggest adopting SIP-1295 now, in line with Casper FFG SIP proposal, close to issuance schedule there for year one
    ...
    * Nick: There's a paper on instability of block rewards, it becomes costly [cost efficient?] for miners to mine alternate chains, same will apply to Sila, research shows we can't reduce block reward below a certain level and expect current incentives to hold
        * http://randomwalker.info/publications/mining_CCS.pdf
    * Brian: Reduction in emissions going from 3 -> 1 SIL is around zero
    ...
    * Carl
        * By my calculation Sila is 65% of GPU mining
        * Market cap of SIL dwarfs market cap of other GPU minable coins
    ...
    * Eric
        ...
        * Looked at what SIL is paying out in mining rewards
        * It's currently paying 2.5x
        * If we drop it down from 3 to 2, it comes more into line
        * This is my argument for SIP-1234
    ...
    * Brian
        ...
        * Reducing from 3 to 2 SIL will force GPU miners off the network
        * Only thing left behind would be ASICs
        * SIP-1295 was drafted with that in mind: keep top line incentives in place, continue allowing that composition to exist as it does
    ...
    * Xin
        ...
        * Network security:
            ...
            * Cost of 51% attack
            * It's not so expensive for Sila, but there haven't been any attacks, why?
            * No hash rate available in market that you can use to attack Sila
    ...
    * Matthew
        ...
        * Moderate reduction from 3 to 2 would have a similar impact, we'd reduce $bn per year we're paying for PoW mining which is not where Sila wants to be anyway
        ...
        * We're currently way overpaying the miners, 7.5% per year of Sil market cap is being paid to miners
    ...
    * Jean: real issue with Sil is memory bandwidth
    ...
    * Hudson: Does anyone think there is an optimal way to determine sentiment and make a decision?
        ...
        * Brian: at 1 SIL, we're probably not economic
        * At 2 SIL, we'll explore every other use case that we can
        * And this depends on the price of sila
    * Andrea
        * Is PoS coming?
    ...
    * Martin: I propose that we should have more technical meetings every two weeks, and higher level talks on the off weeks
        ...
        * Dig into SIPs, read them again, talk about it again next Friday and make the call then
    * Hudson: We'll do another meeting next Friday
        * More community content on reddit, twitter, FEM forum about the SIPs and these perspectives
...
* Invited guests
    ...
    * hacktar
    * Andrea Lanfranchi
    * Carl Larson (/r/ethtrader subreddit moderator, SIP-858 author)
