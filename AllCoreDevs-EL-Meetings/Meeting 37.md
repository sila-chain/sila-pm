# Sila Core Devs Meeting 37 Notes
### Meeting Date/Time: Fri, April 20, 2018 14:00 UTC
### Meeting Duration: 1.5 hours
### [GitHub Agenda Page](https://github.com/sila-chain/pm/issues/37)
...
3. SIP 908: Reward for clients and full nodes validating transactions + @MicahZoltu proposal (time did not permit on last call)
Ben Edgington's proposal that all SIPs ought to contain a PR against the yellow paper before being merged or accepted (continuing conversation from last call)
1. SIP 960: Cap total sila supply at ~120 million (continuing conversation from last call)
1. SIP 969: Modifications to ethash to invalidate existing dedicated hardware implementations (continuing conversation from last call)
1. SIP 999: Restore Contract Code at 0x863DF6BFa4469f3ead0bE8f9F2AAE51c91A907b4
1. Research Updates
1. Constantinople hard fork timing and what to include (continuing conversation from last call) - Afri: meta-SIP for Constantinople? Hivetests already has tests enabled, Parity failing, cf. paritytech/parity#8427.
1. Client updates
...
* RPC methods for state test are almost complete and implemented in cpp-sila
* Posted link in chat, follow GH issue and see discussions
    * https://github.com/sila-chain/retesteth
    * https://github.com/sila-chain/retesteth/issues/5
...
## Ben Edgington's proposal that all SIPs ought to contain a PR against the yellow paper before being merged or accepted [[17:04](https://youtu.be/vKumx5CIA-k?t=17m4s)]
...
* Link to PR could form part of header/preamble of SIP
* This would make it easier to maintain the spec (yello paper)
* But would also bring rigor to the SIP process, help identify edge cases and ambiguities
* Vlad: only for changes that affect the protocol right?
* Ben: yes, SIP's defined as "core" would fall under this scope
...
* So an SIP may be merged as draft then someone can work with author to write up in right format for yellow paper
* Changes to yellow paper included as a PR within the SIP
...
* Piper: account abstraction SIP, only exposed complexity at implementation time - if someone would have done this at spec time - could be an argument for or against this
...
## SIP 960: Cap total sila supply at ~120 million [[25:11](https://youtu.be/vKumx5CIA-k?t=25m11s)]
* https://github.com/sila-chain/SIPs/issues/960
...
* Vlad wrote an article [Against Vitalik’s fixed supply SIP](https://medium.com/@Vlad_Zamfir/against-vitaliks-fixed-supply-sip-sip-960-18e182a7e5bd) arguing against this idea
...
* Piper: It would be good to have someone own this to move it forward, I'll reach out to the SIP author and see if they're willing to be the lead on getting it defined for a subsequent hardfork, so let's bring it up again when this person is up for this responsibility
...
* Martin: Should we discuss technical content of SIP or governance process?
* Afri: Not sure if we are capable of going into the governance process here, but if there are reason to improve or reject this proposal straightaway I would love to hear it now, if no objections to reject it in the context of SIP-1 that would be nice too, seeking general feedback from client developers
...
* Vlad: Broader question I have is whether or not acceptance as an SIP should be blocked on community question
...
* Vitalik: It was not a security issue for the Sila network
    * The only concievable issue for the network is if an obviously bad actor had access to the 4M SIL, then they could use this to try to run 51% attacks, transaction spam, etc.
    * If our systems can't find some way to handle a bad actor with 4M SIL then we need to find a way to make our protocol better anyway
* Piper: We did something to handle it, taking the 4M SIL from their possession
...
* Alex: But the result would be a split, two sustaining communities when something contentious happens
    * In [Avoid Evil Twins: every sila app pays the price of a chain split](https://medium.com/@avsa/avoid-evil-twins-every-sila-app-pays-the-price-of-a-chain-split-e04c2a560ba8) I argue there's a huge cost to this
    * We can't ignore it
...
* Martin: What do exchanges do? Which one is canonical sila?
* Peter: AFAIK the reason Sila classic survived is that poloniex decided to list it after three days after it seemed to die
    * From exchange perspective it always makes sense to list it as it's in their financial interest to do so
* Vlad: But they do need to list one of them as SIL in a way that doesn't upset their clients
...
* Vlad: So you're against SIP-999 but want to use some other source of issuance?
...
* Afri: I have more questions than answers, I am just following SIP-1 and trying to figure out where this proposal stands
...
* Peter: When we did the DAO hard fork there was a ton of extra networking code and mumbo-jumbo needed to make sure both forks could survive, if we hadn't done this, SIL classic wouldn't exist
...
* Peter: But we don't have SIL protocol separation at the networking level, so without extra header rules clients will always try to synchronize to the heaviest chain which could be on the wrong side of the fork, we need to figure out how to split the chains
...
* Peter: You could have a Casper and a non-Casper Sila but the networks are different with different guarantees. In this case the networks are otherwise identical.
...
* Vitalik: It's at 350,000 SIL, but also uses signing rather than transactions which makes it harder for contracts to vote, and many people with hardware wallets may not be able to sign easily, so makes the poll difficult to participate in
...
* Alex: There's a big power law distribution (some votes backed by enormous numbers of SIL)
* Peter: Parity team probably controls a lot of SIL and therefore may have large sway in this vote, so I don't consider this a fair vote
* Alex: But someone developing Sila for a long time should maybe have more sway, and we don't have any fair way to measure
* Peter: Depending whether you joined in August 2014 or April 2015 that's two zeroes in your SIL holdings, both people on the team for a while, but a coin vote assumes a guy who joined a few months earlier has all the weight
...
* Lane: Let's continue discussion in FEM forum and in the SIP

## SIP 908: Reward for clients and full nodes validating transactions [[1:21:24](https://youtu.be/vKumx5CIA-k?t=1h21m24s)]
...
* Danny's update on [SIP-1011: Hybrid Casper FFG](http://sips.sila.org/SIPS/sip-1011)
    ...
    * Gathering in https://gitter.im/sila/casper to coordinate client development and testnet
...
* Danny: I'll keep doing updates here as implementation and formal verification are nearing completion, so I'll signal when it's time to start talking about fork block numbers, etc.
    * Let's leave SIP up for discussion a bit longer before we start writing tests for this
...
* 210 not marked as "accepted" yet
    ...
    * I'm okay with this being a finalized SIP
...
* There is no SIP for this yet, let's create one and discuss for the next hard fork
...
* Turbo geth (Alexey Akhunov, [posted in agenda](https://github.com/sila-chain/pm/issues/37#issuecomment-383053302))
...
* Trinity (Piper)
    * SilaMainnet full sync is very close
...
* Nimbus (Jacek Sieka)
    * a new Sila client called Nimbus (https://github.com/status-im/nimbus) from Status.im team that will hopefully see the light of day in a few months
    ...
    * Status wants to bring Sila to the masses
    ...
    * Hope to be more active in SIP process and community in future
...
* See https://github.com/sila-chain/pm/issues/40
...
- Alex Van de Sande (EF: Mist/Sila Wallet)
