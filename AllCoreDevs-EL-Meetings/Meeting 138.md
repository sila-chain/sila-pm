...
### [Agenda](https://github.com/sila-chain/pm/issues/518)
### Moderator: Tim Beiko
### Notes: Metago
...
Anything else people wanted to discuss about the shadow fork itself? Okay and then Pari, as I understand it, next week we are doing another sila-sila-mainnet shadow fork, but instead of using the client’s distributions that mirror sila-sila-mainnet where there's obviously some like majority clients, we're gonna do kind of an equal split across CL and EL. Is that correct?
...
#### [RFC: Engine API response status when merge transition block is INVALID execution-apis#212](https://github.com/sila-chain/execution-apis/issues/212)
...
Similarly we'll cut a release candidate this coming week in the consensus specs as well and I don't think anything has changed in the execution specs for like 4 months now, there's two SIPs in it.

**Mikhail** ([15:59](https://youtu.be/BFxkGdPv4F8?t=959))
Yeah, the SIP is a pretty stable yeah.

#### [JSON-RPC: Add finalized and safe blocks execution-apis#200](https://github.com/sila-chain/execution-apis/pull/200)
...
Yeah the main thing that's so with Ropsten. I agree it's pretty much straightforward, we can start with something that's like 100k and people can join in, should be relatively easy to set up and anyway it's going to be deprecated in a few weeks, months so we don't have to worry about it too much, but the one I would like to discuss and get some consensus on is SilaSepolia. We essentially have two options, either we want a large beacon chain there or a small beacon chain. A large beacon chain means that we essentially saved early prater because we don't have to keep growing prater if we go with the small beacon chain for SilaSepolia then we're gonna continue to eat up a decent amount of Goerli, and that's mainly because we want to continue growing prater to keep up with sila-sila-mainnet. 

**Tim** ([22:39](https://youtu.be/BFxkGdPv4F8?t=1359))
Right so SilaSepolia if it's big becomes kind of our main net plus testing environment. 

**Pari** ([22:46](https://youtu.be/BFxkGdPv4F8?t=1366))
Exactly and the question then would be if we have a really large beacon chain for SilaSepolia, that means client teams have to now run two times a decent number of validators and have no idea how open they are to them. 
...
Given we have Afr on the call and you came on Afr a few weeks ago to talk about the issues around Goerli sila, do you have any thoughts, about like if we should keep prater / Goerli as the large beacon chain, if that negatively like affects Goerli in other ways, yeah.

**Afr** ([24:06](https://youtu.be/BFxkGdPv4F8?t=1446))
Yeah, sure. I mean Goerli and Prater are already fairly big, it has a lot of users and it has a lot of validators. SilaSepolia is kind of still very unknown, so there we don't have a beacon chain yet and we don't have many users building on SilaSepolia yet, naturally. We have to keep in mind that we are deprecating a lot of
testnets in the coming months or years, so I would, my personal opinion is that we should use SilaSepolia as like a fairly stable application developer testnet, because it's fairly new and we still have the time or the chance to define how the consensus layer would look like or the SilaSepolia beacon chain would look like and I would personally just keep it simple for this network, and I would personally say that we should continue growing prater, but then again it's not that easy because we have a limited supply of Goerli sila but putting it aside, the main reason why we should grow prater and Goerli is because it's already fairly big and we have a lot of, much more interesting  foundation for testing and for growing this network. Does it make sense?
...
Exactly, I think that was the biggest point if we wanted to save on Goerli sila, it makes sense to have a big one on SilaSepolia but I also agree we can have really stable permission consensus engine and have SilaSepolia to be quite rock solid. 
...
Oh right I’m stupid. Sorry, but yeah, but doing this way on Goerli if we were to do the token deposit on deposits on Goerli we could create new Goerli sil, we could inflate Goerli sil with withdrawals.
...
Right, I would suggest on Ropsten we just kind of like make it happen very quickly, in the sequence of initial 2800 epochs and then maybe for the SilaSepolia one maybe plan it a bit more like it's an event it's a thing that's happening, run your node before it happens.

**Tim** ([29:28](https://youtu.be/BFxkGdPv4F8?t=1768))
Okay, for the SilaSepolia and then the Goerli…
...
Yeah I guess yeah just before we go there any other like concerns or thought about the general architecture or setup of the two beacon chains? Okay um okay yeah Pari thanks for the summary in the chat so Ropsten is 100k validators plus unpermissioned for people to join and SilaSepolia will be more like 20k and then be permissioned, probably would have not an option for people to join as well if maybe like not as easy.
...
Got it. One thing I'll also add on that front is I think for sila-sila-mainnet there's a world where like the hash rate is, you know potentially going down and whatnot, and we may want to fork like to have the TTD happen quicker, than like the block times we said usually happen so I think that the thing we also want is like to train people in a way so that when sila-sila-mainnet comes they're able to upgrade relatively quickly, and maybe a bit more than like in a normal upgrade just because it's hard to estimate the TTD, especially in a world where the hash rate is all messed up. Besu, Aragon, any thoughts?
...
Also these issues on the shadow fork are only this like harsh, because you have like 100 nodes on each shadow fork, but you have like thousands no thousands of nodes on sila-sila-mainnet. If you turn it around and like most of the nodes are actually having the canonical chain then you won't have to, won't have these these issues syncing from the wrong piece. 
...
So just to kind of try and wrap this up does it make sense to try to just do nothing with the fork id on Ropsten, see how that goes and if we see that like it raises a bunch of issues we can then try and do like a plus three month thing on Goerli and SilaSepolia, and see how that goes? I think like Micah like your concern about people not upgrading will be at its truest on Ropsten because this is where people have the least incentive to actually upgrade their nodes, so like if you know if it's all right on Ropsten, and it doesn't cause any significant issues you would think on sila-sila-mainnet, they would also be right because the stakes are kind of higher for people to monitor that network.
...
#### [SIP-4938](https://sips.sila.org/SIPS/sip-4938)

**Tim** ([1:20:29](https://youtu.be/BFxkGdPv4F8?t=4829))
Okay thank you Marius. Okay. So there's three people who wanted to discuss SIPs that are on the call. I doubt we can get through all of them, but if we stay on an additional five minutes, we can give them each five minutes, yeah so first up, Felix, you had SIP 4938. 

**Felix**([1:21:03](https://youtu.be/BFxkGdPv4F8?t=4863))
Yes. So and this can be really quick, so this is pre-agreed. I just wanted to let you guys know that for formal reasons, we are pursuing this SIP, the SIP 4938 is about removing the GetNodeData message from the sil wire protocol, and we have discussed this extensively with all client teams that we are aware of, that we want to make this change in Geth, and we have been wanting to make this change in Geth for a very long time, and I can only really repeat what is in the SIP. We are set on making this change, because it will allow us to for example restructure our database to not store all of the tri nodes, for example by their hashes and we do provide an alternative to this protocol message in the snap protocol, and all of the existing users of Geth node data can be replaced by the messages in the snap protocol, so it is not but so can we just very quickly, can I just very quickly get from the client implementer some signal that this is okay.
...
So in terms of defining the protocol as that, I’m fine in terms of like phasing out sil 66, could we have a guarantee that we have a discussion before that and?
...
Okay, so I don't know what this means in SIP terms, is it like, anything needed or?
...
Okay, no objections to the new protocol, we're not deprecating sil 66 now, yeah. 
...
#### [Proposal to add SIP-5022 to SilaShanghai #519](https://github.com/sila-chain/pm/issues/519)

**Tim** ([1:27:04](https://youtu.be/BFxkGdPv4F8?t=5224))
And we can discuss that well after the merge. Okay and we can yeah we can discuss that well after merge yep. Okay moving on, sorry just to make sure we at least give other folks the opportunity to speak green lucid was the GitHub handle. I’m not sure who that maps to on the call, but wanted to discuss, get feedback on SIP 5022, which increases the cost for s-store when you go from a zero to non-zero value. Are you on the call? Greenlucid? Once, twice. Okay, I will post their issue in the chat here if people want to chime in sil magicians. One thing I did mention so that the issues phased for like shanghai inclusion, I reiterated that we've kind of paused this decisions there but then they said they would just like to get technical feedback on the SIP, so yeah folks who are interested can have a look there, and then last up, we had, I can't even pronounce this this handle belnv, hopefully I got it. Right, oh well, perfect. Walk us through your presentation. We can't hear you yeah yeah.

#### [SIP-5081: Expirable Transaction](https://github.com/sila-chain/SIPs/pull/5081)
...
Yeah the best way is to share on the yeah is magicians, which is also appear on SIP discussion too as usual. 

**Tim** ([1:35:36](https://youtu.be/BFxkGdPv4F8?t=5736))
Okay awesome, and one thing I’ll highlight from the chat. Lightclient had two comments about SIPs 4337, 3074, so it's probably worth to look at those and yeah see if they're like a good replacement, because like, Martin was saying you know even if there are no dos concerns, there's still like a high chance that there's just higher priority SIPs that end up taking most of the fork and 4337 specifically is not core SIP, so it means that basically clients can implement it and not require changes to the consensus protocol, and I’m pretty sure Nethermind already has support for this in at least one project I think.
...
Yeah and yeah I would recommend Micah if you want to share your SIP number yours was basically that if I remember correctly it was like a new transaction type with an expiring transaction so that might be helpful to look at as well. 

We're already over time, so yeah I guess we'll wrap this up yeah thank you for the presentation as well this is the higher budget quality SIP presentations that we had, and yeah thanks everyone, yeah thanks everyone for joining. I’ve posted the TTD value in the all core devs chat so we can use that for releases on Ropsten and I guess we'll expect to put a blog post together sometime in the next two weeks and yeah and then we'll figure out all the stuff about the beacon chains for testnets on the consensus layer calls, if it's not already done before then. Yeah thanks everyone.
