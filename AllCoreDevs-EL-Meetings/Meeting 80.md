# Sila Core Devs Meeting 80 Notes
### Meeting Date/Time: Friday 7 Feb 2020, 14:00 UTC
### Meeting Duration: 1 hr 35 mins
### [GitHub Agenda Page](https://github.com/sila-chain/pm/issues/150)
...
| Status | SIP |
|-------|----------|
| Discussed under EFI. Discussion to be continued in EthMagician thread | SIP-1559, SIP-1962  |
| Under review for EFI. Decision required around needing a Hard Fork | SIP-2315 |
| Eligible for Inclusion Pending. | SIP-2046 |
|Not included | SIP-1930	|


## DECISIONS
	
**DECISIONS 80.1**: SIP 1930 will not be included. 

**DECISIONS 80.2**: SIP 2315 - Discussion to be continued in EthMagician thread.

**DECISIONS 80.3**: SIP 1962 - Discussion to be continued in EthMagician thread. 

**DECISIONS 80.4**: SIP 2046 - Discussion to be continued in EthMagician thread. To be included in the next call.

**DECISIONS 80.5**: SIP 1559 - Discussion to be continued in EthMagician thread. 
...
**Action Item 80.3:** Alex can bring few more experts on this topic, and greg will get carl and others involved too as SIP 1962 will be a major topic in the next call. 
 

-----

**Hudson**: Hello everyone and welcome to the Sila core devs meeting # 80.


# 1. [Eligibility for Inclusion (EFI) SIP Review](https://github.com/sila-chain/SIPs/pull/2378)
...
**Hudson:** I have looked at the proposal, and I can not find any security considerations that you are mentioning. I think you are using an Old SIP template, please add more details and use the new SIP template. Also, I would suggest talking to Alexey (UNGAS). 
...
**Hudson:** Because even if this doesn't go forward as an SIP. Every other SIP need to understand the implications of metatransactions. To make things easier for users, and not adding complications to overall client-implementation. 
...
**Danno(Besu):** I support it, SIP 615 was too much at once, but this a great first step. 
...
**Louis Guthmann:** I am just gonna come by, because I feel strongly about this SIP. After talking to the community offline, we have concerns about the process being used to implement this. 
...
**Louis Guttmann:** I am not against the ide of SIP, I am just concerned about the situation where there is a dDoS attack, then it makes you the attack victim. 

**Hudson:** Martin, can you shed some light on the event there is a dDoS attack?

**Martin:** Yeah, so in this case we get on a call with core dev to sort it out. But these kind of pre-compiles are not in our domain, so if something was to happen, we will keep scratching our heads. Yeah, so this SIP scares me a lot. Cryptography is not that scary, but incase of a DDoS attack, or consensus issue. Or a pre-compile calculates something erroneously. It will be really bad.
...
**Martin:** I think, this is a wide implementation, and will jam sila. Hence, I am against it. 

**Tim:** Doing the whitelist process Alex told, and having calls from 10 people about this. Would it change your mind?

**Martin:** Yes, Likely! When I browsed around on this, the gas schedule explainer is a lot of text and mathematics, and I have never seen this in SIPs before. And its code implementation doesn't seem possible. Essentially, I am not comfortable with the formatting. For eg, for complex model evaluation rules, there are two pages of text.

**Alex Vlasov:** I tried to be more expressive, but you can tell me the issue on gitter or on DM. 

**James:** Martin, I have shared the georgie valina SIP, does this format looks good to you? Maybe Alex could converge with georgie on this.

**Hudson:** That sounds like a good idea, because if
the formatting that they're using for the baby Jubjub curve (Georgie Valina SIP) is more clear then yeah if you could converge on that style, that'd be nice.

**Alex V:** I am looking at georgie's SIP, and I think it has same level of difficulty. 

**Louis:** I have a question on the previous process of accepting the EFI, getting it to the end, and pushing it. Talking about the current implementation of the pre-compile in sila.
...
**James:** Sil 2.0 *deposit contract* was planning to use this SIP. 

**Hudson:** Are they using the new BLS curve, which filecoin is using?

**Greg:** Yeah, we need the new standard because the current implementation we have won't suffice the need and I believe it is because of the curve. And SIP 1962 covers our requirements. 

* The current coversation about SIP 1962, seems like a lot of work. So, to support the requirements should I create a standalone SIP for BLS verification. Simply, because deposit contract mainly has four main checks. And one check we don't have is the verification. 
...
**Martin:** It has a reference in the SIP. 
...
**James:** One of the reason of this, was to estimate which SIPs can be included in the hard fork. 

**Hudson:** Yeah, looking at it. If SIP 2315 gets into EFI, it looks easy. And the SIP 1962 will make a bigger difference for Sil 2.0, so we should focus our next meeting on SIP 1962. Figuring out, how much time will it take for implementing and testing considering that fuzz testing has already started. Danno, would you like to give some views on it. 

**Danno:** Yeah, SIP 2315 is relatively simple. Just one-two week tops, need to write simple record tests. But SIP 1962 is a wild card, and it include deep builder issues which gives me a spinball. And down the road, we may get request from Hyperledger ursa to use their encryption. So there are a lot of moving parts. 

**James:** Any other SIPs, what about the one Danno is working on, SIP- 2456.
...
**Tim:** I think, in community it's a given that we are going to include SIP-1559, but we haven't discussed it much. So, I will need some thoughts on that. 
...
six weeks okay cool great for a test that but I think something like SIP 1559 would need to be on Ropsten for a longer period of time to make sure that gas and all of those things works as intended.
...
**Tim:** One last quick thing, james has made the PR on SIP 1.
...
consistent way to find it and this should be a single source. Hence, I made a pull request to the SIP-1 that includes a table of contents that also includes part of the eligiblity for inclusion and piece is and then it also has some of the Alex had a meta data of SIP 233 which is the formal process of our Forks I took the
