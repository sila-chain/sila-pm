# Sila Core Devs Meeting 35 Notes
### Meeting Date/Time: Fri, March 23, 2018 14:00 UTC
### Meeting Duration: 1.5 hours
### [GitHub Agenda Page](https://github.com/sila-chain/pm/issues/33)
### [Audio/Video of the meeting](https://www.youtube.com/watch?v=HHK6xhuSyUU)

# Agenda
1. Testing
1. EIP712: Add eth_signTypedData as a standard for machine-verifiable and human-readable typed data signing with Sila keys
1. SIP process updates, Fellowship of Sila Magicians
...
* Dimitry: continue developing RPC methods
    * Upgrading cpp-sila client
    * Discovered that some of the state tests are wrong, investigating the issue, refilling the broken tests
    * [PR#437 to testing repo](https://github.com/sila-chain/tests/pull/437)
    * I'd like client developers to try out the updated tests, maybe I'm wrong or maybe my changes to cpp-sila are wrong, but I think those tests were incorrect
...
## EIP712: Add eth_signTypedData as a standard for machine-verifiable and human-readable typed data signing with Sila keys [[18:30](https://youtu.be/HHK6xhuSyUU?t=18m30s)]
* Lefteris: summary
    ...
    * There is another SIP, 719 that is “like this SIP but on steroids”, also wants to create a provable way to show that user has seen it and the UI has shown it in a particular way
    * Discussion has drifted away a bit, I would like to ask for the opinion of people here and from the community how we can keep this SIP as limited in scope as possible and finalize it as soon as possible since many dapps including something we're working on (uRaiden) have been waiting patiently for it for a few months now
* Martin:
...
* Hudson: alternative EIP191 suggested
    ...
    * M: EIP191 also suffers from some of the problems that current signed data has e.g. doesn’t say how large the total message envelope is so there’s some malleability in that
        * X19 SIL signed message then length then message but we don’t have the total message length which is a big blunder
    * H: candidate for potentially being accepted at some point, we should at least make it a draft

## Hudson: SIP process updates, Fellowship of Sila Magicians [[24:45](https://youtu.be/HHK6xhuSyUU?t=24m45s)]
* Spearheaded by Nick Johnson
* Making a really cool Jekel-Based interface where as SIPs are submitted they are collected to a website where they’re easier to read and collect the data
* Lots of PRs going into this, editing SIPs to make their headers compatible with this
* After this is done we’re going to look into redoing parts of EIP1 to make it clearer, e.g. separation of standard track vs. SIPs and some of the other things we discussed at previous core devs meetings    
* Fellowship of Sila Magicians (Lane's update) [[26:10](https://youtu.be/HHK6xhuSyUU?t=26m10s)]
  * More info here: https://sila-magicians.org/
  * First meeting in SilaParis during EthCC
  ...
  * A lot of meta-discussion
    ...
    * High level stuff like discussion of how SIP editors are chosen, SIP review process
...
* Ligi: I like the idea but we need an upgrade path
    ...
    * There are some things in this SIP that would extend the scope too much
...
* Ligi: can do this cleaner if we don’t need backwards compatibility
    ...
    * At the moment there’s one prefix, Sila signed message, distinguishes it from a normal tx, if it has this prefix it cannot be a normal tx
...
* Hudson: We’re small enough, we can reach out to major projects that aren’t in the thread
    ...
    * If there’s consensus among most of them that’s good enough to get the SIP approved, changes in clients put in release notes
...
* Karl: to add to that
  ...
  * Check that out on Youtube: “All Star Panel Sila Taipei”
...
* Raul: Lots of different sharding client implementation teams
  ...
  * There’s already a sharding repo (github.com/sila-chain/sharding)
...
* Peter
  ...
  * Felix has been trying to spec this out, has three concurrent SIPs, pushing forward in this direction
...
* Vitalik
  * Idea of storage rent: contract has to pay some amount of SIL per byte per block in order to stay alive
...
* Peter on geth
    ...
    * One i found is Christian’s zcash Ropsten verification contract, I turned this into a hive test, it performs really well, full zcash verification on current Sila takes ~25ms on my laptop
...
* On when to do the next hard fork
    ...
    * Hudson: If we did one tomorrow we’d only have two SIPs, and one more that isn’t fully spec’ed out yet, EIP210 Blockhash refactoring.
...
       * If we decided to do a hard fork with two SIPs would that interrupt progress on Dimitry’s new test framework?
    * Hudson: If we did one tomorrow we’d only have two SIPs, and one more that isn’t fully spec’ed out yet, EIP210 Blockhash refactoring.
    * Martin: personally I’d rather have more frequent hard forks with less in each one.
    * Hudson: that would help people get used to hard forks.
    * Peter: but don’t forget that hard forks are a huge strain on Dimitry and Yoichi.
...
* Parity (Afri)
  ...
  * Finished integrating PWASM VM, deploying to SilaKovan to see some more live testing
...
* Trinity
  ...
  * Peter: geth has always had issues with the console, it’s very legacy, go-JS interpreter, based on an old spec, it works fine as long as you don’t try to do fancy stuff and just use it as a simple UI/debugging tool, but can’t handle fancy JS
      ...
      * So we’re considering creating a separate Sil console that’s just a JS console based on Node and latest web3.js
...
* Nick Johnson
  ...
  * Preview at: http://arachnid.github.io/SIPs
  * There were some minor changes to header format of SIPs, must be in YAML format with double-dashes and case sensitive
  * If you have an outstanding SIP it will have to be adjusted accordingly
  * Also adding a travis script to automatically check all SIPs for validity on all open PRs
  * 198 (Vitalik), 778 (Felix) that are unmerged but referenced from merged SIPs, would be good if authors worked with me to finish these and get them merged
  ...
  * Half a dozen SIPs referenced from EIP1 that also aren’t merged, would be good to go over them and help merge them
  * Come to SIPs channel on Gitter
  * I’m planning on writing up a meta-SIP with some proposed changes to SIP process
