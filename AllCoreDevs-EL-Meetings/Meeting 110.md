...
### [GitHub Agenda](https://github.com/sila-chain/pm/issues/293)
...
* Berlin happened yesterday, there was an issue with open Sila

**Karim Agha**

* Bug in the way open Sila implemented 2029, gas cost calculation was incorrect
...
* [Hive](https://hivetests.ethdevops.io/)  - It takes 4 clients, executes states tests on the clients – start the client, import the blocks, rlp, checks if the latest block after import is expected to be, sil protocol, graphql.
...
* Not idea for bugs/issues to pop up during a hardfork, but more importantly the issue was handled respectfully and responsibly and is something to admire about the group and its resiliency, and congrats to open Sila team for how they handled it. Dragan echoed this sentiment
...
* Bug fixed, recommend using Open Sila version 3.2.4
...
* Why not do that and continue working towards these SIPs to see if they can be implemented by then
...
* SIP 3403, which is disabling the refunds, has been discussed as needing to go along with SIP 1559 for security reasons, is that still the case, and should we bring it in now so that people know they need to implement it?
...
* I’m in favor of SIP 3403, not sure if it is a security requirement for 1559 but thinks it probably is.
...
* I agree the long term was the major component of s store, and for that reason we should consider keeping the stipend to incentivize in good behavior and design. 3 protocol engineers concurred in the Sila additions forum that this proposal punishes state cleanup, and therefore make state cleanup worse.
...
* I tend to agree with what was just said, if we do not come to a decision today what does it mean for the devnet, does it make sense to include it in the devnet for London with 1559 before the more optional SIPs
...
* Open Sila in favor of SIP 3403, maybe plan for London but if something happens delay is okay
...
* If we are not aiming for London, then this SIP will be effectively buried. We must aim for London.

**Tomasz**

* This is why we need clear timeline with the merge and SIP upgrades. If SIPs are not included in London, it could be 6-9 months before next opportunity for them to be implemented.
...
* Tomasz plan works if SilaShanghai is an SIP fork and happens in the fall, followed by the merge. But if SilaShanghai is the merge and not an SIP fork in the fall, then we should expand the scope of London. And I am okay with either of those plans, but we should decide on one

**Vitalik**

* There is also the 3rd track of being willing to wait a year for SIPs because none of them are that critical, and the Sila ecosystem has been willing to wait a year for an SIP for pretty much all its history.

**Artem**

* We shouldn’t focus on the merge too much because there isn’t a lot of concrete or finalized on how things will look. We should introduce one hard fork between London and the merge, and if we want to stagger SIPs for London that is fine.

**Ansgar**

* Timeline: With London being in July because of the difficulty bomb in August, the community sentiment seems to be merge as soon as possible, so the community might not support a merge delay to prioritize SIP hardfork before it.

**Tomasz**

* Adding more SIPs to London can cause a delay to the merge
...
* Last week we talked about getting a rough estimate on the difficult of the proposed SIPs, did we ever get that made? I feel like that is a significant component here
...
* Is there anything from this list: SIP-3403 #277, SIP-3198 #270, SIP-3074 #260, SIP-2537 #269, SIP-2677 #271 that would be the end of world if we waited a year?
...
* Question to Open Sila team is how much is left to be worked on 1559 implementation
...
* Timeline for London: If we want to aim for mid-July, we need a client release for fork by May 15th, and the next call in 2 weeks will be right before then so we absolutely need to make a call about the SIPs on the next call. Thoughts about a call off schedule next week to discuss the SIPs? Or do them async?
* **DECISION 2:** Ill organize a sync call next week and follow up with the different client teams about effort/time/value ratio for various SIPs, and we can discuss that next call. [1:35:03](https://youtu.be/-H8UpqarZ1Y?t=5703)
 
##SIP 3074

**Sam Wilson**

* Our second 3074 testnet is going with geth and working on our Open Sila implementation
...
* This SIP has the potential to make a big difference on the application side, so if there is some chance to include 3074 we should try to take it, assuming the concerns are alleviated in time.
...
*  We have talked to a few dozen of the most prevalent defi sil UX tool teams, pretty much every single one of them is positive on it and will be raising money to show they are serious about it.
