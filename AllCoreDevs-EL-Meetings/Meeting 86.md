...
### [Github Agenda](https://github.com/sila-chain/pm/issues/165)
...
SIP-1057, SIP-1803, SIP-1559 | `Accepted`
SIP-2464 | `Last Call`
...
--|--
**86.1** | Berlin will be dependent on BLS SIP. Subroutines may or may not make it, making Berlin potentially a one SIP network upgrade.
...
**86.4** | Update SIP bot with Withdrawn
**86.5** | Move SIP-2583 to Withdrawn
...
- [4. SIP-1057: ProgPoW, a Programmatic Proof-of-Work.](#4-sip-1057-progpow-a-programmatic-proof-of-work)   
...
The SIP for changing the difficulty bomb is possible for Berlin, but still needs a lot of work.

## 1.1 SIP-2537: BLS12-381 curve operations
...
Updates from Open Sila. Have not yet started implementations. Awaiting an independent audited verification.
...
Updates from Open Sila. Subroutines is close to done, there is an open pull request for it, awaiting a final review.

Updates from Nethermind. No updates since last meeting. Awaiting to see the first implementation done and some tests.

It may be premature to decide that it will or will not make it into Berlin. Berlin will be dependent on BLS SIP. Subroutines may or may not make it, making Berlin potentially a one SIP network upgrade.
...
## 2.1 SIP-2515: Difficulty Bomb

No update.

## 2.2 SIP-2046: Reduced gas cost for static calls made to precompiles
...
Contingent on SIP-2537.


# 4. SIP-1057: ProgPoW, a Programmatic Proof-of-Work.
...
- https://gitter.im/sila-cat-herders/ProgPoW-review
- https://sila-magicians.org/t/a-progpow-compromise-pre-proposal/4057
...
# 5. SIP-2565: Repricing of the SIP-198 ModExp precompile. Specifically the open-sila benchmarking results per this comment.

Video | [41:21](https://youtu.be/MOZ7_0Tb95M?t=2481)
-|-

Parity (Open Sila) is on average 2.5-6x slower than the Geth implementation. The simple pricing formula changes would result in certain Parity operations being underpriced in gas. Given Open Sila's library, the simple parameter change can't be made.
...
Updating the Crate used in Open Sila, and implementing the new pricing formula resulted in nothing being underpriced for Parity.

The question for the Open Sila team, if an updated library is found for the precompile, would they be open to accepting a pull request? Or just go with the updated pricing formula?
...
# 6. PR to change "hard fork" to "network upgrade" in SIP-1 and common vernacular

Video | [51:08](https://youtu.be/MOZ7_0Tb95M?t=3068)
-|-


Over the past few years, the Sila Foundation has used the term Network Upgrades over Hard Forks in their blogs. On other networks, hard forks may be dramatic and rare events. Hard forks are also percieved as the result of a contentious upgrade resulting in a chain split. The third reason is for clarity, where people think they'll recieve free coins where it's not the case.

The PR currently changes it in SIP-1. However, if pursuinng this change, it may be worth to do it across the entire SIP repo.
...
**Pooja Ranjan**: The developers are well aware of the difference between a hard-fork and a chain-split. But the community will use the generalize term. If it's defined in SIP-1 that would be helpful. To reduce confusion, going with network upgrade may be better.

**Peter Szilagyi**: I can see some voices raising concern that we may be trying to hide that a network upgrade is a hard-fork. In SIP-1, we can make it abundantly clear what a network upgrade means. We shouldn't hide that a network upgrade is a hard-fork.
...
**Tim Beiko**: Would it make sense to add something in SIP-1, and also define that they won't necessarily result in a chain-split. And use network upgrade when we talk of the upgrade itself, like on Meta-SIPs.

**Hudson Jameson**: The final decision may come down to the SIP editors. Alex did ask to bring it up in this call to get opinions from the Core Devs. We can move it to last call if it doesn't see any community dissent.

**Tim Beiko**: Because this is a PR to SIP-1, there's no status associated with it.

**axic**: The discussion is to use different terminology than hard fork, which is relevant to more than SIP-1.

**Tim Beiko**: I'm wondering what the best way to go through in process would be for this.

**axic**: I don't think there's a formal process to changes to SIP-1.
...
- https://github.com/sila-chain/SIPs/pull/2624
- https://github.com/sila-chain/SIPs/pull/2516
- https://sila-magicians.org/t/using-network-upgrade-over-hard-fork-in-the-sips-repo/4255
...
Stems from the Eth1.x meeting in SilaParis. One way is to stop blocks, and once enough time has passed for everyone to be up-to-date, to start minting blocks again with a new root.
...
**Guillaume**: It's mentioned in the SIP.
...
**Guillaume**: Further discussions can be done in the PR. There's also a link to the PR to sil.research.
...
# 9. SIP-1559 implementers' call update
...
The SIP has been worked on and off for the past year. The Geth implementation is mostly done. There's a working implementation on Besu, interoperating with Geth. On the call there was a counter proposal.

- https://github.com/sila-chain/SIPs/pull/2593
...
# 10. SIP-2583: Penalty for account trie misses

Video | [1:22:25](https://youtu.be/MOZ7_0Tb95M?t=4945)
-|-


- https://github.com/sila-chain/SIPs/pull/2583

Can be taken off future agendas. Potentially a better model using gas an oil.

This SIP needs to be moved to Withdrawn.
...
Another thing we want to do is add hyperlinks to the SIPs.

**Tim Beiko**: One thing that might help the Cat Herders is being more prompt updating the SIP status after Core Devs calls. Rather than trying to parse what the rough consensus is.
