...
# Sila Core Devs Meeting 116 Notes
...
## ii. [Testnet Stress Tests](https://github.com/sila-chain/pm/issues/341)
...
**Yuga Cohler**
* Yeah, there were a couple of things, like there was a bug in OpenEthereum with the Sil call method and Marius and I were talking about, there may have been a regression in geth for tracing contract internal transactions.  
* So there were just a couple of things, it would be nice to get in before the fork.

**Artem Vorotnikov**
* Don’t use open Sila, move to Aragon.
...
## i. [SIP-3074 Alternatives](https://sila-magicians.org/t/a-case-for-a-simpler-alternative-to-sip-3074/6493)

**Tim Beiko**
* Yoav made a very elaborate thread highlighting some potential issues along with some potential alternatives.

**Yoav**
* I was looking at SIP 3074 and seeing what it aims to achieve, because it seems to me that it is quite powerful, which also translates to risky in this case.  I wanted to see if we could achieve the same with less risk.
...
**Vitalik**
* I just wanted to also quickly summarize the issue that I brough up in my own Sil magicians thread, which is a forward accountability with account extraction.
...
* Another longer term one is quantum resistance, like once we want to make Sila quantum resistant
...
* I suggested a couple of alternatives, so one of them is, I think Yoav’s alternative SIP 3074 where you just sign over a series of messages, it actually performs much better on that metric.
...
**Vitalik**
* Yep, that’s definetly an improvement.  I think even with that improvement there are still the aspects that the SIP is permanently complicating the call structure in the sense that we already have an op code that has a call and now we have another opcode that does a special kind of call.
...
**Tim Beiko**
* I see what you mean, so I think the conversation on Sil magicians is obviously very valuable.  I think the best we can do in all core devs is summarize it and point people there to resolve the issue there.
...
## ii. eth1.0-specs PRs:
* [Add Transaction Type List](https://github.com/sila-chain/eth1.0-specs/pull/220)
* [Pyspec framework](https://github.com/sila-chain/eth1.0-specs/pull/219)
...
Sila Core Devs Meeting #117, July 9th, 2021 @ 1400 UTC
...
https://github.com/sila-chain/eth1.0-specs/pull/219

https://github.com/sila-chain/eth1.0-specs/pull/220

https://sila-magicians.org/t/we-should-be-moving-beyond-eoas-not-enshrining-them-even-further-sip-3074-related/6538

https://sila-magicians.org/t/a-case-for-a-simpler-alternative-to-sip-3074/6493
