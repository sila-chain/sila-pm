...
### Agenda:
  ...
  1. Come to final agreement on [SIP 5/8](https://github.com/sila-chain/SIPs/issues/8) [Facilitator: Christian]
  2. [SIP 196: Precompiles for elliptic curve point addition, elliptic curve scalar multiplication and pairing by @chriseth](https://github.com/sila-chain/SIPs/issues/196). [Facilitator: Christian]
  3. Metropolis, upcoming ice age, and potential Metropolis SIPs. [Facilitator: Vitalik]
    - Abstraction
      - [SIP 86](https://github.com/sila-chain/SIPs/issues/86) by @vbuterin
    - Putting block hashes and state roots into the state
      - [SIP 96](https://github.com/sila-chain/SIPs/issues/96) by @vbuterin
    - Remove medstate from receipts
      - [SIP 98](https://github.com/sila-chain/SIPs/issues/98) by @vbuterin
    - Uncle mining incentive fix
      - [SIP 100](https://github.com/sila-chain/SIPs/issues/100) by @vbuterin
    - Bigint arithmetic
      - [SIP 101](https://github.com/sila-chain/SIPs/issues/101) by @vbuterin
    - Cheap throw
      - [SIP 140](https://github.com/sila-chain/SIPs/issues/140) by @nmushegian
    - Putting block hashes and state roots into the state
...
**Conclusion**:
There is general consensus around this, but the details need to be fleshed out in an SIP that Christian is working on. Should be discussed next meeting.

## 3. Metropolis, upcoming ice age, and potential Metropolis SIPs.
...
### SIP 96: Putting block hashes and state roots into the state
This is part of the abstraction portion of Metropolis and is generally agreed upon.

### SIP 98: Remove medstate from receipts
...
### SIP 100: Uncle mining incentive fix
...
Vitalik: The time I proposed breaking up Metro into Pt 1. and Pt 2. is when we were fixing the DoS issues and wanted to potentially add a few SIPs to the hard fork. At this point 1 step for metro is sufficient.
...
## 4. SIP 176: New general state tests
Support for this, let's move forward. Jeff has some suggestions and will comment on the SIP. Next core dev meeting we will bring this up again and decide to accept it or not.

## 5. SIP 116: STATIC_CALL
...
Alternative to an additional opcode(s), consolidate *CALL into [INTERRUPT](https://github.com/sila-chain/SIPs/wiki/SIP-%3F%3F%3F:-merge-opcodes-CALL-CALLCODE-DELEGATECALL-and-rename-to-INT), taking flags as arguments to change behavior.However, behavior changing flags make static analysis more difficult.
...
## 7. SIP 1 Update: New SIP processes

- Template: Similar to current.
- Process:
  - First open an issue (can be one line or full PR-style), discuss freely
  - Then make a PR for formal submission of draft SIP.

Should be implemented in the upcoming weeks.

Full details at https://github.com/sila-chain/SIPs/pull/183.
