...
### [Github Agenda](https://github.com/sila-chain/pm/issues/203)
...
- [Contents](#contents)   
- [1. SIP Discussion](#1-sip-discussion)   
   ...
   - [1.8 SIP-1057 Next Steps.](#18-sip-1057-next-steps)   
- [2. SIP & Upgrades Updates](#2-sip-upgrades-updates)   
   - [2.1 YOLO / YOLOv2 & Berlin state tests update](#21-yolo-yolov2-berlin-state-tests-update)   
   - [2.2 SIP-1559 Update](#22-sip-1559-update)   
   - [2.3 Account abstraction: AA SIP and AA DoS study](#23-account-abstraction-aa-sip-and-aa-dos-study)   
...
## 1.2 SIP 2718: Typed Transaction Envelope (general-purpose standard for adding new transaction types).
...
There are no formats defined that defines an access list. This only is a format to provide extensibility in transactions. The access list SIP is 2930.

There is discussion about combining some SIPs into new transaction types, as there is overlap.

This SIP changes transactions to have an outer envelope, and a leading integer that denotes the type of transaction.

Traditionally, we've only had 1 transaction type. But we have a couple SIPs that want to introduce changes to transactions. Rather than guessing what transaction type it is by the elements, there may be errors, we specify the transaction type.
...
It may be best to have 2718 not bundled with other SIPs to get it released without much complexity. It may be good for YOLOv2.

There is specification for what to do with pending transactions on the fork block.

## 1.3 SIP-2929: Gas cost increases for state access opcodes.
...
This SIP may supercede other SIPs people have been trying to push historically.

**Alexey**: This SIP introduces a read list. Before we only had a write list. The correct solution I see is, yes increase the gas cost. But instead, introduce a specialized primitive so we can cache.
...
The SIP itself is not super complex. If we want to have another transaction type, this is the simplest one to include.

Geth would like to implement this, along with another client. Geth wants it in YOLOv2.

This SIP may be in YOLOv2, but not accepted yet.
...
## 1.5 General discussion on the idea of combining some of the above SIPs that create a new transaction type, so we just create a single new transaction type that has a whole bunch of the features together.
...
## 1.6 SIP-2935: Save historical block hashes in state.

Video | [57:40](https://youtu.be/-Jefyrs4f70?t=3460)
-|-

You stick the hash of the previous block into a storage slot before processing the current block, keeping a simple and dumb method of ensuring  blocks have a quick merkle path to all historical blocks since the SIP was introduced.
...
This SIP may go into YOLOv2.

Something to consider for YOLO version, clients have to implement all SIPs going into the testnet, or tney'll be kicked out. So, a lot of these SIPs going into YOLOv2 are very simple. This one is 2 lines of specification.

## 1.7 SIP-2711: Sponsored, expiring and batch transactions.
...
# 2. SIP & Upgrades Updates

Video | [5:29](https://youtu.be/-Jefyrs4f70?t=329)
-|-

A lot of questions about Berlin and timing for Berlin. Why the timing for Berlin hasn't been announced is that it's dependent on YOLOv2. For SIPs that go into YOLOv2, it is possible they go into Berlin, but it doesn't mean they are slated for the Berlin upgrade.

SIPs in YOLOv2 are considered for testnet first, then client integration testing, and that will determine its inclusion.

As for the name, it's not called Berlinv2, as SIPs that are in YOLOv2 may not go into a hardfork at all. Testnets after Berlin may be YOLOv3, or something else. YOLO is meant to show that the testnet will live for a short time.
...
EF is doing some simulations on the SIP. Looking at what happens when there's spikes. Next for having agents outbid each other.
...
## 2.3 Account abstraction: AA SIP and AA DoS study
...
Agenda: https://github.com/sila-chain/pm/issues/203
...
also, i went searching for the comment from Micah regarding sip-2930 and it not being a good fit for sip-2718, but couldn't find it -- could someone point it to me please?
...
My suggestion to that is if we have them in the same integration testnet, you get the optimization of work for the clients without having to bundle the decisions around them.So you don't need to combine the SIPs
...
Tomasz - I can provide the new expected gas costs in the ACD costs for SIP-2565oops in the ACD chat*Micah also had a few suggestions for the SIP before it is finalized and it may be a good idea to put the updated test vector results in there as well

From kelly to Everyone: (11:04 AM)

All test vectors for SIP-2565 are carried over from SIP-198 since SIP-2565 just changes the gas pricing formula
...
https://github.com/sila-chain/eth1.0-specs/projects/1Working from a project board here
