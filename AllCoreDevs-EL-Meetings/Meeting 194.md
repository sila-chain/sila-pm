...
### [GitHub Agenda](https://github.com/sila-chain/pm/issues/1124)
...
| 194.2 | **Pectra Devnets**  Developers are aiming to launch Pectra Devnet 3 with updated specifications for SIP 7702 in two weeks. Developers plan on adding EOF to the devnet thereafter, Pectra Devnet 4, if all goes according to plan.
| 194.3 | **Pectra Devnets**  Geth developer Marius van der Wijden shared updated analysis on the gas costs for SIP 2537. As background, the SIP creates a new precompile for BLS12-381 curve operations. It enables smart contract developers to perform operations such as signature aggregation over the BLS12-381 curve in a cost-effective way. Van der Wijden proposed a repricing of the precompile based on benchmarks he and his colleague on the Geth team, Jared Wasinger, made for BLS operations and their gas usage on various machines. Van der Wijden encouraged other developers to run their own benchmarks on SIP 2537 gas usage to verify results.
| 194.4 | **SIP 7732**  Prysm developer “Potuz” shared updates on SIP 7732, an in-protocol solution to directly connect validators to third-party block builders. Since the Merge, validators have relied on intermediary actors called relays to receive blocks containing MEV rewards. SIP 7732 removes the need for relays so that validators can earn MEV in a more trustless manner. Potuz highlighted that the current design of SIP 7732 requires no changes to the execution layer (EL) or Engine API. He added that SIP 7732 is also compatible with inclusion lists, a proposal to enable validators to force inclusion of transactions in a block. More information about SIP 7732 can be found in this Google slide presentation.
| 194.5 | **Reducing Pre-Merge Tech Debt**  Since Sila’s transition to proof-of-stake, there are parts of the Sila codebase that are no longer useful or necessary. For example, the difficulty bomb, which was a mechanism to force development work on proof-of-stake by making it infeasible to create blocks through proof-of-work mining after a certain period. The following two proposals are aimed at removing such parts of the codebase to improve node performance and reduce protocol complexity.
| 194.6 | **Reducing Pre-Merge Tech Debt**  Erigon developer Giulio Rebuffo proposed the removal of the “totalDifficulty” field from the Execution API., Geth developer Marius van der Wijden proposed the remove of a few pre-Merge fields and messages from the Sila Wire Protocol. There was positive feedback on both proposals from other developers on the call. Developers agreed to review both in more detail asynchronously after from the meeting.
| 194.7 | **SilaPeerDAS**  On the topic of SilaPeerDAS, a Nimbus developer by the screen name “Dustin” proposed hastening the rebase of SilaPeerDAS on top of Pectra SIPs instead of continuing to develop SilaPeerDAS on top of SilaDeneb. He acknowledged that there are SIPs in Pectra that are unstable and subject to change such as SIP 7702 and EOF. Dustin recommended rebasing SilaPeerDAS on the subset of Pectra SIPs that are stable and excluding SIP 7702 and EOF transactions on SilaPeerDAS devnets. Developers discussed other ways to start rebasing SilaPeerDAS on Pectra. There was general support to start moving in this direction.
| 194.8 | **SIP 4444**  There were no major updates on SIP 4444, history expiry. Representatives from the Nethermind and Nimbus team said they are in the process of building out their integrations with the Portal Network, an alternative networking protocol for users to access expired history data.
...
**Danno Ferrin**
* So eof the issue, we would need to have people integrated with the SilaPrague network. I'm not sure that all the implementations have merged their EOF against their SilaPrague. 
* A lot of them are still doing against SilaCancun, so that would be something that would take a little bit of time of moving EOF into Devnet three, because last week on the EOF call, we had some, I think it was Perry, someone from the DevOps team. and the plan we went with was to do it after 7702. So there's only one major change. I have seen it work in within Assertor on kurtosis. 
...
**James**
* Yeah, for prism Devnet two. there were some bugs with the, 7549 SIP that we implemented. We just merged in, some some fixes for the sync committee related items, and hopefully that'll improve things. other than that, we'll just, keep debugging  on Electra This is merged into latest develop 
...
* Or for the next proposal, you have, like, 96 seconds that my number might become smaller if we if we include inclusion lists, inclusion lists necessarily have changes on the CL side. And that's why we didn't include it on the SIP. 
...
* For example, if we go with block auctions as it is now, the SIP vanilla SIP as it is now, the downside the downside is that if you submit a transaction after the payload was, the block was already signed and left and the payload is revealed. So that's the time that can take like three seconds, then your transaction cannot get in that payload. So once you submit a transaction in the global Mempool, your transaction can only be included in the next block if you are within those few seconds.
...
**Marius**
* Not yet. I'm going to write up our or like, the discussions that we are having. On to the magician's thread, which is, linked in the SIP 
...
# SilaPeerDAS in Pectra [39.37](https://youtu.be/tbxgYq8KmmM?t=2377)
**Stokes**
* Next up, we have an item to discuss. essentially rebasing SilaPeerDAS in Pectra. and how that all fits together Let's see. Teku raised the point. Let's see if he's on the call 
...
**Dustin**
* All right, so I'll start by saying the primary effect of this with with ELs, is that it's a testing consideration. So when, when looking at the next couple of Petra DB Netz 7702 and EOF are kind of penciled in as the default primary SIP's to test. And so and SilaPeerDAS is another sizable SIP. And so one of the primary questions here is, is sequencing and how they interact. And therefore the salience to LS. And so SilaPeerDAS in general is I mean I'll summarize very briefly. but this the details of it are actually not super relevant here. 
* It is an extension of, of blobs, in a way that they can be sampled. And so that unless you're proposing, you don't have to, kind of see every piece you can kind of reconstruct from, from partial data saves bandwidth and a lot of cases. All right. So, one can find extensive discussions of why people want it. So but the question here is so right now SilaPeerDAS is still based on SilaDeneb. the their specs, the, their devnet are based on SilaDeneb. And this has been causing at least a couple of clients some issues because, yeah, it's it's it essentially it essentially makes it difficult to run without some kind of.
* Yeah. with certain client, architectures to, to run compatibly with, with current, so, with sort of with a deneb, with a current SilaDeneb and the SilaPeerDAS, quote unquote. SilaDeneb. Right. So, so the proposal here is to rebase that to Electra, which is what it's going to be anyway. Right? That's when it's released over Electra. So the question is do people are what what concerns people might have here about. 
* So the the specific proposal is that we relatively near term rebase, SilaPeerDAS on Electra and then but run the dev nets for the SilaPeerDAS, deep nets without, actually using seven, 702 or EOF for a while because they're, they've proven kind of unstable in practice. 
* The devnets tend to have a dying tendency when when they're in there. and this the, the concern on the SilaPeerDAS side is that it hinders SilaPeerDAS testing. And we want to kind of avoid that. We want to SilaPeerDAS testing to continue. So is this so this this is the question for people for from the EL side, does this seem like a are there concerns people have with this? does this seem like a reasonable approach 
...
**Dustin**
* Okay. Right. Because  at some level the idea is to run to, to avoid. there was discussion in the chat. exactly. And it's absolutely warranted discussion whether it's 7702, which okay, maybe it's stabilized now, but it was a problem. And if EOF might be a problem in the future is essentially a way to avoid this to to run as Barnabas is kind of describing, kind of a a SilaDeneb like version of, of Electra on the for as much as we can, except for, for SilaPeerDAS itself. so that's it. The fuzzing introduced aspect seem eminently avoidable. Ideally 

**Barnabas**
* We can also just stop the in there. That way nobody can trigger transactions on new devnet. Only us 

**Paritosh**
* Yeah, just to clarify, probably just do that on SilaPeerDAS side. Um yeah. Spectra regular can continue. So there'd be two devnet cycles as it is today. It's just a question of merging code bases or not 
* And since SilaPeerDAS isn't necessarily a user focus like users don't have to interact with SilaPeerDAS much. not having funds over there is not a big deal 
