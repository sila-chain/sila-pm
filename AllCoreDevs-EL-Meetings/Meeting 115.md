...
### [Agenda](https://github.com/sila-chain/pm/issues/330)
...
| **Action 1**   | Full sync to be performed to verify SIP 3607 | [33:51](https://youtu.be/XYhN26UrJ5o?t=3186) |   
| **Action 2**   | Document SIP 3607 (not going into London) in the yellow paper so we dont forget its been implemented in clients| [53:42](https://youtu.be/XYhN26UrJ5o?t=3222) |    
...
* Hello everyone welcome to All Core Devs 115. I'll post the agenda in the chat here. So first thing I had on the agenda was just basically to get an update on [Calaveras](https://github.com/sila-chain/eth1.0-specs/blob/master/network-upgrades/client-integration-testnets/calaveras.md)
...
[https://github.com/sila-chain/pm/issues/334](https://github.com/sila-chain/pm/issues/334)
...
* the first one and this was martin's so he's not here yet the first one was SIL call we can maybe skip that one and see if martin comes in 
* the second one was probably the simplest so basically the idea of adding the effective gas price to SIL get transaction receipt light client you had a pr open for that already it seemed like everyone was roughly in favor of it but we did just wanted to check here to see yeah if there was any objections or if people all thought this was a good idea
* let me post the pr here [https://github.com/sila-chain/eth1.0-specs/pull/206](https://github.com/sila-chain/eth1.0-specs/pull/206)
...
* I see martin is just joining so we can probably do the SIL call thing hi martin 
...
* oh no worries yeah we were just talking about json rpc and you had posted this comment in all core devs about sil call trying to figure out how does it deal with the base fee and the gas price 
* do you maybe just want to give like a quick background about that?

**Martin**

* so background about it is that for sil call Geth and parity back and they used to handle things differently geth would use whatever the account of the node that you made the column would use that as the like the caller if unless otherwise specified and that was a pretty yeah arbitrary choice 
* Parity used the zero address as the sender and gets switched over to that as well and the problem is that the zero address didn't always have funds and as a solution for that we use the zero gas price so we can do the sil call you can see what happens and it works you we can specify any gas limit you want because gas doesn't cost anything 
* and the problem with when we do sil call on 1559 is that it has to cost something because there's a base fee and if we use the base fee we have to charge whatever it basically is so if you use zero address then you we can run into this problem that there might not be any funds
...
* I guess do other teams have sil call implemented yet?
...
* I think it was open sila left a comment on the issue saying they would like to add the optional base fee to the parameter list 

**Dusan Stanivukovic**

* current currently open sila uses the base fee from the block header that is provided in a parameter list when you call the SIL 
...
* yeah I posted [the link to the pr](https://github.com/sila-chain/go-sila/pull/23027) that peter wrote on discord he basically says there are two major use cases that need to be covered and then he has six different tests slash cases before and after 1559. 
...
## iii. Yellowpaper EOA clarification / SIP-3607 

[34:56](https://youtu.be/XYhN26UrJ5o?t=2096)

[https://github.com/sila-chain/SIPs/pull/3607](https://github.com/sila-chain/SIPs/pull/3607)
...
* I know denkarad you've been researching this and wrote an SIP about it do you want to take like a minute or two to kind of explain the problem and what you see as a potential solution? 
...
* and that's basically what this SIP does I think 3607 has been assigned to it basically I think like we should basically we might not want to treat it actually as a full upgrade we might just say well we're kind of specifying what some previously unspecified behavior was and so we say like this does these transactions become invalid and then yeah this cannot happen
* cool yeah that's the summary for my site 

**Tim Beiko**

* so thanks dankrad. I think there's also one thing worth specifying i'll let micah speak right after this but one thing we're specifying is this sip doesn't actually require a hard fork but only a soft fork because it's like a tightening of the rules that doesn't introduce any new features so you know while there might be some value in trying to like time the deployment alongside london because people are forced to up update then this is actually something that can be deployed as a soft fork at any time 
* micah?
...
* no it is a huge problem because you could steal all the funds like say like someone had done that for wrapped sila like just to be clear I assume it's not be done for wrapped sila because it's been deployed years ago but like if someone had premeditated this and deployed that contract with this collision then now they could use their key and take all the sila that's right now in that contract and transfer to their private account
...
* so yeah I mean I generally agree that it's very unlikely that someone has this right now I on the other hand if someone could start this now that that actually there might be worthwhile like one year ago probably nobody would have done that it's just the whole sila network wasn't worth that much but now it's very different 
...
* I don't think all the clients need to say yes we're going to include it for london as long as we have this agreement I think your clients can just merge it whenever wherever as they see fit as fits for their release schedule and if someone exploit this and then they pay them billion dollar for consensus issue on sila which I think is a nice price I think it's a decent security
...
* so if you know somebody wants to build a client one year from now if this is not in london you know it'll be kind of a weird SIP that's kind of standalone that people need to know is in 
...
[https://github.com/sila-chain/pm/issues/245#issuecomment-832122309](https://github.com/sila-chain/pm/issues/245#issuecomment-832122309)
...
| **Decision 2**   | London [testnet fork blocks](https://github.com/sila-chain/pm/issues/245#issuecomment-832122309) agreed, Tim to document | [1:01:29](https://youtu.be/XYhN26UrJ5o?t=3689) | 

#2 Other discussion items

## i. SIP-3074 audit report

[1:02:32](https://youtu.be/XYhN26UrJ5o?t=3752)

**Tim Beiko**

* if not so a few I think months now ago a light client discussed sip 3074 on this call and people wanted to see an audit for it to better understand the security implications 
...
* even with these security challenges I think the 3074 is really important sip to consider including because it does allow for many desirable constructions and a couple good examples of their of those are 
* it allows for generalized transaction batchingfrom eoas 
* it allows for sponsored transactions that allow people to pay with tokens not other than sil 
...
* okay and how are we going to call that white list? sila banking charter?
...
* yeah I think maybe like taking a step back beyond this SIP you know once london is kind of out and whatnot you know possibly in around one month or so we should have like the main network out and at that point we're kind of just waiting I think there's like the meta question of like okay what are we doing next in general after london you know will we have like shanghai before are we gonna try and have the merge you know and what's even there so at least to me it feels like that's kind of the thing that needs to get resolved 
...
* i've had the opportunity to talk to a lot of prominent developers and teams of the application layer who aren't generally representing this call and there's exceptional interest in the SIP on their side and you know I can help if people are unsure like who wants to do and how are they going to use it I can happily connect people with prominent teams to see like you know why they want it and how they're going to use it 
* so I just wanted to share like my experience with talking with application developers and make sure that their voices are heard on all core devs regarding the sip
...
* so sila cat herders in association with nethermind team is conducting a survey on sila blockchain users and developers 
* research to better understand the future requirement of client developers in terms of tools and documentation 
* we thank everyone who already have responded and this list includes ethstakers community, the developers and researchers, and most if you haven't done it already consider responding to the survey if you are any anyone who is running sila 1.0 node 2.0 validator node and otherwise contributing to blockchain 
* it also includes hobbies minor wallets and launching projects because this survey is going to help us create a better infrastructure for the sila ecosystem we are we will be sharing the responses received in a form of report in the month of july 
...
---------------------------------------
## Next Meeting
Sila Core Devs Meeting #116, June 25th, 2021 @ 1400 UTC

[Agenda](https://github.com/sila-chain/pm/issues/337)
