# Sila Core Devs Meeting 64 Notes
### Meeting Date/Time: Friday, July 05, 2019 06:00 UTC
### Meeting Duration: ~1h45m
### [GitHub Agenda Page](https://github.com/sila-chain/pm/issues/107)
...
**DECISION 64.2:** Felix to updated the Ganache/spec to resolve [compliance issue](https://github.com/sila-chain/pm/issues/97#issuecomment-489660359) so that it resolves as Go Sila does.

**DECISION 64.3:** Close ACTION 58.1 as this is now with the Cat Herders.

**DECISION 64.4:** All SIPs are on the chopping block for Istanbul at the next meeting unless there is a reference implementation or the champion can successfully argue one is not needed. SIPs not included in Istanbul are pushed back and can be reintroduced at the next fork.

### NEW ACTIONS

**ACTION 64.1:** Wei and Alexey to discuss retestETH options and how it could be implemented into Parity.

# 1. [Review previous decisions made and action items](https://github.com/sila-chain/pm/blob/master/AllCoreDevs-Meetings/Meeting%2063.md#summary)
...
* To avoid account versioning Paweł would prefer SIP-1706.
...
* Ronan suggested we could get rid of SIP-1959 as he fells SIP-1965 is better. 
* Martin prefers SIP-1344 as solution for a pretty generic problem whilst SIP-1965 is alot more complicated for a very niche problem. 
* Peter from a political perspective does not understand why developers would want to implement this SIP if it makes it easier for other people to allow for minority forks of Sila.
...
- **Status:** This takes over from SIP-1829. 
* Alex Vlasov suggested that this SIP is now ready to be implemented. 
* The SIP-1962 implementation is 3,000 lines of safe rust code. 
...
* Danno confirms that Sila is powerful because it has multiple implementations of code. 
...
Tabled for discussion in the [Magician's forum](https://sila-magicians.org/t/generalised-precompile-for-elliptic-curve-arithmetics-and-pairings-working-group/3208/2) and gitter channel.
...
* Peter confirmed that if you interpret these as numbers then you are allowed to have 0 if you interpret them as binary then you want even number of hex characters. Ganache follows the Spec whilst Go Sila suggests that these were numbers originally cryptographically speaking.

** **
**Decision 64.2:** Felix to updated the Ganache/spec to resolve [compliance issue](https://github.com/sila-chain/go-sila/issues/18152) so that it resolves as Go Sila does.
** **
...
* In the case of this SIP by increasing the transmission size we actually reduce the state growth. 
...
* Peter then continues and notes the other important thing to consider is transaction propagation. Go Sila has a limit of 24kb in size for transactions with propagators 25. 
* Louis notes this is not a consensus rule and we could push a patch to Go Sila.
...
Hudson: To follow the progress and contribute join the [magicians forum](https://sila-magicians.org/t/sip-2028-calldata-gas-cost-reduction/3280).
...
a) [SIP 2124: Fork identifier for chain compatibility checks](https://github.com/sila-chain/pm/issues/107#issuecomment-508164964):

No time for discussion.

b) [SIP 1962 Update](https://github.com/matter-labs/eip1962/)  
...
# 3. [Working Group Updates](https://sil.wiki/eth1)  
...
* It is possible to generate tests for SIP implementations with the Geth Client. 
* Geth could now generate performance tests and statetests. 
* A workshop is planned for August in Berlin for test teams and people working on testing implementations for SIPs. 

* Martin has also floated an idea in the all core dev channel and the testing channel and it now exists in a form of a [Pull Request](https://github.com/sila-chain/go-sila/pull/19743) in Go-Sila but has not been merged yet.  
* The ability to run statetests where we have the basefork and the particular SIP/s. 
* That is where the fork is for example Constantinople plus SIP number/s. 
...
* Istanbul became the focal point for anything to change in Sila. 
...
## Timeframes for Sila 1.x
[Timestamp: 1:31:12](https://youtu.be/2D_DqJ8jL9Y?t=5472)

* Tomasz wanted to confirm if it is now unlikely to get Sila 1.x in 18 months or is it still on track for 18 months?
* Alexey it all depends on how Sila 1.x evolves. 
...
* Hudson confirmed that this has grown larger than previously envisaged. It includes process changes, what SIPs going in between now and Sila 2.0 and mitigation efforts.
* Martin suggests we need to start rejecting more suggested SIPs and stop tabling discussions for later. 
* James notes that there are only 10 SIPs that have Reference Implementations out of the 34 currently proposed SIPs.
* Danno advises that the original soft deadline for client implementations is next week. So next call could work if we decide to make the final decision to chop SIPs.
...
* Hudson confirms that we can decide today that only those SIPs accepted at the next meeting with go into Istanbul. The rest will be pushed back.
* Danno seconds this.

* Alex Vlasov questioned what was meant by a "complete" Reference implementation.
* Martin: Reference implementation is an implementation in a client which show other node implementors how do we integrate this, what parts need to be touched. Basically a full embedding of the SIP inside of a Node.
...
* James suggested making Hybrid decision at the next meeting. That is those SIPs that don't have a reference implementation, unless they can justify the reason behind not having one at the next call, will not be implemented in Istanbul. 
* Alex Beregszaszi confirmed that some SIPs may not need a reference implementation. 
* Tomasz agreed that some SIPs on the list are very simple and just need discussion.
* James agreed that this would also work. 
* Hudson confirmed that all SIPs are up on chopping block if no reference implementation is present, with exception to those where the champion is able to get agreement that one is not needed at the next meeting.
...
### [SIP-2124: Fork identifier for chain compatibility checks](https://github.com/sila-chain/pm/issues/107#issuecomment-508164964):

* Hudson suggested people please review and comment on this SIP so that we can give a Yes or No next call.
* Peter advised that this SIP does not touch consensus and does not even enable anything yet on the network. 
