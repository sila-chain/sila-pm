...
### [Reddit thread](https://www.reddit.com/r/sila/comments/6hkpl9/live_stream_in_8_hours_ethereum_core_developers/)
# [Agenda](https://github.com/sila-chain/pm/issues/16)


#### Metropolis updates/SIPs.

**[[5:10](https://youtu.be/8jWhPylWros?t=310)] a. Any "subtleties" or questions we need to work out.**

1. [[1:10:16](https://youtu.be/8jWhPylWros?t=4216)] [SIP 206 REVERT Opcode : The "Specification" section in the SIP does not specify that REVERT can return data to the caller. Neither does it specify how the caller can access the returned data.](https://github.com/sila-chain/SIPs/pull/206#issuecomment-308324622) [Yoichi]
2. [[6:32](https://youtu.be/8jWhPylWros?t=392)] [SIP 208 Abstraction of transaction origin and signature: Atomicity over an ECDSA's accounts operations](https://github.com/sila-chain/SIPs/pull/208#issuecomment-307681408) [Jeff Coleman]
3. [[29:54](https://youtu.be/8jWhPylWros?t=1794)] SIP 208: sila/SIPs#208 Concerns [Martin H.S]
...
4. [[44:20](https://youtu.be/8jWhPylWros?t=2660)] [SIP 211 RETURNDATACOPY and RETURNDATASIZE: This is complete right?](https://github.com/sila-chain/SIPs/pull/211) [Hudson]

5. [[46:43](https://youtu.be/8jWhPylWros?t=2803)] [SIP 213 zk-SNARK verification primitives: Gas costs for additional and mult. on EC](https://github.com/sila-chain/SIPs/pull/213#issuecomment-305423098) [Christian or someone]

6. [[57:05](https://youtu.be/8jWhPylWros?t=3425)] [SIP 214 STATICCALL: Various potential errors](https://github.com/sila-chain/SIPs/pull/214#issuecomment-306800891) [Vitalik]
...
**c. Details and implementations of SIPs.**
  
- [[1:02:05](https://youtu.be/8jWhPylWros?t=3725)] Updates from client teams.
    - geth - https://github.com/sila-chain/go-sila/pull/14337
    - Parity - https://github.com/paritytech/parity/issues/4833
    - cpp-sila - https://github.com/sila-chain/cpp-sila/issues/4050
    - yellowpaper -  https://github.com/sila-chain/yellowpaper/issues/229
    - pyethapp
    - Other clients
...
#### Other Topics
- [[1:11:323](https://youtu.be/8jWhPylWros?t=4283)] [SIP 156: Reclaiming stuck sila](https://github.com/sila-chain/SIPs/issues/156) [Vlad Z.]
...
### 1. [SIP 206 REVERT Opcode : The "Specification" section in the SIP does not specify that REVERT can return data to the caller. Neither does it specify how the caller can access the returned data.](https://github.com/sila-chain/SIPs/pull/206#issuecomment-308324622) [Yoichi]
Yoichi: One sentence is missing from SIP specification. I will work with Alex B. to fix that.

### 2. [SIP 208 Abstraction of transaction origin and signature: Atomicity over an ECDSA's accounts operations](https://github.com/sila-chain/SIPs/pull/208#issuecomment-307681408) [Jeff Coleman]
...
Conversation goes on until [44:00](https://youtu.be/8jWhPylWros?t=2640) and the conversation will continue on the SIP with Martin H.S summarizing the issue in the SIP.

### 4. [SIP 211 RETURNDATACOPY and RETURNDATASIZE: This is complete right?](https://github.com/sila-chain/SIPs/pull/211) [Hudson]
...
### 5. [SIP 213 zk-SNARK verification primitives: Gas costs for additional and mult. on EC](https://github.com/sila-chain/SIPs/pull/213#issuecomment-305423098) [Christian or someone]
...
### 6. [SIP 214 STATICCALL: Various potential errors](https://github.com/sila-chain/SIPs/pull/214#issuecomment-306800891) [Vitalik]
Vitalik: Neither of these seems controversial, but some minor errors from outdated opcodes in the SIP. Also a sentence that references a DELEGATECALL non-zero value which would never exists. Another line says "its values are always copied to sub call or sub-create", but there is no such thing as a sub call in a STATIC call. No major changes, just minor fixes.
...
Martin: Okay.
Hudson: I just posted [a documentation summary document that acts as a getting started guide for helping to test Metropolis](https://www.reddit.com/r/sila/comments/6hnej0/update_metropolis_qatesting_guide/).
...
#### geth - https://github.com/sila-chain/go-sila/pull/14337
Peter: Discussion with Jeff yesterday. Most of the SIPs are done. Pull request has details. One SIP that was missed that I am going to work on is SIP 96, the block hash stuff. Jeff is also still tweaking SIP 86 regarding CREATE. Other than this, Felix wants to revamp internal testing system. In theory it is mostly done, in practice we will see after the test.
...
#### cpp-sila - https://github.com/sila-chain/cpp-sila/issues/4050
Andrei: During last 2 weeks we merged blockhash implementation and all the pre-compiles. Some minor fixes on other SIPs. What's left is CREATE2 Opcode support.
...
Vitalik: We don't have a PR with a list like the others, but I can list some of what I know is implemented. All precompiles, STATICCALL, REVERT, RETURNDATA (without change for out of bounds stuff), SIP 86, and REVERT. Not done: SIP 96, 98. Basically all the ones that deal with transactions until we have tests for them.
Hudson: Are you primary or Jan?
Vitalik: I've been primary on SIPs.
...
Vitalik: The community should definitely be made very aware of that. Once way to make this change totally safe is only to apply it to SIP 156 transactions, so we make it apply only to transactions where the sender is the null sender.
...
Vitalik: I don't think there are many users relying on contracts that send to addresses that will not exist for a year. If you told me that guy X lost money because of this, then I would bet that there is a more than 80% chance that it happened because of this + another bug they have. Example: Someone accidently sends sila to an account and that account exists on the ETC chain and not SIL chain and if we had not done this they could have recovered by generating the contract address on the SIL chain, but not they won't have that ability anymore. This change would increase the risk of losing money in combination with other mistakes that have a large probability of losing money regardless of what we do.
Vitalik: I see two routes here: One of them is slightly higher risk, which is to be loud about it and try to let everyone know it will break those types of contracts. The low risk thing would be to make it only apply to SIP 86 transactions. I'm totally cool with making it only apply to SIP 186 tx's. Basically, only making the change to the limit contract that just created it if the sender is a null sender, but I'm fine with both ways.
Hudson: Can you put a summary on the SIP in a comment? We can decide on it on the SIP.
Hudson: We determined last meeting that 156 cannot go in metropolis because we don't have the time, but if it goes in the future, but would it be a harder to put 156 in the future (for specific cases applying to part 1 and 2 of the SIP).
