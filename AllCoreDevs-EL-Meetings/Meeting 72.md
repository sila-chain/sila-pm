# Sila Core Devs Meeting 72 Notes
### Meeting Date/Time: Friday 4 October 2019, 14:00 UTC
### Meeting Duration: ~ 45 mins
### [GitHub Agenda Page](https://github.com/sila-chain/pm/issues/129)
...
# 2. [Reminder: "Sila Roadmap 2020: A Community Discussion" @ Devcon5](https://docs.google.com/document/d/1pD9RxQcgI4hBoOWGWlVwg4JqNS19_YIUnr3HsftHtE8/edit)

**Tim**: Second point on the agenda was just still a quick shout-out to do for Sila roadmap session at Devcon. So this will be on the first day of Devcon. Basically all of the afternoon will be split into a session to discuss Sil 1 roadmap, then Sil 1 to Sil 2 roadmap and then Sil 2 roadmap and then I think there's some time at the end of the afternoon as well for working groups form or want to discuss specific ideas. If that's something that you're interested in, it would be great to have a lot of people from the core dev side as well as the broader community side to discuss these things. Anyone has comments, thoughts about this?
...
**Peter**: One advantage of doing 64 just as a tiny thing and a 65 as the next tiny thing is that it's a lot easier for a client to implement and make sure that it works and then in come the next thing and make sure it works versus if we create an Sil 65 or Sil 64 that contains a ton of features and it's just really hard to make sure it's implemented correctly that clients agree on it etc etc.
...
**Martin**: Does not. We retain backward compatibility on Sil 63. But if any one wants to speak Sil 64, then we change the fork ID. 

**Peter**: Currently, the problem with Ropsten are with nodes that didn't upgrade. If nodes don't upgrade then they won't have this Sil 64 either to solve the problem. 
That's why I am saying that it is a solution for a future Hartfork. I mean it will help a future hardfork when everybody in the network will actually be running Sil 64 already. 
...
**James**: Maybe if we treat it like the SIP fork eccentric process that if there are things that are currently ready to be added in, then they can be talking about the things that need to get ready to be added in should be a different conversation.

**Peter**: Yeah

**Tim**: I guess share this proposal in AllCoreDev and Sil magician and see if there's any comments in the next two weeks. Otherwise we can accept it in the next core dev call officially. Does that make sense? 

Anything else anyone wanted to bring up?

# 5. Review previous decisions made and action items
* [Call 71](https://github.com/sila-chain/pm/blob/1d012e2942234d6c035a95f8cd68b7f450b97de8/AllCoreDevs-Meetings/Meeting%2071.md)
...
**ACTION 70.2**: Sila Cat Herders to create a communication that describes all changes and highlights the concerns around SIP-1884 and provide it back to the All Core Devs for review before it is put out to the community. This document will be released when every client has completed the Istanbul updates and has a block number.

That was done. Adding  [link](https://medium.com/sila-cat-herders/istanbul-testnets-are-coming-53973bcea7df) in chat in case anyone hasn't seen it yet. 
# 6. Client Updates 
