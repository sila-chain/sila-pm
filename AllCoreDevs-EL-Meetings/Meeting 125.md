...
### [Github Agenda](https://github.com/sila-chain/pm/issues/401)
...
* yeah it doesn't seem super valuable to be in there although if you look at this pr the vip block time target is essentially a proxy for slots per second and so if it makes it into this sip then it is making its way into the configuration there 
...
* cool and so the reason why we brought this to the call now is that what we want to suggest is that we we basically have created an idea for a roadmap how we how we make sila stateless and the idea is this the idea is to spread the changes over three different hard forks and at the end of it well actually like if sila would not be stateless in itself but but it would would gains optional statelessness and i will explain in a minute what that means
