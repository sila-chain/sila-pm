...
### [Github Agenda](https://github.com/sila-chain/pm/issues/219) <!-- omit in toc -->
...
- [1. YOLOv3 & Berlin discussion](#1-yolov3--berlin-discussion)
  ...
  - [1.2 Discord conversation](#12-discord-conversation)
    - [Decisions](#decisions)
  - [1.3 SIP-2537 fuzzing update](#13-sip-2537-fuzzing-update)
- [2. Other updates / discussion](#2-other-updates--discussion)
  - [2.1 SIP-2938 (Account Abstraction)](#21-sip-2938-account-abstraction)
  - [2.2 Ropsten issues](#22-ropsten-issues)
  - [2.3 SIP-2666](#23-sip-2666)
- [Annex](#annex)
...
Merged, not syncing. Some failure on the Besu Nethermind SIP-1559 equation. For Berlin YOLOv2, will join as soon as Geth and Besu has a running version, which should be done for 2929, so soon. Still looking for what's being added. 2935, 2666. Trying to build and connect library to support modex repricing. 
...
1. Do we agree that successful YOLOv3 SIPs can go into Berlin?
...
Once gas is redone, there needs to be benchmarking. Alex Vaslov will merge an update to the SIP to have a new commit hash for YOLOv3.
...
**Hudson Jameson**: Not to take all the time on this topic. The question wasn't if EVM384 is a good idea. But if EVM384 should replace SIP-2537 in Berlin. Otherwise, Berlin could be pushed well into next year if more discussion keeps happening. Personally, I think we should have both of them in. 
...
**Hudson Jameson**: Also, Open Sila may have an audit for this SIP.
...
## 2.1 SIP-2938 (Account Abstraction)
...
May review in a breakout room in Sil research discord. 
