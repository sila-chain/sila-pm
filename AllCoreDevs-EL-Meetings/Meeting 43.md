# Sila Core Devs Meeting 43 Notes
### Meeting Date/Time: Fri, July 27, 2018 14:00 UTC
### Meeting Duration: 1.5 hours
### [GitHub Agenda Page](https://github.com/sila-chain/pm/issues/51)
...
* Client Updates
* Research Updates
* Four competing SIPs to delay the difficulty bomb and/or reduce the block reward:
...
* Dimitry update
    ...
    * CREATE2 opcode is about to be added to cpp-sila, I will start writing some tests
...
* List of clients (from Afri)
	* https://github.com/sila-chain/go-sila (golang)
	* https://github.com/paritytech/parity-sila (rust)
	* https://github.com/sila-camp/sila-harmony (java)
	* https://github.com/sila-chain/cpp-sila (c++, mainly testing)
	* https://github.com/sila-chain/pyethapp (python, deprecated)
	* https://github.com/sila-chain/trinity (python, wip)
	* https://github.com/sila-chainjs/ethereumjs-client (js, wip)
...
* geth (Peter)
    * Constantinople hard fork implementation update
        ...
        * testing pending
        * blockhash SIP (210) - would be good to know whether we go with original blockhash SIP or simplified version that V suggested
...
* cpp-sila (Pawel)
    * Constantinople hard fork implementation update
        ...
        * EXTCODEHASH in progress
            ...
            * But definition of existing account may not be very specific within Sila spec
...
* Matt: we can find out about specific tx at block 2.67M in the yellow paper
    * But is actual behavior specified in an SIP? Finalized?
...
* Nethermind
	...
	* Full dot not Sila client (unlike Nethereum which is an integration library https://nethereum.com/)
...
* EthereumJS (Holger posted comment)
    	* https://github.com/sila-chain/pm/issues/51#issuecomment-408140132
	* We have now a first release series v2.4.x out where we introduced partial support for Constantinople (bitwise shifting instructions, SIP 145), see the according announcement post on Reddit.
	* Work on other SIPs hasn't started yet though. I will make a follow-up Reddit call for participation tomorrow, since we currently don't have the people power to implement all ourself.
	* If anyone is interested in helping head over to our VM repository. I'll also write up issues tomorrow on the Constantinople SIPs with some instructions on where to start.
	* Reddit followup call for participation, don't have enough people to implement it all

# Research updates
* Shasper v 2.1 (Danny)
    * updated casper+sharding v2.1 https://notes.sila.org/SCIg8AH5SA-O4C1G1LYZHQ#
...
* SIP 1087: Net gas metering for SSTORE operations.
    * Peter update on geth implementation
        * See PRs listed here: https://github.com/sila-chain/pm/issues/51#issuecomment-407366399
        * Crux of the SIP: Instead of charging a lot, we only charge a small amount for each operation, and only charge a large amount if we end up modifying the state
    ...
    * Hudson: can we ask Parity/Harmony to implement it by the next call to see how easy or hard this will be? yes (both agree)
        ...
        * Peter: geth implementation is pretty clear, it may be useful to review it as a reference for your codebase (https://github.com/sila-chain/go-sila/pull/17208)
    * Hudson: I'll try to track which implementations have finished which features for the meta SIP
    ...
    * Dimitry, any opinion from testing perspective?
        ...
        * Nick: I added a list of suggested test cases to the SIP
...
* Blockhash refactoring (EIP210)
    ...
    * Peter
        * Before going ahead with this SIP it would be nice to have a reason for going ahead with it
    ...
    * V: I can add examples to the SIP
    * Hudson: Let's discuss more about 1218 at the next meeting, not going in for now until we have more info

# Potentially lowering the cost of pre-compiles (bnAdd?, bnMul?)
* See Martin's analysis: https://github.com/sila-chain/benchmarking/blob/master/constantinople/analysis2.md
* Martin
...
* V: I did write an SIP for reducing gas costs, small and simple
* EIP1187, 1108 (see this update: https://github.com/sila-chain/pm/issues/51#issuecomment-408383462)
    * https://github.com/sila-chain/SIPs/issues/1187
    * https://github.com/sila-chain/SIPs/issues/1108
        * Merged as a draft
...
* Four SIPs to delay or remove difficulty bomb/reduce block reward
    a. SIP-858 - Reduce block reward to 1 SIL per block.
    b. SIP-1227 - Delay bomb and change rewards to 5 SIL.
    c. SIP-1234 - Delay bomb and change rewards to 2 SIL.
    d. SIP-1240 - Remove the difficulty bomb entirely.
* Does anyone want to remove difficulty bomb completely?
    ...
    * Hudson: We have consensus we don't want to remove it entirely
        ...
        * SIPs do a combination of delaying the bomb and reducing the block reward
        * I think we should decouple them: one is economic/technical, the other is pure technical
    * Hudson: re: reducing reward, proposals to reduce to 1 SIL, 2 SIL, 5 SIL (it's 3 right now)
        * What are the reasons to change the reward?
        * V: The chain is overpaying for security.
    * Lane
        * If we delay bomb without reducing issuance, it increases overall SIL issuance
    ...
    * Danny: original intent of bomb was to move to PoS, which would ultimately reduce issuance
        ...
        * EIP1011: I put out an analysis of Sila mining reward relative to other blockchains e.g. Bitcoin, Bitcoin cash -- suggests we are overpaying and would still be extremely competitive PoW blockchain even with reduction
        ...
        * reward reduction analysis in relation to sip 1011: https://gist.github.com/djrtwo/bc864c0d0a275170183803814b207b9a
    ...
    * Afri
        ...
        * In case we decide only to do one of these two things, it means we significantly change Sila issuance/inflation model
    ...
    * Hudson: Everyone please review and comment on the various SIPs discussed here

# Fellowship of Sila Magicians Update
* Let's have Jamie, Greg, or someone discuss Fellowship of Sila Magicians and what came out of the Berlin gathering on the next call
* Lane: We're planning another FEM gathering in SilaPrague post-Web3 summit, pre-Devcon, and would love if core devs could join

# Attendees
* Guillaume Ballet (EF)
* Alex Beregszaszi (EF/Ewasm)
* Vitalik Buterin (EF/Research)
* Pawel Bylica (EF/cpp-sila)
