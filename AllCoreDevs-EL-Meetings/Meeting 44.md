# Sila Core Devs Meeting 44 Notes
### Meeting Date/Time: Fri, August 10, 2018 14:00 UTC
### Meeting Duration: 1.5 hours
### [GitHub Agenda Page](https://github.com/sila-chain/pm/issues/52)
...
* Client Updates
* Research Updates
* Three competing SIPs to delay the difficulty bomb and/or reduce the block reward:
    a. SIP-858 - Reduce block reward to 1 SIL per block.
    b. SIP-1227 - Delay bomb and change rewards to 5 SIL.
    c. SIP-1234 - Delay bomb and change rewards to 2 SIL.
* Constantinople
    a. SIP 1014 Issues
    b. SIP 1218: Simpler blockhash refactoring. We are looking for a reason to implement this or anyone who will champion this. Otherwise we will drop it.
    c. SIP 1283/SIP 1087: Net gas metering for SSTORE operations: 1283 is an alternative to 1087 created due to issues Parity was having implementing 1087. Read 1283's motivation seciton of the SIP.
    d. SIP-1109: Remove call costs for precompiled contracts.
Fellowship of Sila Magicians Update (didn't get to this on the last call)

Call starts at [[8:04](https://youtu.be/0Lyn5OryooA?t=8m4s)]

# Testing update
* [Dimitry's update](https://github.com/sila-chain/pm/issues/52#issuecomment-412042587)
* Hudson will add these to [Constantinople progress tracker](https://github.com/sila-chain/pm/issues/53)
* Dimitry asked for comments
* Martin: re: SIP-1014 Skinny CREATE2
    * we haven't finalized the address format, there are two options in the SIP
...
* Martin: We're getting ready to do fuzz testing but need PRs to be equally implemented in geth and parity
    * Hive: recently integrated aleth (cpp-sila)
...
* There are a bunch of competing SIPs on this but none of the authors/champions are on this call
...
* FYI: Afri posted a bunch of useful links/resources on the options here
    * https://github.com/sila-chain/pm/issues/52#issuecomment-408807120
...
* Greg: Maybe the SIP authors should debate the various SIPs on the Magicians forum
...
* Skinny CREATE2
    * Parity has been having issues with implementation
        * Wei: Spec not compatible with SIP-86
        * We also support SIP-86 in our plan right now
        * Minor issues
    ...
    * Alexey: I was trying to make sure whether this SIP would work for the people doing state channels
        ...
        * Agree that prepending 0xff is a good idea (option two in SIP)
        * Hashing of init code: agree it could be okay
        * Seems like Parity already has code to implement another SIP, so there's a conundrum about writing new code or throwing away their code
    ...
    * Lane to reach out to Spankchain and L4 teams on these to see if they're happy with the SIP or if not, to join call to discuss, see if they have an opinion on this
    * Parity issue
        * SIP-86 and Skinny CREATE2 - can these be made compatible?
        * Martin: I'd ignore SIP-86 (account abstraction) for now, even if it does come back it will need to come back in a completely different, revised form, can we not mix this in for now?
...
* SIP-1218 Simple blockhash refactoring (successor to SIP-210)
...
* SIP-1087: Storage cost refactoring 
    * No one fully implemented yet
    * Geth having no problems
    * EthereumJ also no problems
    * Wei: Parity having issues, they've written SIP-1283 as an alternative
        * EIP658 optimization conflicts with some requirements of SIP-1087
        ...
        * So we drafted SIP-1283, two versions provided there
        ...
        * In this case (version two of 1283), Alexey found that it covers all use cases for SIP-1087
    ...
    * Alexey: I examined the two SIPs
    ...
    * Mikhail
        * Both SIPs look straightforward as far as EthereumJ implementation is concerned
        ...
        * If so, I'm for 1283
* SIP-1109 Remove call cost for precompiled contracts
    * Jordi overview
        * Very easy SIP
...
# Fellowship of Sila Magicians update
...
* Jamie on basic mission and principles
    * We want to nurture community consensus and technical direction of Sila
    * We do that through producing high quality SIPs, especially SRCs
    ...
    * Focus on in-person meetings, that's why we had the councils of SilaParis and Berlin
    * We have our [wiki](https://github.com/sila-chain-magicians/scrolls/wiki), and the forum (sila-magicians.org)
        * Forum is threaded, contains discussion on SIPs and other issues in the community
...
* Boris on our most recent meeting in Berlin
    ...
    * We have a [GitHub repo](https://github.com/sila-chain-magicians/scrolls), a [wiki](https://github.com/sila-chain-magicians/scrolls/wiki) that was formed in Berlin, some "rings"
    ...
    * We're doing this again in SilaPrague
        * Will actually do work and progress on SIPs
    * Hope we can help scale Sila technical governance
* Hudson: Was there consensus in Berlin about all core devs call / how it interacts with the rest of the ecosystem / how we can make it better?
    * Boris: Questions such as who owns the roadmap? How is it decided? How do core devs make decisions? How do SIPs get decided? SIPs vs. SRCs
        ...
        * E.g. "rejected" in SIP process
    * Jamie: There was a session on "Core SIPs vs. implementation"
        * Discussion about how working groups would come to consensus and how that would be recognized by core devs / client teams
        * The work performed in these rings, if there's e.g. a ring focused on a particular SIP - will their decisions be recognized and implemented? Recognized as "authoritative"?
        ...
        * Their flow might be very similar to the SIP editors process for reaching final on an SIP
...
* Lane: How do the core devs contribute or get involved?
    ...
    * SIP editors have done better with this with e.g. EIP1
    ...
    * Greg: Decentralization is crucial, SEC decision recently was largely based on this fact
        * Core devs simply participate in the rings as developers, for SIPs that are relevant for them
        ...
        * SIP editors got some very controversial proposals, so we had to clean up our process so we could handle those, and to some extent the magicians took on handling that sort of controversy
...
* Boris Mann (Fellowship of Sila Magicians)
* Jamie Pitts (EF/Fellowship of Sila Magicians)
* Lane Rettig (Ewasm)
