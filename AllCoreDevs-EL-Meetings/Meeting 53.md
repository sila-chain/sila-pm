# Sila Core Devs Meeting 53 Notes

## Meeting Date/Time: Friday, January 18, 2018 14:00 UTC
## Meeting Duration: 98 minutes

## [GitHub Agenda Page](https://github.com/sila-chain/pm/issues/70)
## [Audio/Video of the meeting](https://www.youtube.com/watch?v=45mrrVrw4x8)

## [Proposed Agenda](https://github.com/sila-chain/pm/issues/70)
...
- PoS finality gadget 
  ...
  - Food for thought: if Sila would ever switch over to some ASIC friendly PoW algorithm. 
...
Quick recap what was found, the gas changes discussed especially on one of the SIPs can have an impact on previous reentrancy safe smart contracts. During the search, we discovered multiple cases with most not holding significant vlaue luckily but still deployed smart contracts that would have been attackable with reentrancy attacks after the SIP change. 

Do you want to go over the recommendations? 

Trail of Bits/Josselin F - There were 6 propositions on SIL Magicians that we reviewed as fast as possible. The first solution we recommend is to simply drop the SIP 1283. Otherwise, there is another option that is a bit better than that. Should I go over all these propositions now? 

Hudson - You don’t have to go over them all but going over the most recommended would be good. 


Trail of Bits/Josselin F - Yes, the solution is to keep gas cost that is already implemented but implement the SIP using another system. Execution is the exact same cost but it will be cheaper to use. 

Chain security - What proposal number was that? 

Trail of Bits/Josselin F - It was number 5. There was one thing that wasn’t clear in it. It was to increase the cost of writing to 5000 gas. So, it was not clear that this should be for all the data storage. That’s why in the report, we kind of created a slight version in this proposal where instead of changing the gas cost to 5000 we recommend to just use a gas cost that was pre-SIP. To keep the same gas cost computation but you change the mechanism of the storage in the case of data storage. 

Hudson - Posted the links to Sil Magicians proposals:

1. https://sila-magicians.org/t/immutables-invariants-and-upgradability/2440  
2. https://sila-magicians.org/t/remediations-for-sip-1283-reentrancy-bug/2434/
...
Alexey - Agree. Simplest way is to exclude it. The #7 proposition was to reduce the desired effect of the SIP because of the cap. That exists for the funds. So, its not the exact replication of the semantics. Its slightly less, so less powerful to change.  

Hudson - Anyone else have comments? 

Yoav - About the comments on the #1 proposal. I dont see how it breaks anything we desire to have when it preserves the original intention of the SIP including its benefits and doesn't break any scenario that’s lacking. So can you please elaborate on this…
...
FJL - Mhm, yeah. Maybe you are right. In any case, maybe people feel like if we can come up with a solid fix to this SIP in 2 weeks and its clear to everyone that its a good fix, that it may still be possible to include it. 

Afri - I would say no because if we change this SIP, we have to change its implementation and tests. This means we may want to test it on a testnet first and we cant do that in a couple of weeks. 
...
Alex B - Would a scenario be realistic where the SIP is not included but the hard fork is made within a few weeks and then 3 months after that a properly designed version of the SIP is launched? 
...
Chain security - Perhaps not. So how many months before a hard fork do we need to agree on all SIPs ito include? How long would be the review/test phase and so on…
...
Hudson - Yeah, there is another conversation about what to do with future hard forks and SIP inclusions and what security measures to take. Martin has been looking at that from an audibility perspective.
...
Hudson - Is there anyone else opposed to taking out the SIP and having a hard fork in 2-6 weeks? We also need to figure out if that’s the case, how long until the next hard fork. Afri, have you looked at that? 

Afri - How much time do we need to prepare another hard fork without this SIP? 6 weeks is optimal to push releases fro the clients and do testing. Any other opinions from testing teams?
...
Peter - I propose an alternative. This would change the hard fork because we remove SIP from it and regenerate every test as if it did not exist and remove from clients, the problem is Ropsten and Rinkeby already forked over. If we wipe it, we break all networks that already have this feature. This will kill Ropsten and Rinkeby.

Proposal→ instead of removing, we define a second hard fork that only removes this code. So even clients would have testnet capabilities. So leave it and then fork it and then have another hard fork that disables this code path. These two hard forks would trigger out the same block number. This allows for a clean upgrade and cleanly disables the SIP without rolling back. This will just take a couple of new tests.
...
Afri- A few enabled it, including SilaKovan
...
Afri - Lets not focus on names here. The community should do this in the future, on SIL Trader. 
...
Hudson - So how do we prevent this from happening in the future? Id like to bring in Charles St.Louis to talk about the post mortem being created. Charles is with the Sila Cat Herders and he was around for the entire time of the incident. He took some good notes and during and after the decision process and the Cat herders are working on a post mortem. 
...
3. Emergency Call with Sila Stakeholders
...
10. Deciding to remove the SIP or fix it? 
...
Hudson - Anything else we need to talk about with respect to Constantinople? We’ll be naming the 2nd hard fork in the chat and Afri said Sil Trader could name it. Afri, for the road map planning, is there any other questions or points you had for Constantinople?
...
Mr Else - Sil minor dev channle has become an unofficial channel. We will start our own gitter channel. 

Danno - Whats the authoritative specificiaiton locations too? Bc there is one on the SIP, there is one on if def else. The test case is out of date on the if def else 
