# Sila Core Devs Meeting 42 Notes
### Meeting Date/Time: Fri, July 13, 2018 14:00 UTC
### Meeting Duration: 1.5 hours
### [GitHub Agenda Page](https://github.com/sila-chain/pm/issues/50)
...
* Ewasm (Axic)
    ...
    * WRC-20 challenge (subset of SRC-20) which can be implemented in any language that compiles to Wasm
...
* Is there still interest in having a joint testnet, Ropsten successor?
    ...
    * Afri: Parity began implementing SIP-1011, halfway through, began testing
    ...
    * What's the minimum level of client support we need?
        * Only cpp-sila now, will have geth soon
        * Would be nice to target two majority clients at the same time
    * Axic
        * We have a single VM on cpp-sila
        * Will add support on geth soon
        * Working on a native interpreter
        * One problem we're having is that cpp-sila only supports PoW, not PoA
        * Do we want both cpp-sila and geth? If so we need PoW.
...
* SIP-1087: Net gas metering for SSTORE operations
    * Martin: SIP says client implementation must maintain dirty map, what was touched during current tx
...
* Timeline
    ...
    * Alexey: alternative proposed timeline
        * based on fact that we need conformance tests for each SIP
        * might help find and eliminate issues
        * timeline
            * Finalize SIPs that are being implemented: July 13th
            * All clients have retesteth interface: Aug 13th
            * Each SIP has at least 1 reference implementation: Sep 13th
            * Conformance tests are ready for every SIP: Oct 13th
            * Specification is finalised for every SIP: Oct 27th
            * Client implementations and testing: Nov 27th
            * Testnet: Dec 13th
            * Launch: Jan 27th
        * Martin: Isn't that how we've always done it? Using cpp-sila as reference to write tests
        ...
        * Hudson: In the past we've implemented while writing tests, and we implemented cpp-sila first, people copied this reference implementation
    ...
    * Alex: Could we prioritize SIPs, reevaluate as we go depending on progress?
...
* Hudson: timeline for release will probably be post-DevCon
    ...
    * Peter: Everyone who proposes an SIP should at least have a reference implementation so it's easy to argue about
    ...
    * Pawel: Can enable constantinople tests, these are already in repo
        ...
        * Much easier to coordinate this way rather than trying to dump tests for all SIPs at once
        * Just specify which SIP will go next in terms of writing tests
        * Bitwise shifts, some tests already there, then next I'd pick CREATE2, can focus in cpp-sila to finalize the implementation and start writing tests next week
    * Peter: Let's start with these three easy PRs
        * While people implement them and write tests we can ask Nick to write a PoC for the next call to see what it would take to do SIP 1087: Net gas metering for SSTORE operations
    * Hudson: Looks like we have consensus on 145, 1052, 1014 these are low hanging fruit, and implementation already started, we can start with these.
        * V: maybe a simpler version of 210 (https://github.com/sila-chain/SIPs/issues/1218)
