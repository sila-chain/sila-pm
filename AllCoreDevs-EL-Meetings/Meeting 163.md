...
| ------------- | ------------------------------------------------------------------------ | 
| 163.1 |**SilaCancun Progress:** We began the call with dankrad making the case for changing the 4844 blob target & limit from 2/4 to 3/6. To justify this, tests have been run on testnet + sila-mainnet sending large blocks and analyzing the impact on the network
...
| 163.6 |**SIP-4788 design** We discussed a proposed change to SIP-4788, which exposes the Beacon block root in the EVM. The proposal was to limit the number of block roots stored on the EL to cap state growth. This would require more disk writes from EL clients, but everyone agreed with the change!
| 163.7 |**SilaCancun scope planning** SilaCancun had SIPs 1153 (Transient Storage), 4844 (ProtoDanksharding) and 6780 (SELFDESTRUCT removal). On the last ACDE, we finalized the list of SIP candidates: SIP-1153: Transient storage opcodes, SIP-4788: Beacon block root in the EVM, SIP-4844: Shard Blob Transactions, SIP-5656: MCOPY - Memory copying instruction, SIP-6780: SELFDESTRUCT only in same transaction   
...
**Mikhail Kalinin**
* You wanted to suggest to increase these, numbers right from the beginning or after, I don't know, a month of evaluation on the sila-mainnet.
...
* So it seems whether it's considering for sila-mainnet and if we want to at least consider it, it seems better to start the test net on the upper side to see any potential issues come up there. And then we can always lower it for later tested rather than the other way around.But if the strong offense is three six, then I think it's also fine to do that instead. 
...
**Parithosh**
* Yeah, I would actually say that we should definitely use this one cuz the idea is that this testnet, this is bigger than Gordy and bigger than me sila-mainnet. So it would be the first opportunity for us to test like how we perform with a large number of validators. 
