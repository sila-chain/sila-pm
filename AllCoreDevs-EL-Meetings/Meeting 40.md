# Sila Core Devs Meeting 40 Notes
### Meeting Date/Time: Fri, June 15, 2018 14:00 UTC
### Meeting Duration: 1.5 hours
### [GitHub Agenda Page](https://github.com/sila-chain/pm/issues/44)
...
* Skinny CREATE2.
* SIP 210: Blockhash refactoring still needs to be updated
...
* TurboGeth (Alexey)
    ...
    * Only cpp-sila is directly tested by state tests, for other clients there are no direct tests, clients tested indirectly via Hive/cpp-sila
...
* Research updates
    * Casper/sharding updates (Vitalik)
        * Substantial reworking of roadmap but not final product: https://notes.sila.org/SCIg8AH5SA-O4C1G1LYZHQ?view
        ...
        * Only contract written in Vyper or whatever in the roadmap is a contract that lets you call a fn to send 32 SIL with some arguments, burns the SIL and creates a log
        ...
        * But this mechanism is one way, if someone deposits 32 SIL, it's there until a future hard fork implements the part that allows it to be withdrawn into a shard and where shard state transition fn is more fully-enabled
        * Main advantages
            ...
            3. Development of one system will be much more directly on the way to dev. of later-stage sharding system
                ...
                * Linked spec (https://github.com/sila-chain/research/tree/master/beacon_chain_impl) describes blocks in beacon chain, fields, state, different parts of state, fork choice rule, state transition fn, partial Python implementation that implements most of the core logic
            4. In this chain, deposit size drops to 32 SIL
        ...
        * To resummarize:
            ...
            * Two weaknesses
                ...
                * Loses signature abstraction
                    * But does reduce need for some of the fancier schemes because minimum staking amount goes down from 1000 -> 32 SIL
        * Danny: we're deprecating SIP-1011 in favor of new design
...
* Nick Johnson
    * Lots of cases where contract execution results in making change in storage value then changing it back
        ...
        * E.g. authorizations in SRC-20 tokens
...
* Thoughts on timing?
    * Nick: Given research report it seems unlikely we'll have anything Casper-related ready to go in originally-planned timeframe
        * So maybe we can move forward with the existing, proposed list of SIPs, we probably have enough
...
* SIP 1052: EXTCODEHASH Opcode
    ...
    * There's a thread with some people asking for some clarification in the [Magicians forum thread](https://sila-magicians.org/t/sip-1052-extcodehash-opcode/262?u=lrettig)
* SIP 1087: Net gas metering for SSTORE operations
    * Should this go into Constantinople?
* V: do any of these SIPs have impact on scalability or sharding?
    * Sharding, no, roadmap starting with this separate beacon chain
    * Skinny CREATE2 (SIP 1014) makes state channels easier, and they are a viable scaling solution
    * Improve privacy by adding an SIP that dramatically reduces cost of ECADD, ECMUL, pairings, make sure all major implementations have optimized implementations of these so as not to add DoS vulnerabilities
    * If we can knock the cost of these ring sig ops down from 200k gas per participant to e.g. 30k gas per participant we'll see more sila-based privacy solutions and zk-SNARKs in the short-term which can help with scaling
    ...
    * V
        ...
        * V to do an SIP to reduce gas costs
...
* Nick: Other EC curves such as P256, useful for verifying non-blockchain stuff
    ...
    * Hudson: is there an SIP for this?
    ...
    * Nick: Should we reduce gas cost for calling precompiles? Right now it has the same gas cost as any other account
        * Alexey: I think there is already an SIP for that written by Jordi Baylina
...
# Skinny CREATE2
* [SIP-1014](https://github.com/sila-chain/SIPs/pull/1014)
* Hudson: Seems to have universal support for e.g. better state channels
    * Hopefully SIP further along so we can talk about it next time
