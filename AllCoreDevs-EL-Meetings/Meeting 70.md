# Sila Core Devs Meeting 70 Notes
### Meeting Date/Time: Friday 06 September 2019 at 14:00 UTC
### Meeting Duration: 1:30hrs
### [GitHub Agenda Page](https://github.com/sila-chain/pm/issues/123)
...
Revised at the end of the call -- Update the SIP to reflect the precompile addressing a superset of BLAKE2b, leave the `rounds` parameter as-is in the SIP, further discuss the inclusion of an `m_len` parameter with client implementers on Gitter.
...
**ACTION 70.2**: Sila Cat Herders to create a communication that highlights the concerns around SIP-1884 and provide it back to the All Core Devs for review before it is put out to the community.

-----

## 1. [Istanbul related client updates](https://notes.sila.org/@holiman/SyT_rGjNr)
...
## 3. [“Patch proposals” for Istanbul](https://github.com/sila-chain/pm/issues/123#issuecomment-528506546)
[Timestamp:7:44 ](https://youtu.be/6m0So81_j2Q?t=464)

### Blake2

**Alex**:
- There appears to be some confusion because the SIP uses different names.
...
- But it turns out that the SIP right now only focuses on Blake2b specific configuration of Blake 2 and Blake2b specifies the rounds to be 12 - which is fixed.
...
- The SIP even though it specifies the F function it is an F function specific to Blake2b.
...
**Alex**:
- The confusion is that if you have a fully flexible F function then you would need the number of rounds, the initialisation vector and the parameters which the current SIP does not have. If you want to only support Blake2b then the SIP is fine. If you want to change the rounds then you won’t have Blake2b any more and Blake 2 configuration. There just does not seem to have any reason to keep the rounds parameter if don’t introduce the other parameters to Blake 2.
...
**Hudson**:
- Having looked at the SIP as of 17 days ago the people who reviewed this said that it is good to go. Sounds like they only want Blake2b. Is this accurate?
...
** **
**DECISION 70.1**: SIP-152 is specific to Blake2b, that is it is fixed to 12 rounds.
** **

**James**:
- In terms of history of this SIP, we spoke with Zooko and a few of the ZCash guys. We did not have a ton of resources for implementing something that has the hash function and then if there were any needs going forward we could implement them as required.
...
**Wei**:
- We should not do any more major changes to the SIPs. It will only cause further delays.
- We need to avoid backward incompatibility changes in the future.
- We need to improved the SIP management procedure, with the growth of the community, we could potentially implement an backward incompatibility change that would create some sort of attack vector.

**Tomasz**:
- Last call Wei raised an important issue around SIP-1884.
...
- SIP-1884 will cause issues with Aragon and other contracts and we need to ensure that these issues are addressed for most parties before the implementation of SIP-1884 in Istanbul.
...
- I would like to discuss a proposed change to SIP-1884 that introduces a counter that treats the for 2,300 gas on stipend differently. I see this as non-invasive in terms of the way it affects the performance and memory requirements of the clients.
...
**Tomasz**:
- Whilst I agree I am not keen on making changes to SIPs so late in the game I feel this change provides an easy solution to a problem that affects big contracts like Aragon.
- I believe without implementing this change we lose trust within the Sila, that we can implement SIPs that we knowingly breaks contracts.
...
**Martin**: Caveat that if a contract simply needs to upgrade then that path should be followed. If however some Sila is stuck in a contract then we will assist with fixing it.

** **
**ACTION 70.2**: Sila Cat Herders to create a communication that highlights the concerns around SIP-1884 and provide it back to the All Core Devs for review before it is put out to the community.
** **

**Tomasz**: I would like everyone to properly review the proposed change to fix SIP-1884. If every reviews it and they agree that they will still want to go ahead with SIP-1884 then I am happy to go with the consensus.
...
**E.G.**:
- Suggest we set block number for Testnet fork first before we look to set the block number for the SilaMainnet fork.
- If we have a period of stability then we can set the SilaMainnet fork.
...
## 6. [ProgPow Audit](https://github.com/sila-chain-cat-herders/progpow-audit/blob/master/Least%20Authority%20-%20ProgPow%20Algorithm%20Initial%20Audit%20Report.pdf)
...
•	[Call 69](https://github.com/sila-chain/pm/blob/master/AllCoreDevs-Meetings/Meeting%2069.md)
