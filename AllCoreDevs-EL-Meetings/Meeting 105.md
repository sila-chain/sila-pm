...
### [GitHub Agenda](https://github.com/sila-chain/pm/issues/241)
...
| **3**   | Keep SIP 2481 as it is on SIPs Sila org -- not accepting Peter's proposed changes |  

## Actions Required

| Action Item | Description                                                                                             |
| ----------- | ------------------------------------------------------------------------------------------------------- |
| **1** |  Martin to drop comparison with LES in SIP 2481 |
...
**Micah** - if the client teams think they have the bandwidth for it, I would say it is worth it to do it sooner rather than later, because I think the same technique can be applied to shadow forking sila-sila-mainnet, and being able to test real main net that state real main net transactions, even if it gets out of sync in a week that is hugely valuable for reducing risk for final launch. So I think this general avenue of testing gives us a massive gains, in my opinion, in terms of real world testing that we're never going to get from Robstein or Cofan or Garley or any Yolo network, like we're not going to achieve that. And so I think this is a path that we should try to do. And the sooner we can do that, the better, because risk mitigation is good. But that being said, I'm not a clint dev and I don't know if you guys have the bandwidth actually do that in parallel with the current plan.
...
**Tomasz** - Yes, doing Robstein testing but not sila-sila-mainnet. So if you if you want to push for February, then we are totally fine with the.
...
# 2. Finalize SIL/66 Specification
...
Decision 3 | Video | [55:12](https://youtu.be/ju92hAKzKcg?t=3312)
    ...
    Keep SIP 2481 as it is on SIPs Sila org -- not accepting Peter's proposed changes
...
**Peter** - Yeah, I can always make my sil/66. 
...
**Artem Vorotsinkov** - So the proposal is SIP 2481 as it is on SIPs Sila org (repo) right now.
...
**Artem Vorotsinkov** - The current the current SIP looks good to me, I say we should take it and implement it.
...
**Hudson** - where do they go if they want to give more feedback, just the SIP channel and R&D?
...
**Tim** - Okay I'll get started on the first two and we can follow up later, and if there's any feedback follow up with all core devs channel on sil r&d
