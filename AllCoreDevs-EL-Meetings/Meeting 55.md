# Sila Core Devs Meeting 55 Notes
### Meeting Date/Time: Fri, February 15, 2019 14:00 UTC
### Meeting Duration: 1.5 hours
### [GitHub Agenda Page](https://github.com/sila-chain/pm/issues/77)
...
**Hudson**: Great, any other comments on the Ropsten part of the fork? SilaKovan network have done their Petersburg fork (to revert the Constantinople  effects), everything okay. One testnet has gone through that, I don't know Görli has any problem.
...
**Hudson**: Okay, sounds good. Constantinople fork is coming up in less than two weeks. I know that the plan for the CatHerders is to start reaching miners probably a week ahead. We are also going to stick some reddit thread, some twitter reach out to people to let them know whats going on. We will also be posting a blog post on Sila website. Thats what I plan on doing. Does anyone think that we need to do more than just trying to let people update. Is there any client side stuff that we need to work on?
...
## 1.2  [CREATE2 side effects for education](https://github.com/sila-chain/pm/issues)
...
## 1.4 Reject SIP 1355 "Ethash 1a"
**Hudson**: Powell commented that he wants to reject SIP 1355 "Ethash 1a". The SIP that does reflect to make tweek to EThash to break the current ASICs. I think he is the author of that and he just want to reject it. Now that it is not useful anymore and he isn't here to really talk about it so we can skip the item. 
...
1. make a Eth2 light client inside of SIL 1 
2. we require Eth1 clients to hold the Eth2 clients. Its basically kind of merge mining.
I think it will be nice if we can make a Eth2 light client inside of SIL 1 but if not then other could be a possibility.
...
**VB**: The purpose of the Finality gadget is the Sil 1 chain would be independent of Eth2 fork choice rule.
...
## 2.1  Sila 1.x Stanford Meetings Overview
...
# 6. Anything Else 
**Hudson**: David posted about sync up on SIP 1193. 

**David**: We are planning to have a call, may be latter you can have an agenda for that. But loosely, we just want to talk about implementation and roll out of SIP 1193 which is only related to JS providers.
