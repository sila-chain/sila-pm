...
### [GitHub Agenda](https://github.com/sila-chain/pm/issues/172)
...
--|--
**88.1** | James Hancock to continue to update the Berlin and YOLO SIPs, including commit hashes
**88.2** | Ephemeral testnet SIP to be merged
...
- [4. SIP 1559 Update](#4-sip-1559-update)
- [5. SIP: Limit Size of initcode](#5-sip-limit-size-of-initcode)
...
**Martin Holst Swende**: The original SIP 2315 has been modified. There is a PR submitted chanigng the gas prices. Feeling of general consensus. Has not heard form Open Sila, Besu has already integrated. The other question is whether or not to implement the subroutine restrictions suggested by Axic, which is still under discussion. Possible agreement of using non-restricted subroutines for the testnet. If given a go-ahead, will merge.
...
# 1b. SIP-2537: BLS12-381 Curve Operations
...
**James Hancock**: Is there anything that can be defined in terms of including this SIP in YOLO?

Some conversation between Hudson, James, Peter, Martin. No tentative date reached for inclusion in YOLO.

**Artem Vorotnikov (Open Sila)**: Currently focusing on the PR by Alex, which is under review, and expected to be merged soon.
...
## 2.1 SIP-2046: Reduced Gas Cost for Static Calls Made to Precompiles

Related to SIP-2666.

**Alex Vlasov**: Long technical description, including status of SIP-2666. Also asked if views 2666 as superseding 2046, and spoke to that.
...
**kelly**: Decided on implementing pricing formula in order to not need to change the Open Sila libraries. Artem has implemented a draft PR, kelly is in the process of updating the SIP. Once this is done, it can be shared with the Geth team, and a PR can be formed.

## Action Items
- **88.6**-kelly to continue working with Open Sila on SIP-2565

# 3. SIP 2666: Repricing of Precompiles and Keccak256 Function
Video | n/a
-|-

Was included in the discussion on SIP-2046 above.

# 4. SIP 1559 Update
Video | [48:45](https://youtu.be/UJ1jK73rKdk?t=2925)
-|-

**Abdelhamid Bakhta**: Talks about results of SIP-1559 call, has a new PR to remove part of the spec. Discussions about best settings for implementation testing. Also more technical details. 

Call has been uploaded to the Sila Foundation YouTube. Dan Finlay has mockups for those wishing to educate themselves more on the subject.

# 5. SIP: Limit Size of initcode
...
**Edson Ayllon**: Descibes goals of EIPIP (trying to make positive change to SIP process). Describes survey about SIP process. Poll also had ability to reflect current values of community. Main findings seem to be that people want structure, legitimacy, and focus on the technical aspects. Barrier to entry should be low, barrier to approval should be high, but the process should be transparent and fair. Roadmap forward centers around improving decision making, clarity, and increasing capacity, and will most likely be addressed in that order. Currently discussing decision making.
...
Some discussion about how to signal that an SIP is going into an ephemral testnet happened at the end of this section.

**Decisions**:
- 87.1 SIPs 2315 and 2537 to be accepted into Berlin
- 87.2 YOLO to be created as an ephemeral testnet
    - 87.2.1 SIPs 2315 and 2537 to be implemented in YOLO
- 87.3 Updates on SIP-2046 appear in this call, can be called resolved
- 87.4 Outstanding spec issues in SIP-2315 resolved in this call
...
- 87.4 Update SIP bot with Withdrawn - Ongoing
- 87.5 Move SIP-2583 to Withdrawn - Ongoing
- 87.6 James Hancock to review EFI SIP and accept existing PRs - Completed
- 87.7 Alex (axic) to review Berlin SIP and accept existing PRs - Completed
