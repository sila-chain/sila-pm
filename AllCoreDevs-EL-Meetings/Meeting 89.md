...
### [GitHub Agenda](https://github.com/sila-chain/pm/issues/180)
...
**89.2** | SIP-2565 moving to EFI, officially.
**89.3** | Alex S. and team to look into another security audit for Sil 2.0 Deposit Proxy Contract that utilizes SIP-2537. Anyone interested, reach out to Hudson or Alex S. 
...
- [1. Berlin SIPs - Integration Updates](#1-berlin-sips---integration-updates)
- [2. Call to discuss precompile repricing (SIP-2046: Reduced gas cost for static calls made to precompiles & SIP 2666: Repricing of precompiles and Keccak256 function)](#2-call-to-discuss-precompile-repricing-sip-2046-reduced-gas-cost-for-static-calls-made-to-precompiles--sip-2666-repricing-of-precompiles-and-keccak256-function)
- [3. YOLO Testnet Update](#3-yolo-testnet-update)
- [4. Eligible for Inclusion (EFI) SIP Review](#4-eligible-for-inclusion-efi-sip-review)
- [5. Update on Sil 2.0 Deposit Proxy Contract that utilizes SIP-2537](#5-update-on-sil-20-deposit-proxy-contract-that-utilizes-sip-2537)
- [6. Account Abstraction Research Update](#6-account-abstraction-research-update)
- [7. sila/PM calendar](#7-ethereumpm-calendar)
...
**Hudson**: Hi everyone and welcome to Sila Core Dev meeting number 89. I'm your host Hudson and we're going to jump into the first agenda item which is the Berlin SIPs integration update. The first link is clickable and brings you to a Google spreadsheet that shows some updates from different client teams and what's the latest on there. So I'll pass it over to James who can give it update on that.

# 1. Berlin SIPs - Integration Updates

Video | [04:07](https://youtu.be/c_JmTqeQkU4?t=249) 
-|-

**Hudson**: Hi everyone and welcome to Sila Core Dev meeting number 89. I'm your host Hudson and we're going to jump into the first agenda item which is the Berlin SIPs integration update. The first link is clickable and brings you to a [Google spreadsheet](https://docs.google.com/spreadsheets/d/1BomvS0hjc88eTfx1b8Ufa6KYS3vMEb2c8TQ5HJWx2lc/edit#gid=0) that shows some updates from different client teams and what's the latest on there. So I'll pass it over to James who can give it update on that.
...
# 2. Call to discuss precompile repricing ([SIP-2046: Reduced gas cost for static calls made to precompiles](https://sila-magicians.org/t/sip-2046-reduced-gas-cost-for-static-calls-made-to-precompiles/3291) & [SIP 2666: Repricing of precompiles and Keccak256 function](https://github.com/sila-chain/SIPs/pull/2666))
Video | [07:35](https://youtu.be/c_JmTqeQkU4?t=455)
-|-

**Hudson**: The second agenda item there was a call to discuss precompile repricing specifically the SIPs 2046 and 2666. I think that meeting happened and James I think you organize that if you want to talk a little about it or passed it over to someone to give a update.
...
* Martin made a test for Geth and I have run test for OpenEthereum. It looks like for geth, Static call should be priced around 40 and for open sila, it can be made around 30, most likely.
...
**James**: One more thing. I'm happy to do more of these SIP specific calls, so  when someone has something that they think would fit for that please reach out to me and I can keep making those as we need them. I really like the format and it helped progressed SIP very well so we should do that. We should tend to do that as often as make sense.

**Hudson**: Yeah I agree. I think that's great that you're willing to step up on that and it helps us to better time management for these core devs call so we're not spending a large amount of time discussing specific SIPs all the time.
...
**Hudson**:  We can go to the next item on the agenda this is the EFI SIP review SIP 2565 repricing the VEIP 198 mod exp precompile. Kelly?

**Kelly**: There's some final changes being made to the SIP draft, they were nothing really substantial, we are working through some final benchmarking to make sure that there are no issues with the pricing but we have the test vectors.  My estimate is that, that will be complete early next weekend and fina, and we will submit a PR to the SIP repository at that point.
...
**James**: Okay, and you will make an edit on the SIP to show that?
...
**Hudson**: We did, I forgot which meeting but yeah, it's in the EFI. Oh! you know what actually I'm looking at it right now the SIP is outdated but I just have to look at old meeting notes and see if it's motion to go in but I swear we did it at some point.
...
**Hudson**: Does anyone object to having this SIP as an EFI meaning that it has the go-ahead to be continued to be worked on and could make it into a hard fork, likely. 
I don't hear any dissent and we've been following this one for a while so and there's been a lot of good cooperation to really well-written SIP so I think that it's fine to go to EFI, so **that motion passes**. I guess we're calling it motions now. I just don’t remember ever calling it that. So just let's put in the EFI, sounds good.
...
**Alex S.**: Right. so the other different ways to break it up right now what the contract does is **you send to the full deposit along with the Sil** and so when I say full deposit I mean like their credentials that end up going into the Eth2 system. So all of that data plus the Sil goes to this contract. **This contract verifies the BLS signature to make sure that's correct** cuz that's what we've been seeing is that basically there's been some incorrect signature creation and then that leads to an invalid deposit and then the Sil is basically lost. This contract helps because it checks the BLS signature, if that all passes and it looks good and then does a call for you to the deposit contract itself, so it wraps in a sense. If it turns out that like it takes too much gas to do this way or something, we could actually split it out so that the verification lives along side but then the actual Call to the deposit contract is done by yourself, separately. 
...
**Will**:  We’ve also been exploring how that intersects with the general discussion in meta transactions as well and howaccount abstraction overlaps with that we've been involved in this discussion in the Sil research recently. We see it under the same umbrella as well. So, we've also been putting effort & time into it. 

**James**: Did you see the updated version of the oil and gas proposal from Alexey that talks about meta transaction research. 

**Will**: We did, we responded to that and then we posted a new post on Sil research that continues to experience some of these discussions as well.

**James**: Great!

**Hudson**: Alright and if anyone wants to go back over what Will just discussed or go to any of the links for the spec or the discussion on Sil research, it's in the agenda at the latest [comment](https://github.com/sila-chain/pm/issues/180#issuecomment-643287338) that was posted about half an hour ago. Can you post a link in the agenda or on here to the spec you made,  Will? Which of the implementations you made it at the fork of geth?

**Will**:  Oh yeah, I can put a [link](https://github.com/quilt/go-sila/pull/13/).
...
**Tomasz**: I just wanted to add that we will be there probably quite soon because we wanted to finalize this hive work to execute the consensus test from hive and then I will be able to add the new test for new SIPs. I hope soon we'll have state test results for you.

**Martin**:  Much appreciated, thanks.

**James**: I think we should add some of that language to the Yolo SIPs, perhaps Martin. Something we could follow up on. Writing those out and then tracking state test based on per client.

**Hudson**: Anybody else?

# 9. [Review Previous Decisions and Action Items](https://github.com/sila-chain/pm/blob/master/AllCoreDevs-Meetings/Meeting%2088.md)
...
**88.1** James Hancock to continue to update the Berlin and YOLO SIPs, including commit hashes. That's been ongoing right, James ?

**James**: Yep, that happened.

**Hudson**:  

**88.2** Ephemeral testnet SIP to be merged several testnet SIP to be merged 

**James**: that has happened 

**Hudson**: 
**88.3** James Hancock to follow up with Open Sila, Axic, and Martin to discuss SIP-2315 after this call

**88.4** Axic to make a PR to update opcodes for SIP-2315
...
**88.6** Kelly to continue working with Open Sila on SIP-2565. 
That has been resolved and the SIP will be updated, right Kelly?

**Kelly**:  Correct 

**Hudson**: I didn’t do a great job of writing down the decisions made it actions required today but it sounds like 
* SIP 2656 to be moved to EFI officially and the SIP to be updated.
*  James H. to follow up with Martin and others on State transition test for clients and how that relates to Yolo.
* (Next call) Evaluate adding SIP-2046 to Yolo
...
**James**: It’s a good reminder that Dapp developer should be looking at YOLO to test their things unless you are actively working on the SIPs, helping evaluate them or client integration test.
...
**10:40:11**	 
From James Hancock : https://github.com/sila-chain/pm/issues/180#issuecomment-643287338

**10:40:14**	 
From James Hancock : Wills Comment

**10:40:28**	 
From Matt : https://github.com/quilt/go-sila/pull/13/

**10:40:33**	 
From Matt : AA ^

**10:41:12**	 
From James Hancock : https://calendar.google.com/calendar/embed?src=sila.org_semftevk58vu53rv149mvkla78%40group.calendar.google.com
