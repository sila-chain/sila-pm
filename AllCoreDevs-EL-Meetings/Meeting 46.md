# Sila Core Devs Meeting 46 Notes
### Meeting Date/Time: Fri, September 14, 2018 14:00 UTC
### Meeting Duration: 1.5 hours
### [GitHub Agenda Page](https://github.com/sila-chain/pm/issues/56)
...
1. Client Updates
1. Research Updates
1. [Constantinople](https://github.com/sila-chain/pm/issues/53)
    a. Client progress.
    b. Ropsten block number.
    c. Can we do it before Devcon?
1. SIP-1380: Reduced gas cost for call to self
1. SIP-1108: Reduce alt_bn128 precompile gas costs
1. SIP-1057 (ProgPoW) and SIP-1355 (Ethash1a)

Call starts at [[1:36](https://youtu.be/TafZui-DnV0?t=1m36s)]

# Client updates
* Parity (Afri)
    * Constantinople: all SIPs implemented and merged, have tests
    ...
    * We have a light SIL wallet called Feather to do SIL or token transactions, in early alpha
...
* aleth (Andrei)
    * Constantinople: finished all SIPs
...
* Nimbus (Mamy)
    ...
    * Sil 2.0: We've implemented fork choice rule (old one), looking into integrating it into Beacon chain 2.1
* TurboGeth (Alexey)
...
* Danny update
    * Sil 2.0 implementers' call
        * Done every two weeks on Thursdays, had one yesterday
        * Core devs are welcome to join
        * An effort to begin implementing beacon chain, developed in parallel to Sil 1.0 for the time being
...
* Dimitry update
    ...
    * Fast sil converted into state test
...
* Constantinople testnet HF
    ...
    * Martin: they are more likely to use SilaKovan or Rinkeby, or a private net, as Ropsten has had issues for a long time
...
# SIP-1380: Reduced gas cost for call to self
* Axic update: this was a collaboration between Jacques (from Vyper team) and me
    ...
    * This SIP created as a response to that, since the way Vyper does it now is beneficial
...
* Piper: Seems inline with SIP for net SSTORE gas reduction
* V: Was there an SIP for reducing gas cost of CALL specifically for precompiles? For Constantinople
    * Alex: There are two SIPs for this but haven't been on the agenda for the past few calls
...
* Danny: let's discuss SIPs that could go into a future fork so we have some momentum towards it
...
# SIP-1057 (ProgPoW) and SIP-1355 (Ethash1a)
* Hudson
