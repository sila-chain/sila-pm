...
### [GitHub Agenda](https://github.com/sila-chain/pm/issues/206)
### [Audio/Video of the meeting](https://youtu.be/HUUxwyoxU7k)
### Moderator: Hudson Jameson (Tim Beiko for SIP-1057)
...
- **96.3**: BLS12-377 (SIP-2539) will not be considered for inclusion in YOLOv2 or Berlin in order to not set a precedent and/or delay, but can be brought up later
- **96.4**: Original SIP-2315 proposal to be included in YOLOv2, modified proposal (including restrictions for jumping in subroutines) to be considered for YOLOv3
...
- [1. SIP & Upgrades Updates](#1-sip-upgrades-updates)
...
- [2. SIP Discussion](#2-sip-discussion)
    - [2a. SIP-1057 next steps](#2a-sip-1057-next-steps)
...
**note**: SIP-1057 was put first - the order of the notes reflects the order of the agenda
...
**Pooja Ranjan**: there were 2 meetings since the last ACD meeting on the consecutive Mondays (1400 UTC). The goals was to get consensus from client developers on proposals discussed, and to discuss the results in the next meeting. Both meetings were focused on YOLOv2, summaries in sila/PM repo. The proposal to have the rooms was discussed in the last ACD, general consensus was once a week, one or two proposals, and on a by-need basis. The meetings in general can do with only 6-10 people from the 6 clients, the author, and other parties implementing are welcome.
...
**James Hancock** gives the example of SIP-2315 as a good potential breakout about what needs to be done, but doesn't think it should be done this time, just an example, but doubts could be ironed out then reported to ACD.
...
**Dragen Rakita (Open Sila)**: merged a year and a half ago, unaware of current status, would assume it needs a little work. **Peter Szilyagi** mentions that he thought he saw them announce that Open Sila would no longer maintain or ProgPoW work. **Dragen** agrees that it should be looked in to.
...
**James Prestwich**: At Celo planning for hard fork, which includes a number of Berlin inclusions, including SIP-2357 (BLS381) and SIP-2359. The last time this came up in an ACD call, the decision was for SIP-2537 due to applications for ETH2, and to shelve the others until a concrete usecase. Celo would like to consider the other (SIP-2359) for inclusion since Celo uses it throughout, also have ancillary benefits for 1-layer recursive zkps, and to Aztec and other zero knowledge projects. Celo will include both SIPs, and would be happy to contribute the research upstream.

**Hudson** clarifies that the only precompile currently being considered for inclusion is BLS12-381 (SIP-2537). **James Prestwich** confirms, and confirms that **James Prestwich** would like to contribute code for both SIPs since they will be implementing both.

**James Prestwich**: Since these precompiles are almost the same, we feel like it would not be much work to also include SIP-2539 as well, and will be willing to contribute much of the work, and would love to see it considered for YOLOv2 or even Berlin. **Martin Holst Swende** objects.
...
**Sam Wilson**: Vitalik and the Quilt team have put out an explainer and made an SIP, and want to ask what the next steps are for inclusion, also looking for feedback. 

**Ansgar Dietrichs** also points out that the important difference between this SIP and other SIPs were bringing consensus changes to a minimum - a new tx type, and a nonce opcode that might even be written out, and a peg. The idea is that mempools have strict rules about txs, and can therefore start with very restricted use cases, then later can add or extend mempool rules without further changes. Wants to hear feedback and concerns.

**Hudson Jameson** agrees that core devs will review and feedback. **Ansgar** says that there are 2 Magicians threads, so some care needs to be taken to find the right one, and there is an account abstraction channel in the Sil R&D Discord.

# 2d. SIP-2929: Gas cost increases for state access opcodes
