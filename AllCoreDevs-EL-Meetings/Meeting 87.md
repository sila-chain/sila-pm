# Sila Core Developer Meeting 87 Notes
### Meeting Date/Time: Friday 15 May 2020 at 14:00 UTC
### Meeting Duration: 1 hour 30 minutes
### [GitHub Agenda Page](https://github.com/sila-chain/pm/issues/169)
...
SIP | Status
---|---
SIP-2315, SIP-2537 | `Accepted` Targetting Berlin
SIP-2046, SIP-2565 | Proposed for Berlin
SIP-2464 | `Last Call`

For more up to date SIP status see the [EFI tracker](https://github.com/sila-chain/SIPs/pull/2378).
...
**87.4** | Update SIP bot with Withdrawn
**87.5** | Move SIP-2583 to Withdrawn
**87.6** | James Hanckock to review EFI SIP and accept existing pull requests.
**87.7** | Alex (axic) to review Berlin SIP and accept existing pull requests.
...
**Hudson Jameson**
Hi, everyone. I didn't start the stream correctly. This is Sila Core Developer Meeting 87.

### Agenda #1: [Berlin SIPs](https://docs.google.com/spreadsheets/d/1BomvS0hjc88eTfx1b8Ufa6KYS3vMEb2c8TQ5HJWx2lc/edit?ts=5ebe9851&pli=1#gid=0)

And we are going to Agenda Item Number One, which is the Berlin SIPs.

That is going to be a [clickable link in the agenda](https://docs.google.com/spreadsheets/d/1BomvS0hjc88eTfx1b8Ufa6KYS3vMEb2c8TQ5HJWx2lc/edit?ts=5ebe9851&pli=1#gid=0) that opens up a spreadsheet that Pooja from the Sila Cat Herders created and is working with the Sila Cat Herders and James Hancock to continue to be updated. I'm gonna have James take over, and he's looking to share his screen in a second, or I'll share my screen if he can't figure it out, to go through the different clients and the status of each SIP. James, you want me to share my screen?
...
**Artem Vorotnikov**
So basically we have merged 2315 into master branch today as it was one month ago. So if the spec was changed today, then the current code in Open Sila master does not reflect it. So I guess we need to have some investigation here.
...
**Tim Beiko**
For Besu, we're in the same boat. We've merged or have a PR ready to merge the spec as it is, but we're kind of blocked on basically the spec getting finalized. Like Martin said, we wrote some acceptance tests that reflect the spec as it was when we wrote the test, but the biggest blocker for us is just getting a final agreement on the SIP.
...
**Martin Holst Swende**
Well, I don't know. I guess the [AllCoreDev channel](https://gitter.im/sila/AllCoreDevs).
...
SIP 2537, the BLS curve operations, starting with Geth:
...
**Artem Vorotnikov**
Actually, it has been implemented in Open Sila approximately a month ago, I think.
...
**Alex (axic)**
Somebody - was it you, Martin? You wanted to also replicate the results on Go Sila.

**Martin Holst Swende**
Yeah. And I told all the client implementers that you guys may want to test your nodes, your clients. But in particular, I care about Go Sila and want to benchmark that.
...
**Hudson Jameson**
And we want to keep in mind the SIP-centric process where we're trying to work around which SIPs are done...
...
**Hudson Jameson**
Because my thought was as this is an SIP that is wanted by a few different teams across the ecosystem, namely the Sila 2.0 team, if they were to bring in someone who can get familiar with the implementation code base for this SIP, and just double check that it's integrated into Geth properly, that could be something that I would help arrange and find the right people for, so you all wouldn't have to waste time [unclear] that out.
...
**Peter Szilagyi**
I just wanted to say that I don't really see it. So if you want to set up a testnet, then set up a testnet for everything that's supposed to be in Berlin. Just picking out the single SIP will make things a lot more horrible because then clients will all of a sudden need to support a weird testnet with half-baked SIPs in it.
...
**Alex Vlasov**
Well, so far I don't know on any plan to make a new draft which would affect the part which is implemented in the precompile, actually only two precompiles out a set of nine, which are parts of this SIP will only be affected by any changes in the future, and everything else that is not affected by changes in draft.
...
**Hudson Jameson**
So FileCoin, Sila Foundation, and I believe StarkWare are all coordinating on which IETF draft of the curve that they're going to be implementing. Again, I think that's seven. I'll check on that. But if your concern is everyone being on the same page, Axic, then I think that we will get information from the Eth2.0 team once we start talking to them about if this is messed up or not, because they'll be looking at it.
...
**Hudson Jameson**
That's just gonna make people think it's an Sila testnet, because that just sounds so similar.
...
**Hudson Jameson**
Oh, Görli is click, isn't it? Everyone supports click, then, yes. Sila JS may not, but I don't think they've had a representative on the call for a while.
...
### Agenda #2 [Eligible for Inclusion (EFI) SIP Review](https://youtu.be/bGgzALuyY3w?t=2836)

Okay, we're skipping EFI so we can go to some of the SIPs that people want to bring up for the first time. I think that's a good idea, at least. What do you think, James? We've kind of gone over some of these before.
...
**Hudson Jameson**
Yeah. So it would be 2315 simple subroutines and 2537 BLS 12-381 curve. Those would be the two additional SIPs that would be involved in this ephemeral testnet.
...
### Agenda #4 [SIP-2565: Repricing of the SIP-198 ModExp precompile](https://youtu.be/bGgzALuyY3w?t=2969)

Next up, I just put this agenda item number four back in from the last meeting. I don't know if anyone's here to speak on it. It's 2565 repricing of the SIP-198 ModExp precompile.

**kelly**
Hey, Hudson. Yeah, I'm here. Let me share my screen real quick.

Great, so hopefully everyone can can see my screen. This should be a fairly quick update just looking for some directional guidance here on which direction to take with this SIP.

So just as a quick refresher, SIP 2565 is to reprice the ModExp precompile. Came in about a month ago after doing quite a bit of benchmarking on GO Sila. We found out that GO Sila was about 10 x overpriced. And so we recommended changing a simple parameter from 20 to 200 to more accurately price that precompile.
...
So what we went off and did last week was go and look at alternative libraries for Parity or Open Sila to use. We found some Rust bindings to - first we looked at just simple version bumps of the Parity precompile, because it was quite a bit out of date, at the suggestion of Alex Vlasov. Unfortunately, these didn't get us near the performance of Geth. We were still sitting about two to four x slower than Geth.

So we went and looked at Rust GMP. We were able to benchmark this, and if Parity or Open Sila uses this, they'll actually be faster than Geth. So this was based off some feedback from Peter two weeks ago. He said, "hey, it seems kind of weird that Rust is slower than GO. And sure enough, if they use this precompile or this library they will actually be faster.

So there are two paths forward with this SIP. The first option is to continue with a simple parameter change for both Geth as well as Open Sila where we change this parameter from 20 to 200, and Open Sila goes in and implements this GMP library instead of what they have today. And that will keep everything above 20 or 25 million gas per second, which is sort of the standard that we're seeing for precompiles.
...
And so I really just wanted to have a discussion. I'd like to move this SIP towards Eligible For Inclusion, and we just need some direction on which of these two options to go with here.
...
**kelly**
Fantastic. That's great feedback. So, should that be successful, it sounds like the path forward with this is to swap out that library and then make the simple parameter change in the pricing formula in both Geth and Open Sila where we change this G quad parameter from 20 to 200.
...
**kelly**
Okay, so just to be clear, you know, is the case that if we are able to compile this Rust GMP library with the the new toolchain on Windows that this is the appropriate path forward for Open Sila?
...
So, essentially, in Geth, when you say that, okay, next hard fork is Berlin, then the hard fork, you just specify one block number and it implicitly collects the SIPs that you enable. So if we want to throw out one SIP from Berlin, that means we need to modify the code so that Geth will not run on the testnet anymore because the Berlin hard fork version - and I don't want to add [crosstalk] versions in Geth.  
...
**kelly**
Okay, great. Thanks, James. Yeah, so it sounds like the plan is we'll make this change from 20 to 200 on this SIP, and I'll work directly with Artem on the toolchain stuff, and we'll get an answer to you on Monday. James, thanks for the feedback.

**Hudson Jameson**
All right. Thanks, Kelly.

### Agenda #5 [SIP-2481: Adding request IDs to SIL protocol request and response objects](https://youtu.be/bGgzALuyY3w?t=3574)

All right. The next item on the agenda is [SIP-2481: Adding request IDs to the SIL protocol request and response objects.](https://github.com/sila-chain/SIPs/pull/2481)
