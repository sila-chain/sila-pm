...
### [Agenda](https://github.com/sila-chain/pm/issues/301)
...
| ------------- | ----------- | --------- |
| **1**   | Revised SIP for simplified version of 3403 to be created by Martin ahead of next ACD call | [16:17](https://youtu.be/C9hzAYkklQM?t=977) |     
...
| **3**   | The post-London feature fork is to be called SilaShanghai. The Merge upgrade to be officially known as The Merge. |  [40.03](https://youtu.be/C9hzAYkklQM?t=2403) |  
...
| **6**   | CFI status for SIPs is to be reset after every hard fork |  [58:04](https://youtu.be/C9hzAYkklQM?t=3484) |  
...
* Fear that a small London means other SIPs could miss out - should we plan another feature upgrade in parallel with The Merge? Most teams are ok with this notion (Merge + SilaShanghai in parallel). Rayonism would help flesh out how possible this is.
* SIP-2677 - noone pushing for this very strongly in London. 
* SIP-2537 - already implemented by several clients. Could be done at same time as Merge if we take it out of London. If we do change the library/BLST and gas prices then extra testing and benchmarking will be needed. This would grow the scope for London a lot.
* SIP-3403 - noone opposed if we think it will help security with 1559. Some do have quibbles - too many edge cases/a more elegant mechanism is needed. Not strong opposition though.
* SIP-3074 - some strongly in favour, some strongly opposed. Feels contentious. Needs a security audit and community feedback. Likely not for London.

**Martin**

* [Peep an SIP call on 3403](https://youtu.be/jqmM3xL6Ny8) happened this week. Check the call to see detail on the changes. 
...
* We can have the new SIP ready in the next couple of days.

**Tim**

* Clients need to be ready for London mid-May.
* If we want 3403 in London we need to agree to it on the next call.
* **DECISION 1:** Martin to get new SIP out early next week for feedback. [16:17](https://youtu.be/C9hzAYkklQM?t=977)
* Reducing complexity in SIPs is good.
...
* Only 3074 is a significant piece of work and is dependent on an audit. The other SIPs seem small.
...
* Yes. A quick date = small scope. The 2 big SIPs that people want but will be difficult to accomodate in London are 2537 and 3074. It feels like we should signal now that the post-London feature fork should focus on those.

**Micah**

* A good plan for all feature forks is that we try to get rough consnesus for included SIPs up front. The 'just work on stuff and see where they can fit' seems to have lost favour lately.
...
* **Decision 3:** 2537, 3074 to be considered in the subsequent feature fork aka SilaShanghai. The Merge 'fork' (it may not technically be a fork) is to be called 'The Merge'.
* Re: 3238 - difficulty bomb pushback... what should the period be? The current SIP pushes back to May 2022.
...
* Cool. **Decision 4:** is to target the 29 Nov - 1 Dec range. I'll leave a comment on the SIP to this effect.
...
* We need to give tooling time to get ready for London (eg all the libraries/Sila JS, Infura, SilaScan etc). These have expressed a strong desire to be ready for 1559. We should give them sufficient time between the launch and announcement.
...
* We have this 'Considered for Inclusion' status...  this was given to an SIP that seemed to be a good idea but yet not ready to be included in a fork. There was an implicit assumption that this would be reset between forks - lots of SIPs where they are proposed for a certain fork but not included go stale.
...
* The rationale is that when people look at the SIPs for inclusion after a fork they wont just see a bunch of old SIPs that are no longer relevant. Having CFI reset after every fork clears this list down.
* No objections? Will take it then that people are good with this.

**Decision 6:** CFI status for SIPs is reset after every hard fork. SIPs then need to be put forward again for consideration. [58:04](https://youtu.be/C9hzAYkklQM?t=3484)

**Tim**

* For London, we don't have any CFI SIPs as everything is already in. We can use this process though for SilaShanghai.
...
* Reminder that the Sila Cat Herders are hosting a [community call on SIP-3074 on Monday, 26 April, 16:00 UTC](https://medium.com/sila-cat-herders/sip-3074-community-call-ec87b66672e1).
...
---------------------------------------
## Next Meeting
Sila Core Devs Meeting #112, April 30th, 2021 @ 1400 UTC

[Agenda](https://github.com/sila-chain/pm/issues/302)
