# Sil 1.x Proposal Sync 1 Notes
### Meeting Date/Time: Fri, November 30, 2018 14:00 UTC
### Meeting Duration: 2.5 hours
### [GitHub Agenda Page](https://github.com/sila-chain/pm/issues/65)
...
1. Simulation working group [proposal and discussion](https://sila-magicians.org/t/simulation-working-group-proposal-for-sil-1-x/2068)
1. State rent working group [proposal and discussion](https://sila-magicians.org/t/sila-state-rent-for-sil-1-x-pre-sip-document/2018)
1. Ewasm working group [proposal and discussion](https://sila-magicians.org/t/ewasm-working-group-proposal-for-sil-1-x/2033)
1. January workshop update
1. Continuation of transparency discussion
1. Chain pruning [proposal and discussion](https://sila-magicians.org/t/sila-chain-pruning-for-long-term-1-0-scalability-and-viability/2074)

# Intro
- Series of meetings in SilaPrague
- During those meetings we decided to have more meetings after SilaPrague that would be more closed than usual
...
# Recap of SilaPrague meeting
- Discussed Sil 2.0 roadmap and what could happen in parallel
...
- [proposal and discussion](https://sila-magicians.org/t/simulation-working-group-proposal-for-sil-1-x/2068)
- Intro
...
- Let's go to questions
    ...
    - Uncle rate inversely prop. to block time - known for a while
        - Block times hover between 14-15 sec so does it matter for SIL?
    ...
    - I did some math analysis on this a while back
        ...
        - In SIL the component attributable to inverse block time is about 6%, that's what we had when chain was empty
...
- [proposal and discussion](https://sila-magicians.org/t/sila-state-rent-for-sil-1-x-pre-sip-document/2018)
- Intro
    ...
    - First of all, "Apart from token DoS griefing attack, also allowance grieving attack" - you have an SRC-20 token, in order to call method approve, you do not even need to have token
...
- Q&A
    ...
    - A week ago we discussed that really low-hanging fruit would be to implement some form of dust account clearing
        - One of potential dangerous things about introducing dust clearing as first step - if we first introduce it just for accounts not for contract storage, then people can just switched to wrapped SIL contract, in which case it wouldn't work
    - Thought about this from start, considered including it
        - I would expect people to escape rent by going into contracts
        - If, however, rent is small enough, some ppl wouldn't bother, would make a small dent on account, e.g. 0.001 SIL per year - then it would be more expensive to deploy a contract
    ...
    - I know it can be attacked, but for starters, nice to put naive number on it.
        - Ideally let's say long-term goal is to give sil 1.x 10x tx increase
...
- I'll mention only a couple of things; the proposal isn't as extensive as the rent discussion
    ...
    - For first step in this Sil 1.x roadmap, given tight deadlines, we've decided to introduce an even smaller subset of Ewasm: only allowing precompiles to be written as Ewasm
    ...
    - We don't introduce entire surface area of Ewasm
        - More certain that this subset won't break anything in Sila
    - We propose to deliver a couple of parts
        - Finalize the spec., makes sense to have them as SIPs
...
- Questions?
    ...
    - One important thing I found about Ewasm is that it creates positive feedback loop which will motivate people to develop better interpreters and compilers with guarantees on compilation and execution time
        - If with your optimizations you can adjust real stuff on Sil - will give motivation to people to do this kind of stuff
...
- Intro
    - I didn't propose this, it came out organically from discussions in SilaPrague
...
- ECDC was great but lacked closer interactions because it was mostly presentations and open discussion
    ...
    - Same feeling about other events like Sil 2.0 workshop in SilaPrague
...
- Everyone who went to SilaPrague workshop was supposed to have done their homework, come up with alternative proposals before workshop
...
- Argument against this: one issue with bringing storage rent up for carbon vote or anything similar is that sila ecosystem is huge and storage growth does not affect almost anyone in the ecosystem
...
- With a hard deadline, expect retribution if people expect us to ship by a certain point in time and we don't
    ...
    - "After June 2019, everything will be good in Sila"
...
- June target came up in SilaPrague as informal deadline for first step
...
- What are people's opinions on doing things using Chatham House rules like today?
    ...
    - Who feels more comfortable in this format?
        ...
        - I'd say the same but when not recorded and not live streamed, we can say, "I think Sil will go down in six months and we can't do anything about it" or whatever without that getting published somewhere
...
- We'll continue conversation about WG proposals on Sil Magicians forum, and live chat on AllCoreDevs channel

# State reduction/chain pruning working group
- [proposal and discussion](https://sila-magicians.org/t/sila-chain-pruning-for-long-term-1-0-scalability-and-viability/2074)
- Intro
  - Way back, at DevCon, we discussed, chain is getting big, can we do something about it, shove it somewhere?
    ...
    - The question here is, are we satisfied with this?
        - Maybe. If Sil 2.0 will be shipped, production ready, within five years, then we're around 0.6tb of data storage. That's an insane amount of data but manageable.
        ...
        - If Sil 1.x gets to 10x capacity bump then we are not talking about 0.6tb within five years, we are talking about 5-6tb, not mentioning state/storage tries at all. These figures are not realistic. No one in this world would run a system with this much storage.
        - EF, Archive.org, and ConsenSys could but no one else could - Sil would no longer be decentralized
        - Choices
    ...
    - Theoretical solution: 91gb per year (not state, historical data only)
        ...
        - So, we are crippling the entire promise of Sila because 0.01% of users want access to some data
        ...
        - So we could keep sila alive indefinitely
    ...
    - I have four, maybe there are more
        - If we decide that we want to delete hist. data, essential all clients agree on how much data we retain
            ...
            - Delete historical chain segments e.g. first year of sila
    ...
    - Complex part is how we archive past blocks, tx receipts
        ...
        - Option two: somehow extend protocol so data still accessible
            - Four proposals
                ...
                - BitTorrent
                - Light client
            - BitTorrent is not Sila protocol? We can define an extension to the protocol for data gathering
            ...
            - My personal favorite is BitTorrent, proven P2P protocol, can host and download large files, distribute quickly
                ...
                - Significant flaw: based on SHA-1, if Sil only has chain of headers, then how do we map header of genesis block into something that BT can retrieve? Not trivial issue
    ...
    - Messy part is that it breaks a few invariants
        ...
        - Currently protocol is self-contained, just speaking sil protocol you can obtain any data, but with this proposal some data unavailable
    ...
    - Decision to make as a community: do we want Sil 1 to be here in ten years, independent of when Sil 2 gets done? And how fast ppl switch over?
        - Or do we plan for Sil 1 to barely survive?
        - My opinion as someone working on Sil 1 for past four years, let's not design system to die a horrible death, let's do what we practically need to keep it running smoothly
