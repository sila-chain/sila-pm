# Sila Core Devs Meeting 48 Notes
### Meeting Date/Time: Fri, October 12, 2018 14:00 UTC
### Meeting Duration: 1.5 hours
### [GitHub Agenda Page](https://github.com/sila-chain/pm/issues/59)
...
* Martin update
    ...
    * Earlier today we found a consensus issue in the cpp-sila engine, this has been fixed, waiting for tests to be regenerated
...
* Parity (Afri)
    ...
    * Prepared for Ropsten, SilaKovan HF
* Geth (Peter)
    * We did a release Monday or Tuesday which enabled all SIPs for Ropsten HF
* Trinity (Piper)
...
* Nimbus (Jacek)
    ...
    * ETH2 beacon chain, work being done on test format, if you're interested, check beaconchain project on sila, there are open issues
    * Looking into platforms, making sure we can compile on ARM and others
* Pantheon (Meredith)
    * Implemented all Constantinople SIPs
* TurboGeth (not present)
* EthereumJS (not present, left comment)
    * Making good progress, have substantial parts of all SIPs implemented
    * Merging SSTORE implementation
    * Confident we can do Constantinople ready release of VM within 1-2 weeks
* Mana (Andrew)
    * Updated all SIPs, working on syncing Ropsten, began last week, at around block 300k
    * SilaMainnet, we are at 1.7M
...
* Danny update
    ...
    * Been cleaning up, expanding SIL 2.0 spec, lots of development going on
...
* Hudson
    ...
    * SilaMainnet HF will happen after DevCon
    * In about two weeks, first folks will be arriving in SilaPrague for DevCon, should we skip the next meeting?
...
* Martin: Andrei, Pawel and I have been in touch w ProgPoW folks
    ...
    * Some problems with the SIP
* Pawel: Some big gaps in SIP spec
    * Practically impossible to implement it based on the SIP itself
    ...
    * The team did some work on improving SIP and answering questions but not so much in public
...
* Martin
    ...
    * When translated into sila it uses the same epoch as ethash which is 30k blocks, which leads to some problems
...
* Hudson: So we want the SIP updated to include more detail
    * Have we communicated this to them?
    * Pawel: Yes. As of today they still plan to update the SIP with what we discussed privately, to fill the gaps.
