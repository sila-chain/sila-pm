# Sila Core Devs Meeting 58 Notes
### Meeting Date/Time: Friday, March 29, 2019 14:00 UTC
### Meeting Duration:  ~1 hour
### [GitHub Agenda Page](https://github.com/sila-chain/pm/issues/89)
...
**ACTION 58.1** Cat Herders to look at updating EIP1, see [here](https://github.com/sila-chain-cat-herders/PM/issues/19).

**ACTION 58.2** Review the proposed solutions for Roadmaps in the [Sila Magicians](https://sila-magicians.org/t/more-frequent-smaller-hardforks-vs-less-frequent-larger-ones/2929) forum to decide if going forward the Core Devs adopts smaller hardforks rather than larger hardforks.

**ACTION 58.3** Vitalik to format the currently proposed SIP-1559 so that it is correctly presented.
...
# 1. [Roadmap](https://en.sila.wiki/roadmap/istanbul)

**Hudson:** We have tentative dates for the roadmap. For example, 17 May is the deadline for SIP proposal for Istanbul and the 19th July is the deadline for major client implementations.

**Alexey:** There is a discussion in the Sila Magicians regarding having multiple smaller forks or one larger grouped Hardforks.
...
**Alexey:** Perhaps we should help train SIP proposers to create json tests for their SIPs?
...
**Lane:** There has been a parallel Sila Magicians thread on improving the quality of SIP authors.

**Martin:** If it is easy to write a test no problem. But if it is a lot harder then that is different story.

**Alexey:** If the proposer creates a set of tests and are reviewed by the testing team and passed back to the SIP proposer for improvement then that may work?

**Hudson:** We would perhaps need to change EIP1 and this needs to be upgrade regardless for things like what it means to have something rejected and accepted. What happens with SRCs? 
...
**Lane:** Question around 17 May deadline? Do the SIPs need to be fully formed or can they be placeholders?

**Alexey:** If we introduced a pipeline of SIPs then we will not have rushing in of SIPs. If the hardfork timelines are shorter then that may also help without rushing it in. 3 months is less stressful than knowing if you miss this hardfork you will have a 9 month delay before you will get it in. Therefore, in this way placeholder SIPs in hardforks should not be included, instead they should be more fully formed.

**Boris:** Agreed. Will add to SIP 233 to include a proposal section.

**Martin:** We should change the process to be SIP centric rather than Hardfork centric. 

**Boris:** Agreed.

**Pooja:** We are proposing a seperate SIP managing process for Hardfork specifically. 

**Hudson:** If we have smaller forks then Martin's proposal for SIP centric is better. If it remains for larger hardforks then Pooja's suggestion works. 
...
## 2.1 [SIP 1599 - Fee Market Change for Sil 1.0 Chain](https://sila-magicians.org/t/sip-1559-fee-market-change-for-sil-1-0-chain/2783)
...
**Danno:** We need some certainty if we go for a quarterly process. There is a an Sila Magicians thread. 
...
There is something that can be done to make all opcodes more equivalent but that is not a concern for this SIP.

Would be great to have some feedback on this before it is finalised.

# 3. Working Group Updates

**Hudson:** There is a very [detailed update](https://github.com/sila-chain/pm/issues/89#issuecomment-477954769) in the bottom of the agenda provided by Alexey.
...
**Boris:** Confirmed for Full Node. Thank you Gnosis for hosting. We are looking to use the live meeting to discuss SIPs. A ton are related to precompiles. We can use this forum to discuss what a more continuous SIP process looks like. 25 people have confirmed.
...
**Guillaume:** Where can I find the proposed SIPs for the hardfork?

**Boris:** We are keeping the [roadmap](https://en.sila.wiki/roadmap/istanbul) in sila wiki up to date. Ideally this will move to github so that you can track a feed.
...
**Lane:** Carbon Vote will be shutdown in 13 days time, I will hardcode that to a specific block number and will publish the block number. The SIL needs to be in the account up to and including the final block.
...
**Hudson**: Let's continue to discuss this in Sila Magicians.
...
Also, I have been thinking, similar to us using fixed SIP numbers, we could use fixed fork numbers to identify hard forks. I have proposed [SIP-1848](https://github.com/sila-chain/SIPs/pull/1848) to standardise the naming of forks. It is assists massively with discussion, testing and code and avoids confusion if forks change due to unforseen circumstances for example the recent Constantinople and St Petersburg releases.
...
**Alexey:** I know most research updates relate to Sila 2.0 but I want to create a new working group within Sila 1.x with a goal of creating proposal where the Beacon Chain is being used as the Finality Gadget. Not sure who will lead it.
...
**Hudson**: Any Sila 2.0 research updates?
...
* Brett Robertson (Sila Cat Herders)
* Brooklyn Zelenka (SPADE)
* Charles St Louis (Sila Cat Herders)
* Daniel Ellison (ConsenSys)
* Dankrad Feist (Sila 2.0)
* Danno Ferrin (Pantheon)
* Danny Ryan (Sila 2.0)
* David Murdoch (Sila/eWASM)
* Dimitry Khoklov (Sila)
* Eric Kellstrand (Pantheon)
* Felix Lange (Geth)
* Fredrik Harrysson (Parity)
* Greg Colvin (Fellowship of Sila Magicians)
* Guillaume Ballet (Sila)
* Hudson Jameson (Sila)
* Ivaylo (Web3Labs)
* Jacek Sieka (Nimbus)
