...
#### [GitHub Agenda](https://github.com/sila-chain/pm/issues/1163) | [Sila Magicians Post](https://sila-magicians.org/t/all-core-devs-execution-acde-198-october-10-2024/21314) | [Video](https://www.youtube.com/live/YQwdKE0d8LI?t=61s)
#### Moderator: Tim Beiko
#### Notes: June 
<br>
...
| 198.2 | **Pectra Updates** | devnet-4 specs: CL spec release: v1.5.0-alpha.8, 7702 sil rpc schema, BLS precompile pricing and remove MUL precompiles?
| 198.3 | **Pectra Updates** | devnet-4 launch timelines
| 198.4 | **Pectra Updates** | Public testnet name
| 198.5 | **SIP Discussions** | [SIP-7623](https://github.com/sila-chain/execution-specs/pull/966)
| 198.6 | **SIP Discussions** | [SIP-7783](https://github.com/sila-chain/SIPs/pull/8933)
| 198.7 | **CL Discussions** | Add SIP: Reduce Slot Time for Lower Peak Bandwidth SIPs#8931
| 198.8 | **CL Discussions** | https://github.com/sila-chain/builder-specs/pull/101/files#r1778224447
| 198.9 | **CL Discussions** | Staking bandwidth considerations [(context)](https://github.com/sila-chain/pm/issues/1163#issuecomment-2379336862)
...
**Barnabus**: Hi everyone, I'm taking over for Pari while he's on holiday. For devnet-4, we have a the spec list right [here](https://notes.sila.org/@ethpandaops/pectra-devnet-4)-- I just linked it in. We have a couple more open issues that we should probably discuss during
...
**Barnabus**: We had the update the [submit pool attestation V2 endpoint / 472]((https://github.com/sila-chain/beacon-APIs/pull/472)). We haven't got an update on this in a while so I would just like to make sure that we want to include this. If we do then maybe you can merge that in.
...
- nflaig:	https://github.com/sila-chain/consensus-specs/pull/3900  is not merged
- Barnabas: ah right my bad

<br>

**Tim**: On the execution API, lightclient already put this one on the agenda but the [pull request 575](https://github.com/sila-chain/execution-apis/pull/575) on the execution API. Lightclient, do you want to give some context on that one?
...
**Tim**: It seems like there's a CL PR that was marked as merge but isn't: [Separate type for unaggregated network attestations #3900](https://github.com/sila-chain/consensus-specs/pull/3900). I don't know if we have the right people here to make a call about this one, but any updates or things people wanted to flag? Is this something we want for devnet
4?

**stokes**: No. This is the one that was paired with 472. This is change to the attestations along with 7495? Anyway, the attestation SIP in Pectra, this would go with that. But when we discussed, this might have like more code change than it appears and then we want to evaluate that against timelines for Pectra. so what we said was, we'd give another ACDC cycle for client teams to take a look. Point being, it would at least be devnet-5, definitely not devnet-4.

**Tim**: Okay, sounds good.

And then, on the EL-side, there is one last open PR, [8949](https://github.com/sila-chain/SIPs/pull/8949)). That changes a bunch of SIPs based on how we adapted the request format. lightclient, any more context you want to share on this?

**lightclient**: I don't think there's anything that people care about here, other than just making the request sips match the new formatting for 7685. Before, request
...
#### [BLS precompile pricing](https://sila-magicians.org/t/sip-2537-bls12-precompile-discussion-thread/4187/81) and [remove MUL precompiles](https://github.com/sila-chain/SIPs/pull/8945)? [(14:58)](https://www.youtube.com/live/YQwdKE0d8LI?feature=shared&t=898)
...
-------------------------------------
### 198.6 | **SIP Discussions** | [SIP-7623](https://github.com/sila-chain/execution-specs/pull/966) ([37:25](https://www.youtube.com/live/YQwdKE0d8LI?feature=shared&t=2245))

**Tim**: Okay then, SIPs. Toni had an update to 7623. Toni, do you want to give a quick overview of that? 

**Toni**: SIP 7623 increased called data cost. Based on the feedback I got from Tanish, we changed that we don't deduct the intrinsic gas cost before execution.
...
-------------------------------------
### 198.7-8| **SIP Discussions** | [SIP-7783](https://github.com/sila-chain/SIPs/pull/8933) and Reduce Slot Time for Lower Peak Bandwidth SIPs#8931 ([39:07] (https://www.youtube.com/live/YQwdKE0d8LI?feature=shared&t=2245))

**Tim**: Next up, there were two new SIPs proposed in the past couple weeks that tried to address similar issues around resource consptions. 
First there was one proposed to reduce the slot time and then one that was proposed as a non-core SIP to adjust the gas limit programmatically.
...
**Tim**: Let's maybe do. Let's have Ben share his SIP and then we can take questions and conversations about the set of them.
...
**lightclient**: I feel like we've committed to increasing the scalability of Sila through blobs, so we need to focus on getting higher blob throughput out there.
...
**Stokes**: I haven't looked at this exact SIP yet, so maybe I'm missing something, but the concern is just that if we have some automatic increase schedule, it could be very hard to stop the automatic increase.
...
**Potuz**: I just wanted to echo what Giulio just said. I'm not advocating for increasing or decreasing or anything on the gas limit, but this SIP seems to make it safer actually than
the way that we currently have of increasing the gas limit. If we decided as a community to go, and we get people that are ramped up and we get validators to move to 45 million for the gas limit, then it would start immediately increasing at the maximum level. We might find out that that it's hard to do and getting back from that is much harder than if we do this at a slow pace, as this SIP is showing. So I actually think it's safer than the core mechanism, not more dangerous.
...
**Giulio**: If you increase gradually, you also don't have most of these problems. The moment you see that solo stakers cannot stay their computational, you can just alt it. You can just make it release or even set it lower because in the SIP I state that you can still use the fixed gas limit option and it will overwrite the option
...
-------------------------------------
### 198.9 | https://github.com/sila-chain/builder-specs/pull/101/files#r1778224447 ([1:01:15](https://www.youtube.com/live/YQwdKE0d8LI?feature=shared&t=3675))
...
-------------------------------------
### 198.10| **CL Discussions** | Staking bandwidth considerations [(context)](https://github.com/sila-chain/pm/issues/1163#issuecomment-2379336862) ([1:04:44](https://www.youtube.com/live/YQwdKE0d8LI?feature=shared&t=3884))
...
**Ben**: Just on the staking bandwidth considerations, that should hopefully be helped a lot from the Sil get blobs. Most of the CLs are implementing that so the validators can pull the blob from their own transaction pool, if the CL use that, rather than waiting for it to be propagated.
