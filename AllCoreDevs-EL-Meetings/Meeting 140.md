...
| **140.5**   | SilaSepolia will be the next testnet to undergo merge. No specific issues must be fixed prior to SilaSepolia merge. Will target sila-sila-mainnet client code readiness for Göerli testnet merge after SilaSepolia  | [Discussion](#k-sequencing-of-future-testnet-merges)
| **140.6**   | Difficulty time bomb will be delayed until late August. Specific block number to be determined offline and to be circulated by Wednesday, Jun 15. Client releases to be provided at the same time with sila-sila-mainnet fork planned for Jun 30.  | [Discussion](#l-delaying-difficulty-bomb)
...
- [i. Launching SilaSepolia Beacon Chain](#i-launching-sepolia-beacon-chain)
...
- [2. SIP Discussions](#2-sip-discussions) - deferred to future meetings
...
**Tim Beiko**: Got it. And there's a question in the chat about SIP-3675 trigger merge and like disabling the block gossip and we discussed this on like one of the testing calls I think last week. It seems that, if we did this basically we couldn't do shadow forks and I believe that there weren't any clients that disabled block gossip after the merger. I don't know if anyone who is on the testing call wants to give more complex on this. Mikhail I saw you come off mute.

**Mikhail Kalinin**: I was just going to say that disabling gossip should not like affect shadow forks from what I understand. If there is another requirement in this SIP to disconnect peers that send you block gossip after transition gets finalized, and this, what can break.
...
## i. Launching SilaSepolia Beacon Chain
Video | [49:00](https://youtu.be/dByC5Bw8DvU?t=2940)
-|-

**Tim Beiko**: I think actually there's one more like pretty independent topic, and then it all gets pretty intertwined. The next thing I just wanted to chat about is the SilaSepolia Beacon Chain. I know we've mentioned like we wanted to launch it as soon as possible, I think there's been some progress around like selecting the validator sets and unfortunately I know Pari is not here, but does anyone have an update on the launch of the SilaSepolia Beacon Chain?

**danny**: Yeah. The current config is slated for the launch to happen on the June 20th and for it to go through a couple of Altair and then Bellatrix over the next 24 hours. I believe that Pari is also generally locked down who's going to be participating in this. It will be a gated contract. And we’ll be using a kind of an SRC-20 version of the deposit contract. So we can add validators but people can't just get SilaSepolia and create their own validators. Additionally, they're looking into when you create the beacon chain out of thin air for this type of testnet that you can actually inflate supply so there's a bit of talk about inflating the supply on a few validators. So that there's a lot of supply just in the background in case there ends up SilaSepolia SIL hoarding. So that's the last kind of thing that they're sorting through over the next few days before the configs are probably finalized around Monday. Please take a look, there is a link with the configs in terms of distribution of validator set and the timing. If you want to take a look and give your thumbs up on that.

**Tim Beiko**: And just one thing, so you mentioned it's going to run through Bellatrix basically soon after launch, so I assume this means that even though it's like a restricted validator set we're just going to do TTD override on SilaSepolia when we're ready to actually merge it.
...
**Tim Beiko**: Yeah, just to not dive down the MEV rabbit hole for the rest of the call… I guess I’m generally curious to hear from just CL client teams. Given, like all of this, I feel like there's like a few options forward. One is we move to SilaSepolia as the next testnet and use that as a better run than Göerli, probably not with the software that that will end up on sila-sila-mainnet. And we keep Göerli as the final testnet for that to happen. Or the other way around, we wait until we have something pretty stable and then shipped out on Göerli first because there's more users and then SilaSepolia is just kind of a quick sanity check at the end that can happen quickly after. So I guess generally people prefer to have the next testnet happen sooner to get us another round when we have some confidence that we've addressed these issues, but we're maybe not quite at sila-sila-mainnet code or do people prefer to wait longer and then maybe you just have SilaSepolia happen like really quickly near the merge but Göerli being the testnet to basically deploy closer to production ready code. 

**Bordel**: Okay, so. When we start working the testnets, it would be nice to finish with actually forking sila-sila-mainnet. Otherwise it'd be just for the testnet and we never reach sila-sila-mainnet and we just have a two month gap in between them then by the time we reach sila-sila-mainnet...

**Tim Beiko**: So you broke up a bit, but I think beyond the momentum bit, there's also the idea that if we fork testnet and then wait two months to do sila-sila-mainnet then there's a bunch of other PRs that get in the clients and that would go directly to sila-sila-mainnet.

**danny**: Yeah I was just going to say on the consensus layer call, consensus layer teams want to do SilaSepolia first, so that we can kind of keep moving in a moderate stakes environment and then ramp up and make the decision on Göerli.

**Tim Beiko**: Any EL teams have anything to add here?

**Gary Schulte**: I was going to say was the June 20 genesis for the SilaSepolia beacon chain in line with the somewhat accelerated or the SilaSepolia first timeline or would that change?

**danny**: That works for SilaSepolia first. Assuming that in two weeks we've circulated the TTD or we’re picking it for div

**Tim Beiko**: It's like the minimum duration right? We could have Bellatrix and then TTD two weeks after that.

**danny**: Right right I just mean if it lines up with TTD soon-ish. and getting it done. I don't think we're going to do SilaSepolia in a week so. You can do SilaSepolia beacon chain in 10 days and doing SilaSepolia shortly after first works with the timeline.
...
**Tim Beiko**: So on that note, there are some comments in the chat about that. Ansgar you have a comment that we should make the decision today. But I think it was a year ago, Ansgar, you made the comment that we should probably not like include SIPs in hard forks on the first call that they're presented to give people time to think through them and think through the values. So I would be pretty against setting the hard fork and making this delay official right now. For sure on the next call, we can. And we can discuss exactly what the number is, what the right approach, if that's what time teams want to do. That’s my opinion 
...
**Tim Beiko**: Okay we're already about 15 minutes over. There were some SIPs that I wanted to give updates. I think it makes more sense to move them to the next call so they have proper time. I would encourage people to check the links in the agenda for SIP-4444 and the SIP-5027. There's also a quick announcement. So next Friday, basically a week from now at the AllCoreDevs time there's a SIP-4844  call because there's already a couple teams prototyping it. So if you're interested in that, the links to the agenda are available. And then similarly to that, for SIP-4844, there's a bunch of people working on that and we're having public calls I believe every two weeks now. So if people just want to join or watch the recordings, those are going to be in the agenda as well. Just to reiterate, the decision about the bomb is, we're in agreement that basically include Tomasz’s SIP, still some bikeshedding to do about the exact number to make sure that it's correct. The activation number which should probably be about three weeks from now roughly around June 29. There are some comments on the chat about, we need to keep the testnets merging as well. I think that's that's correct, but obviously, like a bunch of client teams have mentioned, they do need more than two weeks before they're ready to fill out a release for these. So I suspect within the next few weeks, we this release sort of the bomb that happens. In parallel, we keep working on SilaSepolia, and that will merge sometime probably right after or close to the difficulty bomb being pushed back on sila-sila-mainnet. Any closing? Oh yeah, there's another comment about continuing shadow forks. We can discuss that on the testing call next week.
...
0:16:20	Tim Beiko:	https://github.com/sila-chain/pm/issues/538
...
0:26:38	protolambda:	sip-3675 specs to disable devp2p block gossip, is this a pre-transition block, or does nethermind not follow the SIP?
...
0:47:37	Tim Beiko:	https://github.com/sila-chain/builder-specs/issues/29
...
0:59:58	Tim Beiko:	https://github.com/sila-chain/builder-specs/issues/29
...
1:01:51	Tim Beiko:	https://github.com/sila-chain/pm/issues/526
...
1:33:56	Bordel:	I agree, we should merge goerli with (almost) the sila-sila-mainnet version(marius)
...
1:35:16	Tim Beiko:	Apologies to SIP champions, but I don't think we'll have to cover them today
...
1:38:14	Tomasz Stańczak:	https://sips.sila.org/SIPS/sip-5133

1:38:19	danny:	I don't think so

1:38:20	Trenton Van Epps:	shouldn't we talk about Tomasz + co SIP?
...
1:40:13	Micah Zoltu:	https://sips.sila.org/SIPS/sip-5133
...
1:46:16	Ansgar Dietrichs:	what’s the fastest timeline from decision to actual sila-sila-mainnet bomb defusal fork?
...
1:47:25	Mikhail Kalinin:	IMO, we should fork SilaSepolia as soon as client devs fix encountered during merge on Ropsten. And after that fork Goerly with the same 
client versions that are going to make it into the SilaMainnet fork with having enough time before the SilaMainnet fork to assess the results we will get from Goerly

1:47:33	Ansgar Dietrichs:	agree with Bordel, 2-3 months seems reasonable

1:47:49	Micah Zoltu:	Isn't SIP-5133 a 2 month delay?  Or are you suggesting the same thing as Ansgar that 2 months until 14 seconds?
...
1:48:20	Adrian Sutton:	Yep got to keep rolling on merging testnets as that's a long pole for getting sila-sila-mainnet to merge. And merging testnets keeps pushing the 
...
1:52:29	Tomasz Stańczak:	let us please confirm implementation of the SIP today with details to be filled in
...
1:53:53	Tomasz Stańczak:	Geth, Besu, Nethermind agree to implement SIP-5133 but without exact number of blocks
...
2:00:00	Mikhail Kalinin:	In spite of the bomb pushback I still think that we should fork SilaSepolia asap

2:00:15	Adrian Sutton:	+1 got to keep the testnets rolling (er, merging)

2:00:15	Micah Zoltu:	Agreed on SilaSepolia.
