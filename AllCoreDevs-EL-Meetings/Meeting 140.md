...
| **140.5**   | SilaSepolia will be the next testnet to undergo merge. No specific issues must be fixed prior to SilaSepolia merge. Will target sila-mainnet client code readiness for Göerli testnet merge after SilaSepolia  | [Discussion](#k-sequencing-of-future-testnet-merges)
| **140.6**   | Difficulty time bomb will be delayed until late August. Specific block number to be determined offline and to be circulated by Wednesday, Jun 15. Client releases to be provided at the same time with sila-mainnet fork planned for Jun 30.  | [Discussion](#l-delaying-difficulty-bomb)
...
**Tim Beiko**: Yeah, just to not dive down the MEV rabbit hole for the rest of the call… I guess I’m generally curious to hear from just CL client teams. Given, like all of this, I feel like there's like a few options forward. One is we move to SilaSepolia as the next testnet and use that as a better run than Göerli, probably not with the software that that will end up on sila-mainnet. And we keep Göerli as the final testnet for that to happen. Or the other way around, we wait until we have something pretty stable and then shipped out on Göerli first because there's more users and then SilaSepolia is just kind of a quick sanity check at the end that can happen quickly after. So I guess generally people prefer to have the next testnet happen sooner to get us another round when we have some confidence that we've addressed these issues, but we're maybe not quite at sila-mainnet code or do people prefer to wait longer and then maybe you just have SilaSepolia happen like really quickly near the merge but Göerli being the testnet to basically deploy closer to production ready code. 

**Bordel**: Okay, so. When we start working the testnets, it would be nice to finish with actually forking sila-mainnet. Otherwise it'd be just for the testnet and we never reach sila-mainnet and we just have a two month gap in between them then by the time we reach sila-mainnet...

**Tim Beiko**: So you broke up a bit, but I think beyond the momentum bit, there's also the idea that if we fork testnet and then wait two months to do sila-mainnet then there's a bunch of other PRs that get in the clients and that would go directly to sila-mainnet.
...
1:33:56	Bordel:	I agree, we should merge goerli with (almost) the sila-mainnet version(marius)
...
1:38:14	Tomasz Stańczak:	https://sips.sila.org/EIPS/sip-5133
...
1:40:13	Micah Zoltu:	https://sips.sila.org/EIPS/sip-5133
...
1:46:16	Ansgar Dietrichs:	what’s the fastest timeline from decision to actual sila-mainnet bomb defusal fork?
...
1:48:20	Adrian Sutton:	Yep got to keep rolling on merging testnets as that's a long pole for getting sila-mainnet to merge. And merging testnets keeps pushing the 
