...
### [GitHub Agenda](https://github.com/sila-chain/pm/issues/342)
...
Because open sila seemed to signal that they rather not provide effective gas price before London for transactions. 
...
I think we do the same at open sila on the other hand it think it is a good idea to return effective gasprice so maybe we will do that
...
Because all the infrastructure that is in place currently in sila has only a single place to retrieve the gasprice from and that is the transaction object called gasprice so it means post fork every single software that ever touches an sila transaction needs to be changed and is violent in my opinion
...
I assume its like a common use case imagine you go on sila scan its like something you want to see in your transaction, but then they also show you everything else right
...
So I guess we can probably go with that obviously if someone feels very strongly in the future they can make an SIP to change it and if we announce that this field is being deprecated before the next hard fork or something somebody can also step up and say don't depreciate this it breaks my app for these specific reasons, does that make sense?
...
We also agree to have the effective gasprice in pre SIP 1559 blocks and LightClient said he will add a deprecation notice in the 1559 transaction format in the JSON RPC spec so that people are aware that this is happening
...
The reasons I am asking is if there is an sila.org blog post saying download this release for Besu you probably want the one with that transaction fix right?
...
Yeah I guess it seems like at a high level every client team sorts the transaction pool by basically the effective tip or what goes to the miner and requires a bump of the effective tip by at least 10% and 12% for open sila
...
I just shared the link to the [wiki page](https://github.com/ledgerwatch/erigon/wiki/Transaction-Pool-Design#changes-for-sip-1559) in the chat here if anyone wants to have a look
...
Yes I am posting a [link](https://github.com/sila-chain/go-sila/blob/master/core/tx_list.go#L299), and we can continue just for reference since I mentioned it
...
The next step and what is the priority now is to actually write a specification for the execution layer which will consist from a couple of SIPs on the consensus side on the core protocol side and also the specifications for the state and block sync
...
Right and yeah this Ransdom or random thing is going to get put into an SIP so it will definitely will be discussed with a lot of care 
...
I can add something a bit unrelated but touching so for the past few months we have been working in the background with super non priority on synchronization stuff for post merge and one of the things we kind of realize is that it is super annoying to implement a thing for sil 65 and below
sil 66 introduces request ____ which makes things cleaner and its just a pain in the ass to implement them both for the new request ___ models and the old non-request ______ models so as promised last year October geth will be dropping 65 after London ships so that we can focus on the merge work
...
in what is in mind is to write the SIP drafts or spec drafts and share them and discuss or should we do anything else before this
...
But basically we kind of agreed to after London we focus on the merge and potentially have shanghaied before the merge if we see that the merge is unlikely to happen by December or have SilaShanghai after the merge if the merge likely to happen by December
...
My two sense is that we cannot focus on two completely different things so if we start doing work for the merge and then in the meantime well maybe we should do SilaShanghai then its just going to be weed of situations where nothing gets done
...
I think I was also curious like even higher level than specifically SilaShanghai and the merge with how as core developers do people feel about scheduling forks
...
even though you are not ready at exactly the 6-month deadline you know that like 3ish months in you can't accept anymore SIPs and things just need to be kicked out in the next one
...
like this fending off the SIPs and different ideas and people trying to push them just to happen in this coming fork

It was very tiring and distracting like, so we could have the conversations and there were lots of conversions on the sides when we were discussing bigger changes the research bigger SIPs but the moment when they were changing into oh so maybe lest have them in London that was very tiring and distracting because suddenly it was a bit harder to plan the fork that was just like one or two months away.
...
I just wanted to briefly say that as one of the SIP authors of 3074 which was like a good case study here in the sense that I think if there was kind of clarity that lets say for example we have 2 feature forks every year, and they are 6 months spaced out 

Then it would be very easy to say yeah the SIP isn't ready yet there are some security concerns lets kind of take a step back and address them then we know if it's realistic to target this one or the next one.

I feel like a lot of these problems with SIPs really being pushed hard also comes form this uncertainty where people have the fear that if they don't make a specific fork its completely unclear how long they have to wait.

So this predictability around forks would also help greatly reduce the amount of pressure people feel to get their SIP into the specific next fork or else their SIP will be lost forever.
...
so basically we have these latent test nets where ones A fork we have a cutoff so say 3 months before fork, and then we basically look at the test nets and take all the uncontroversial SIPs that have been fully agreed upon and that have been also fully implemented and tested already and just take them, so we don't have to discuss much 
...
I know that when I was planning SIP 1559 I thought in March 2020 I thought we would get it done by the end of 2020 and the that early or mid 2021 which is now we would have stateless live and I really wanted to have that done before stateless 
obviously the path around state expiry has changed a ton, SIP 1559 took like 3 times as long 
...
and that will probably hit us right after the merge because people will want withdraw their sila from the beacon chain back into the execution layer, so I suspect well get to test drive that pretty quickly 
...
I would like to keep on insisting on naming the next fork always so when we have conversation about London the more time we mention the name SilaShanghai the more likely it will be that people will start feeling that it is a real thing that happens next, and then they can plan a bit more.

Just simply naming it in conversations would possibly already allow us in the previous weeks to move some of the SIP to be in the next one 
...
and people were saying no yeah we will have SilaShanghai it will be in November or December and maybe it will be the merge then there was this whole "The merge is SilaShanghai" meme 
...
There will be a SilaShanghai in November because the difficulty bomb goes off in december, so we have to do something like there will be a patch that will go out in november, and we can call it shanghai

**Tim**

Yeah, but it's a big difference whether thats Muir glacier part 2 or whether it includes say SIP 3540 or 3074
...
yes but I think from the perspective of SIP champions if you tell them you might be in shanghai but realistically nobody thinks we are going to do feature work aside from the merge then it doesn't really solve the problem
...
I would really like to tell people that "oh yeah it seems like something that should go into shanghai or I think this is more reasonable for SilaCancun and give them the rough dates of November and may.
...
[Agenda](https://github.com/sila-chain/pm/issues/354)
...
09:37:35 From  MariusVanDerWijden  to  Everyone:
	Code: https://github.com/sila-chain/go-sila/blob/9624f92edef5e0a76a97efd302e983077acb6e35/core/tx_list.go#L279

09:37:45 From  MariusVanDerWijden  to  Everyone:
	We require bump in both fields

09:38:12 From  Tim Beiko  to  Everyone:
	https://github.com/ledgerwatch/erigon/wiki/Transaction-Pool-Design#changes-for-sip-1559

09:38:19 From  Tim Beiko  to  Everyone:
	Erigon’s design ^

09:40:37 From  Péter Szilágyi  to  Everyone:
	https://github.com/sila-chain/go-sila/blob/master/core/tx_list.go#L299
...
10:05:56 From  Tomasz Stańczak  to  Everyone:
	I was opting for more forks but only when people do not airdrop additional 3 SIPs each time
...
10:06:15 From  Tomasz Stańczak  to  Everyone:
	like London -> I was fiercely defending less SIPs
...
10:08:34 From  Ansgar Dietrichs  to  Everyone:
	I think it is hard to speculate about the counterfactual, but in my view the difficulty bomb help keep Sila commited to a full PoS shift in the future, and avoid the risk of that being contentious
...
10:13:35 From  Alex B. (axic)  to  Everyone:
	The result then was to try a process where sips are considered based on readyness and not tied to a HF.
...
10:14:30 From  lightclient  to  Everyone:
	it feels like we need a much longer term vision of things, we shouldn't even allow the discussion of new sips added to a hard fork < 6 months before the fork
...
10:16:57 From  Ansgar Dietrichs  to  Everyone:
	would there be value in having regular & hard cutoffs for inclusion decisions, but then a “whatever time it takes” for testing & rollout after? so e.g. “we decide which SIPs to include in the next fork on the first call in April and October, and then fork whenever ready after that”
...
10:20:21 From  Ansgar Dietrichs  to  Everyone:
	ah right, forgot about that, Sila will finally have reached Serenity! ;-)
...
10:29:29 From  Micah Zoltu  to  Everyone:
	FWIW, I can appreciate how stuffy and anti-agile pre-planned hard forks is, but I think Sila core development has reached a scale where the coordination value of having a relatively strict schedule outweighs the benefits of a more agile solution.
