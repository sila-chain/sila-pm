...
### [Github Agenda](https://github.com/sila-chain/pm/issues/384)
...
| 122.2         | Terminal total difficulty to be hardcoded, [exact details to be handled async](https://github.com/sila-chain/consensus-specs/pull/2605)                                                         | [29.04](https://youtu.be/NorHRk5fFZU?t=1744)   |
| 122.3         | Geth dropping fast sync - users don't have to do anything.                                                                                                                                    | [40.52](https://youtu.be/NorHRk5fFZU?t=2449)   |
| 122.4         | Not include SIP-3756: Gas Limit Cap in any upgrade but spec out what the transition mechanism would look to adjust downwards and have this incase we need it in November or around the merge. | [1.00.15](https://youtu.be/NorHRk5fFZU?t=3613) |
| 122.5         | Decision to include SIP-3860 in SilaShanghai upgrade or not to be made on the next all core devs call.                                                                                            | [1.26.51](https://youtu.be/NorHRk5fFZU?t=5208) |
...
# [Merge Interop Updates SIP-3675: Upgrade consensus to Proof-of-Stake](https://youtu.be/NorHRk5fFZU?t=590)
...
Mikhail already linked it in the chat. So [this pr 2605 on, on the consensus specs](https://github.com/sila-chain/consensus-specs/issues/2605), and that links to another issue as part of its rationale. [Yeah. Oh, and then there's also pr 2603](https://github.com/sila-chain/consensus-specs/issues/2603). Oh, I guess anything else on this specific topic?
...
Cool. And that's [issue 2608 in the consensus specs](https://github.com/sila-chain/consensus-specs/issues/2608).
...
# **[Proposal to include SIP-3756: Gas Limit Cap](https://youtu.be/NorHRk5fFZU?t=2657)**

**Tim Beiko**

Next up, like client wanted to give an update on SIP-3756. So the gas limit cap. Yeah, but right.
...
Well, I think actually that all core devs would be fine at making the decision, my concern would be that takes much longer, like how long does a reason, like take from an all core devs decision like? Will it take an SIP every time, which takes what at least half a year or something?

**Micah Zoltu**

Yeah, so when we want to change, increase it, we wouldn't do any if if this SIP passes, and we want to presumably would have some default, I'm guessing 30 million is reasonable default. If we want to then change it later to something bigger, so 45 million or whatever, then we would need to have any SIP and then wait for the next hard fork can get included in hard fork.
...
My question is basically, is this SIP? Is the motivation for it is that we think that before the merge, miners are going to be like, not have the long view on Sila. Let's just say YOLO and in the last time they have? Or is the is the is the motivation more longer than that? Do you think that even in the future when we're up to the merge, if it's going to be needed, or wanted or desirable,
...
Yeah, it's me, Andrew. Well, I think our position is the same as before that we are not in favour of this SIP. But if like the majority decides that we are going through with this SIP then we are not going to fight, and we're not going to die on that hill.

Another question that would be is, like, if we were to move forward with this SIP would the plan be basically have it ready for the future for after the merge, or what the plan B to basically bundle the merge with basically, as like the one exception to the rule of not having features, as part of the merge, just so in case we need it earlier, it were already be implemented and ready for like a short notice emergency fork. I mean, again, with the understanding that he probably wouldn't do that unless it would start being a problem.

**Matt Garnett**

I think the question is more, Are we trying to put it in SilaShanghai? Or we try and do a soft fork before SilaShanghai?
...
Yeah, I would, I would oppose moving this into SilaShanghai or like the next fork. Just because I think we should really keep the focus as clean as possible. And if we were to include a new SIP, it has to be like a hard consensus critical SIP and don't think that this SIP is security critical enough to do that. So I'm in favour of the SIP, but I would want to have the fork in November be as small as possible. And if we were to need something that it would need to be security critical at this point. And not something that may be introduced in the future or something?
...
Yeah, we can have in the back pocket, if we have to, and not otherwise, schedule it as part of SilaShanghai. That's possible.

**Alex Beregszaszi**

I do want to mention, and I apologise if this actually in the in the spec there. But I think it's implied to me that it wasn't because software. If the gas limit on SilaMainnet is already above the target that we'd want to select that you would actually need some sort of hard fork to discreetly change it to that point, is that already coverd in the SIP? apologise if it is? It's not?
...
I didn't quite understand the exact argument against bundling this in SilaShanghai.
...
The exception to that would be if we do see the gas limit being raised to like, levels we consider unsafe on SilaMainnet, having this already speced out it would basically increase the the urgency that we need this with having it already speced out to be valuable. But assuming the kind of urgency for it stays the same, it seems like it, it just might be better to focus on the merge exclusively until we're at least farther along with the implementation of it.
...
I'm fine. Moving on to the next things in the call. I'm still of the mindset that I would like to see it in SilaShanghai, but I understand it seems like that's the minority view.

# [Proposal to add SIP-3855 (PUSH0 instruction) into SilaShanghai](https://youtu.be/NorHRk5fFZU?t=4192)
...
I'm one of the co authors so I'm obviously in favour. But what I wanted to add was I've been basically wanting to have something like this for a number of years. That's why I wrote the limitation of the initcode SIP earlier. That one is a bit of a hack. And I think it's not great. This is more rigorous attach that adds a linear cost, which we are, which is proper, because it is, I think it is currently a denial of service factor against Sila which we should fix. And we should have fixed a couple years ago. But yeah, we're here now. That's it.
...
I was wondering why why should this go in with the merchant and maybe not before? So like in SilaShanghai just to reduce the scope of the merge.
...
I guess my understanding of the pushback against the other two simpler SIPs is that if we wanted to have SilaShanghai not have any features, The initial energy required to have other features in the fork was great enough to not include these, like relatively simple SIPs. But if people want to do SIP-3860, then there has to be effort put into building out the tests, then it seems like the marginal cost of also adding one or two of these other SIPs is incredibly small, because the decision has already been made have features into the fork. Is that a misunderstanding?

**Tim Beiko**

But are you referring to SilaShanghai?

**Matt Garnett**

SilaShanghai or the merge? I am personally fairly opposed to including anything in the merge doesn't need to absolutely go in. But I'm specifically curious about SilaShanghai.

**Marius Van Der Wijden**

I would also be opposed to putting this in the merge. I was thinking about SilaShanghai for for this.

**Tim Beiko**

Got it. Sorry. So I got that wrong. I do think that case, yeah, your argument is probably good lightclient were like, yeah, it if we are going to set up infrastructure for SilaShanghai with this, then it basically becomes a proper feature fork and we need testing and we need to deploy across the testnets and whatnot, then that's. Yeah, that's like a much bigger overhead.
...
Are people strongly in favour of trying to SIP-3860 in SilaShanghai?
...
What is the timeline look like to make this happen, then if we want to do this in SilaShanghai?
