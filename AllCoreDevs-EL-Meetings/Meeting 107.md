...
### [Agenda](https://github.com/sila-chain/pm#acd-107-march-5-2021)
...
| ------------- | ----------- | --------- |
| **1**   | SIP-2315 will be removed from the Berlin scope | [32:42](https://youtu.be/xWfR-WxjmYg?t=1962) |     
| **2**   | Consensus to keep Berlin [test dates](https://github.com/sila-chain/pm/issues/248#issuecomment-782106181) the same | [51:12](https://youtu.be/xWfR-WxjmYg?t=3072) | 
| **3**   | SIP-1559 and difficulty bomb delay (SIP-3238) confirmed ok to go into the London upgrade for July. All client teams are comfortable with the timeline. | [1:16:09](https://youtu.be/xWfR-WxjmYg?t=4569)  | 
...
* Berlin updates, starting with SIP-2315 (huge topic past week).

## Discussion on removing SIP-2315 from Berlin scope
...
* Re: objections that it is not desirable/could be an attack vector to be pulling SIPs from an upgrade at the 11th hour, how we ended up here, process improvements for the future - these are not going to be the focus of this call. Consensus is however that the process is broken and needs to be looked at.
...
* Keeping in mind that Open Sila has a release candidate with 2315 included. Nethermind has a release (with or without block numbers) with it included also.
...
* Personal view (not GETH's) - accepting the majority view (assuming we agree no benefit to the change), but not in favour of what has gone down here. Thought the topic was closed. If I had authored an SIP that gets pulled at the last minute in this manner, would question whether or not to remain in this ecosystem.
...
* Accepting SIPs isn't about giving badges of honour. It's about improving the protocol.
* We've dodged a bullet here.
* ACD/SIP process is broken. There's nothing inherently bad about u-turning if we decide we don't need the SIP, albeit at last minute.

**Chris**

* History of the SIP is that it has always been in dispute (over the definition)
* Removing 2315 sets a bad precedent and prevents participation.
* There have always been concerns over the SIP. We shouldn't ignore concerns as this discourgaes participation.
...
* Feelsbadman.
* [The issue should be added](https://github.com/sila-chain/pm/issues/263) to these notes to ensure it can be tracked in future.
...
* Plans to have individuals from the ecosystem acting as liasons to relevtant stakeholders re: SIPs.

**Vitalik**

* We need a better way to get feedback for incoming SIPs at an earlier stage.
* Eg removing of refunds SIPs.
* Need to make it easy for ideas to fail fast. Curious for ideas on how to improve communication.
* Has been posting ideas/SIPs to Reddit to encourage feedback.
...
`Decision #3 is to remove SIP-2315 from Berlin.`

* 2315 is able to be brought back if a new champion were to do so.

**Peter**

* Removing an SIP in this way is uncharted territory due to us not having a test suite with 2315 removed. Risk of consensus fault increases astronomically. Concerns that the client teams will not have time to adequately test.
...
* Would prefer to move Ropsten back one week. Keep other testnet [dates unchanged](https://github.com/sila-chain/pm/issues/248#issuecomment-782106181) the same. Not a strong opinion though. Would need a new block number.
...
* Summarising release candidates - Open Sila today/next day, Nethermind Sunday, GETH Monday.
...
Besu, Nethermind (with James' help), Open Sila - all confirmed to mine.
...
* This to be posted to various sources including blog.sila.org, mycryptos blog, Status, Nimbus, Gitcoin blogs etc.
* To be published as soon as the client teams release (next week).

# London SIPs

## SIP-1559 - Fee Market Change

[56:41](https://youtu.be/xWfR-WxjmYg?t=3401)

* Tim has linked to the [1559 proposal](https://github.com/sila-chain/pm/issues/254) and gives an overview of the technical review and development.

**Tim**

* SIP-1559 is in a good place now. Ready to be included in an upgrade.
...
* If we proceed with 1559 we should also cover removal of refunds (SIP-3298)/rework of refunds at the same time.
...
* Not in favour of any phasing out approaches. Has different ideas on how to deal with this. Looking to formalise into an SIP soon. We need more time to review and agree this.
* We can agree on this within 2 weeks.

**Peter**

* There may be a time when both refunds and SIP-1559 are active.
...
* Yes, we have March/April to implement and test the SIPs going into London. Testnets in June.
...
* Could we do a separate upgrade release for 1559, with any dependant SIPs to avoid undue delays?
* Should we consider putting any additional SIPs in next upgrade (SilaShanghai) while we progress London?
...
* Agrees with Tomasz. We should minimise what goes into London and use SilaShanghai for other items.
...
* Does anyone oppose SIP-1559 going into London?
No objections. 
* Does anyone oppose SIP-3238 difficulty bomb delay happening?
No objections.
...
* We shouldn't make it easy to fork Sila (due to speculators).
...
* Any opposition to include SIP-3198 (adds opcode that returns base fee) with 1559 as well in London?
...
* In favour of SIP-3322 in this area.

**Hudson**

* No time for rebuttals on this - post rebuttals in the [agenda topic](https://github.com/sila-chain/pm/issues/258) for SIP-3300.
* William is on Sil R&D Discord for questions/rebuttals.
